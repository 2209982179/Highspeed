...
namespace highspeed.framework
{
    ...
    public static class CollectionExtension
    {
        ...
        public static bool AreEqual<T>(this IList<T> sourceCollection, object targetCollection)
        {
            ...
            if (targetCollection is IList<T>)
            {
                var target = targetCollection as IList<T>;
                if (sourceCollection.Count != target.Count)
                {
                    return false;
                }

                return sourceCollection.Except(target).Count() == 0
                && target.Except(sourceCollection).Count() == 0;
            }
