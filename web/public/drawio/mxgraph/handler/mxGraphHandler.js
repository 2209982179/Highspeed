  ...
  if (state != null) {
    if (me.isSource(state.control)) {
      ...
      if (!this.graph.isToggleEvent(me.getEvent()) || !mxEvent.isAltDown(me.getEvent())) {
        ...
        while (this.graph.view.getState(parent) != null && (model.isVertex(parent) || model.isEdge(parent) && !this.graph.isToggleEvent(me.getEvent())) && this.isPropagateSelectionCell(cell, false, me)) {
          cell = parent;
...
mxGraphHandler.prototype.start = function (cell, x, y, cells) {
  ...
  if (this.first == null && this.graph.isCellMovable(cell) && (!model.isEdge(cell) || this.graph.getSelectionCount() > 1 || geo.points != null && geo.points.length > 0 || model.getTerminal(cell, true) == null || model.getTerminal(cell, false) == null || this.graph.allowDanglingEdges)) {
    this.cell = cell;
    ...
    if (this.guidesEnabled) {
      ...
      this.guide.isStateIgnored = mxUtils.bind(this, function (state) {
        ...
        return state.cell != null && (!this.cloning && this.isCellMoving(state.cell) || state.cell != (this.target || parent) && !ignore && !connected.get(state) && (this.target == null || this.graph.model.getChildCount(this.target) >= 2) && p != (this.target || parent));
      });
    }
  }
};
...
mxGraphHandler.prototype.mouseMove = function (sender, me) {
  ...
  if (this.first == null && this.delayedSelection && this.cell != null && this.mouseDownX != null && this.mouseDownY != null && (Math.abs(this.mouseDownX - me.getX()) > tol || Math.abs(this.mouseDownY - me.getY()) > tol)) {
    ...
    this.start(this.cell, this.mouseDownX, this.mouseDownY, this.getCells(null, graph.getSelectionCells().concat(me.getCell())));
  }
  ...
  if (!me.isConsumed() && graph.isMouseDown && this.cell != null && delta != null && this.bounds != null && !this.suspended) {
    ...
    if (this.shape != null || this.livePreviewActive || this.cloning || Math.abs(delta.x) > tol || Math.abs(delta.y) > tol) {
      ...
      var cell = me.getCell();
      cell = cell != null && mxUtils.indexOf(this.cells, cell) < 0 ? cell : graph.getCellAt(me.getGraphX(), me.getGraphY(), null, null, null, mxUtils.bind(this, function (state, x, y) {
