...
mxVertexHandler.prototype.resizeCell = function (cell, dx, dy, index, gridEnabled, constrained, recurse) {
  ...
  if (geo != null) {
    if (index == mxEvent.LABEL_HANDLE) {
      ...
      var sin = Math.sin(alpha);
      var scale = this.graph.view.scale;
      var pt = mxUtils.getRotatedPoint(new mxPoint(Math.round((this.labelShape.bounds.getCenterX() - this.startX) / scale), Math.round((this.labelShape.bounds.getCenterY() - this.startY) / scale)), cos, sin);
      if (!horz) {
        pt.y = -pt.y;
      }
      geo = geo.clone();
...
mxVertexHandler.prototype.redrawHandles = function () {
  ...
  if (this.customHandles != null) {
    for (var i = 0; i < this.customHandles.length; i++) {
      if (this.customHandles[i] != null) {
        ...
        this.customHandles[i].shape.node.style.visibility = !this.handlesVisible || !this.isHandlesVisible() || !this.isCustomHandleVisible(this.customHandles[i]) || this.graph.isEditing() ? 'hidden' : '';
      }
    }
  }
  ...
  if (this.rotationShape != null) {
    ...
    if (this.rotationShape.node != null) {
      ...
      this.rotationShape.node.style.visibility = this.state.view.graph.isEditing() || !this.handlesVisible || !this.isHandlesVisible() || !this.isRotationHandleVisible() ? 'hidden' : '';
    }
  }
  if (this.selectionBorder != null) {
    this.selectionBorder.rotation = Number(this.state.style[mxConstants.STYLE_ROTATION] || '0');
