...
mxSvgCanvas2D.prototype.updateTextNodes = function (x, y, w, h, align, valign, wrap, overflow, clip, rotation, g) {
  ...
  mxSvgCanvas2D.createCss(w + this.foreignObjectPadding, h, align, valign, wrap, overflow, clip, this.state.fontBackgroundColor != null ? this.state.fontBackgroundColor : null, this.state.fontBorderColor != null ? this.state.fontBorderColor : null, 'display: flex; align-items: unsafe ' + (valign == mxConstants.ALIGN_TOP ? 'flex-start' : valign == mxConstants.ALIGN_BOTTOM ? 'flex-end' : 'center') + '; ' + 'justify-content: unsafe ' + (align == mxConstants.ALIGN_LEFT ? 'flex-start' : align == mxConstants.ALIGN_RIGHT ? 'flex-end' : 'center') + '; ', this.getTextCss(), s, mxUtils.bind(this, function (dx, dy, flex, item, block) {
    ...
    box.setAttribute('data-drawio-colors', 'color: ' + this.state.fontColor + '; ' + (this.state.fontBackgroundColor == null ? '' : 'background-color: ' + this.state.fontBackgroundColor + '; ') + (this.state.fontBorderColor == null ? '' : 'border-color: ' + this.state.fontBorderColor + '; '));
