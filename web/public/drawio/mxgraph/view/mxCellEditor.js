  ...
  if (state == null) {
    ...
    if (!this.autoSize || state.style[mxConstants.STYLE_OVERFLOW] == 'fill') {
      ...
      if (isEdge) {
        ...
        if (!state.view.graph.cellRenderer.legacySpacing || state.style[mxConstants.STYLE_OVERFLOW] != 'width' && state.style[mxConstants.STYLE_OVERFLOW] != 'block') {
      ...
      if (this.graph.isWrapping(state.cell) && (this.bounds.width >= 2 || this.bounds.height >= 2)) {
        ...
        if (this.textarea.innerHTML != this.getEmptyLabelText()) {
          ...
          } else if (state.style[mxConstants.STYLE_OVERFLOW] == 'block' || state.style[mxConstants.STYLE_OVERFLOW] == 'width') {
...
mxCellEditor.prototype.getEditorBounds = function (state) {
  ...
  if (!isEdge && state.view.graph.cellRenderer.legacySpacing && state.style[mxConstants.STYLE_OVERFLOW] == 'fill') {
    ...
    var spacingTop = (parseFloat(mxUtils.getValue(style, mxConstants.STYLE_SPACING_TOP, 0)) + mxText.prototype.baseSpacingTop) * scale + spacing;
    var spacingRight = (parseFloat(mxUtils.getValue(style, mxConstants.STYLE_SPACING_RIGHT, 0)) + mxText.prototype.baseSpacingRight) * scale + spacing;
    var spacingBottom = (parseFloat(mxUtils.getValue(style, mxConstants.STYLE_SPACING_BOTTOM, 0)) + mxText.prototype.baseSpacingBottom) * scale + spacing;
    var spacingLeft = (parseFloat(mxUtils.getValue(style, mxConstants.STYLE_SPACING_LEFT, 0)) + mxText.prototype.baseSpacingLeft) * scale + spacing;
