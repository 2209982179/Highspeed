...
mxGraph.prototype.isValidDropTarget = function (cell, cells, evt) {
  return cell != null && !this.isCellLocked(cell) && (this.isSplitEnabled() && this.isSplitTarget(cell, cells, evt) || !this.model.isEdge(cell) && (this.isSwimlane(cell) || this.model.getChildCount(cell) > 0 && !this.isCellCollapsed(cell) || this.model.getChildCount(cell) === 0));
};
    ...
    if (parent != null) {
      ...
      for (var i = 0; i < childCount; i++) {
        ...
        if (state != null && this.isCellVisible(cell) && (ignoreFn == null || !ignoreFn(state))) {
          ...
          var hit = intersection != null && model.isVertex(cell) && mxUtils.intersects(intersection, box) || intersection != null && model.isEdge(cell) && mxUtils.intersects(intersection, box) || intersection == null && (model.isEdge(cell) || model.isVertex(cell)) && box.x >= x && box.y + box.height <= bottom && box.y >= y && box.x + box.width <= right;
          if (hit) {
            result.push(cell);
          }
...
mxGraph.prototype.selectCells = function (vertices, edges, parent, selectGroups) {
  ...
  var filter = mxUtils.bind(this, function (cell) {
    return this.view.getState(cell) != null && ((selectGroups || this.model.getChildCount(cell) == 0) && this.model.isVertex(cell) && vertices && !this.model.isEdge(this.model.getParent(cell)) || this.model.isEdge(cell) && edges);
  });
