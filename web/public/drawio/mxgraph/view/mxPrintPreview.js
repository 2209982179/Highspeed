...
mxPrintPreview.prototype.writeHead = function (doc, css) {
  ...
  if (this.addPageCss && pf != null) {
    var size = (pf.width / this.pixelsPerInch + this.pageMargin).toFixed(2) + 'in ' + (pf.height / this.pixelsPerInch + this.pageMargin).toFixed(2) + 'in;';
    doc.writeln('@page {');
...
mxPrintPreview.prototype.addGraphFragment = function (dx, dy, scale, pageNumber, div, clip) {
  ...
  if (this.clipping) {
    var tempClip = new mxRectangle((clip.x + translate.x) * s, (clip.y + translate.y) * s, clip.width * s / realScale, clip.height * s / realScale);
    var self = this;
