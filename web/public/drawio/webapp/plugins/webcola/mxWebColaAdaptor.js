 ...
 */
{
  ...
  colaAdaptor.nodes(this.nodes).links(this.links).groups(this.groups).size(this.dimension).linkDistance(function (link) {
    return link.length;
  });
  layout.callback = function () {
    layout.renderingChain(colaAdaptor);
  };
  colaAdaptor.avoidOverlaps(options.avoidOverlap).handleDisconnected(options.handleDisconnected)
 ...
 */
{
  ...
  for (var id in cells) {
    var cell = cells[id];
    if (!cell.isVertex() || !this.isLeafOrCollapsed(cell)) continue;
  ...
  for (var i = 0; i < preliminaryGroups.length; i++) {
    ...
    for (var j = 0; j < preliminaryGroups.length; j++) {
      ...
      if (parentId == groupParentId) parentGroup.groups.push(j);
    }
  }
  // finalize groups
  var groups = [];
 ...
 */
{
  if (cell.isCollapsed() || cell.children == null || cell.children.length == 0 || typeof this.graph.getCellStyle(cell)['childLayout'] != 'undefined') {
    return true;
  }
 ...
 */
{
  ...
  if (childrenA == null || childrenA.length == 0 || childrenB == null || childrenB.length == 0) return result;
