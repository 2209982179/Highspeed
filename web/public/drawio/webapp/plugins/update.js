      ...
      for (var id in graph.model.cells) {
        ...
        if (graph.model.isEdge(cell)) {
          ...
          if (Math.random() > 0.5 && state != null && state.length > 50) {
            ...
            update.setAttribute('value', '<object label="%load% minutes" load="' + Math.round(Math.random() * 100) + '" placeholders="1">');
          ...
          if (cell.value != null && typeof cell.value == 'object' && cell.value.getAttribute('update') == '1') {
            ...
            if (Math.random() > 0.5) {
              var update = doc.createElement('update');
              update.setAttribute('id', cell.id);
              update.setAttribute('value', '<object tooltip="%load%% Done" load="' + Math.round(Math.random() * 100) + '" placeholders="1">');
