...
var mxIsElectron = navigator.userAgent != null && navigator.userAgent.toLowerCase().indexOf(' electron/') > -1 && navigator.userAgent.indexOf(' draw.io/') > -1;
...
Editor.initMath((remoteMath ? 'https://app.diagrams.net/' : DRAW_MATH_URL) + 'math/es5/startup.js');
function render(data) {
  ...
  function loadExtFonts(extFonts) {
    ...
    for (var i = 0; i < extFonts.length; i++) {
      if (extFonts[i].url.indexOf(Editor.GOOGLE_FONTS) == 0 || extFonts[i].url.indexOf(Editor.GOOGLE_FONTS_CSS2) == 0) {
  ...
  function renderGrid() {
    ...
    var position = -Math.round(phase - mxUtils.mod(view.translate.x * view.scale - x0, phase)) + 'px ' + -Math.round(phase - mxUtils.mod(view.translate.y * view.scale - y0, phase)) + 'px';
    var pages = document.querySelectorAll('[id^=mxPage]');
    var cssTxt = 'margin: 0;padding: 0;background-image: ' + gridImage + ';background-position: ' + position + ';background-color: ' + document.body.style.backgroundColor;
  ...
  function renderPage(currentPageId) {
    ...
    if (data.format == 'pdf') {
      ...
      bg = data.bg != null && data.bg.length > 0 ? data.bg : xmlDoc.documentElement.getAttribute('background');
    ...
    //handle layers
    if (extras != null && (extras.layers != null && extras.layers.length > 0 || extras.layerIds != null && extras.layerIds.length > 0)) {
    ...
    if (data.print || data.format == 'pdf' && xmlDoc.documentElement.getAttribute('page') == '1' && data.w == 0 && data.h == 0 && data.scale == 1) {
      ...
      if (data.print) {
        ...
        if (data.fit) {
          ...
          data.scale = Math.min(data.pageHeight * v / (gb.height / graph.view.scale), data.pageWidth * h / (gb.width / graph.view.scale));
        } else {
    ...
    if (!graph.pdfPageVisible) {
      ...
      if (data.w > 0 || data.h > 0) {
        ...
        graph.view.scaleAndTranslate(s, Math.floor(data.border / s - Math.floor(b.x)), Math.floor(data.border / s - Math.floor(b.y)));
      } else {
        var s = data.scale;
        if (autoScale) {
