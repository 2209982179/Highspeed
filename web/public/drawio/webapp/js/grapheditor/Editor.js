...
Editor.isPngData = function (data) {
  return data.length > 8 && data.charCodeAt(0) == 137 && data.charCodeAt(1) == 80 && data.charCodeAt(2) == 78 && data.charCodeAt(3) == 71 && data.charCodeAt(4) == 13 && data.charCodeAt(5) == 10 && data.charCodeAt(6) == 26 && data.charCodeAt(7) == 10;
};
    ...
    EditorUi.parsePng(binary, mxUtils.bind(this, function (pos, type, length) {
      ...
      if (type == 'zTXt') {
        ...
        if (value.substring(0, idx) == 'mxGraphModel') {
          ...
          var xmlData = pako.inflateRaw(Graph.stringToArrayBuffer(value.substring(idx + 2)), {
            to: 'string'
          }).replace(/\+/g, ' ');
...
var PageSetupDialog = function (editorUi) {
  ...
  var applyBtn = mxUtils.button(mxResources.get('apply'), function () {
    ...
    if (graph.pageFormat.width != change.previousFormat.width || graph.pageFormat.height != change.previousFormat.height || !change.ignoreColor || !change.ignoreImage || change.shadowVisible != graph.shadowVisible) {
...
(function () {
  ...
  mxGraphView.prototype.validateBackgroundStyles = function (factor, cx, cy) {
    ...
    if (graph.isGridEnabled() || graph.gridVisible) {
      ...
      position = -Math.round(phase - mxUtils.mod(this.translate.x * this.scale - x0 + dx, phase) + ddx * factor) + 'px ' + -Math.round(phase - mxUtils.mod(this.translate.y * this.scale - y0 + dy, phase) + ddy * factor) + 'px';
    }
  ...
  mxGraph.prototype.panGraph = function (dx, dy) {
    ...
    if (this.shiftPreview1 != null) {
      ...
      var position = -Math.round(phase - mxUtils.mod(this.view.translate.x * this.view.scale + dx, phase)) + 'px ' + -Math.round(phase - mxUtils.mod(this.view.translate.y * this.view.scale + dy, phase)) + 'px';
      canvas.style.backgroundPosition = position;
    }
  };
    ...
    var drawPageBreaks = mxUtils.bind(this, function (breaks) {
      if (breaks != null) {
        ...
        for (var i = 0; i <= count; i++) {
          var pts = breaks == this.horizontalPageBreaks ? [new mxPoint(Math.round(bounds2.x), Math.round(bounds2.y + (i + 1) * bounds.height)), new mxPoint(Math.round(right), Math.round(bounds2.y + (i + 1) * bounds.height))] : [new mxPoint(Math.round(bounds2.x + (i + 1) * bounds.width), Math.round(bounds2.y)), new mxPoint(Math.round(bounds2.x + (i + 1) * bounds.width), Math.round(bottom))];
          if (breaks[i] != null) {
            breaks[i].points = pts;
            breaks[i].redraw();
          } else {
  ...
  mxGraphHandler.prototype.isPropagateSelectionCell = function (cell, immediate, me) {
    ...
    if (immediate) {
      ...
      result = !this.graph.model.isEdge(parent) && !this.graph.isSiblingSelected(cell) && (geo != null && geo.relative || !this.graph.isContainer(parent) || this.graph.isPart(cell));
    } else {
