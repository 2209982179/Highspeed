...
mxCodec.allowlist = ['mxStylesheet', 'Array', 'mxGraphModel', 'mxCell', 'mxGeometry', 'mxRectangle', 'mxPoint', 'mxChildChange', 'mxRootChange', 'mxTerminalChange', 'mxValueChange', 'mxStyleChange', 'mxGeometryChange', 'mxCollapseChange', 'mxVisibleChange', 'mxCellAttributeChange'];
        ...
        if (!mxEvent.isAltDown(me.getEvent()) && state != null) {
          ...
          if (!this.panningHandler.isActive() && !mxEvent.isControlDown(me.getEvent())) {
            ...
            if (handler == null || handler.getHandleForEvent(me) == null) {
              ...
              if (this.isTableCell(state.cell) && this.isCellMovable(state.cell) && !this.isCellSelected(state.cell) && (!mxUtils.contains(state, me.getGraphX() - t1, me.getGraphY() - t1) || !mxUtils.contains(state, me.getGraphX() - t1, me.getGraphY() + t1) || !mxUtils.contains(state, me.getGraphX() + t1, me.getGraphY() + t1) || !mxUtils.contains(state, me.getGraphX() + t1, me.getGraphY() - t1))) {
        ...
        if (this.isEnabled() && !this.panningHandler.isActive() && !mxEvent.isAltDown(me.getEvent())) {
          ...
          if (start.point != null && start.state != null && start.event != null) {
            ...
            if (state != null && this.isCellEditable(state.cell)) {
              ...
              if (this.model.isEdge(state.cell) && !this.isCellSelected(state.cell) && !mxEvent.isAltDown(me.getEvent()) && !mxEvent.isControlDown(me.getEvent()) && !mxEvent.isShiftDown(me.getEvent())) {
                ...
                if (this.isTableCell(state.cell) && this.isCellMovable(state.cell)) {
                  ...
                  var table = this.model.getParent(row);
                  if (!this.isCellSelected(table)) {
                    if (mxUtils.intersects(box, new mxRectangle(state.x, state.y - 2, state.width, 4)) && this.model.getChildAt(table, 0) != row || mxUtils.intersects(box, new mxRectangle(state.x, state.y + state.height - 2, state.width, 4))) {
                      cursor = 'row-resize';
                    } else if (mxUtils.intersects(box, new mxRectangle(state.x - 2, state.y, 4, state.height)) && this.model.getChildAt(row, 0) != state.cell || mxUtils.intersects(box, new mxRectangle(state.x + state.width - 2, state.y, 4, state.height))) {
                      cursor = 'col-resize';
                    }
                  }
                }
                ...
                while (cursor == null && current != null && (this.isTableCell(current.cell) || this.isTableRow(current.cell) || this.isTable(current.cell))) {
                  if (this.isSwimlane(current.cell) && this.isCellMovable(current.cell)) {
                    var offset = this.getActualStartSize(current.cell);
                    var s = this.view.scale;
                    if ((offset.x > 0 || offset.width > 0) && mxUtils.intersects(box, new mxRectangle(current.x + (offset.x - offset.width - 1) * s + (offset.x == 0 ? current.width * s : 0), current.y, 1, current.height))) {
                      cursor = 'col-resize';
                    } else if ((offset.y > 0 || offset.height > 0) && mxUtils.intersects(box, new mxRectangle(current.x, current.y + (offset.y - offset.height - 1) * s + (offset.y == 0 ? current.height : 0), current.width, 1))) {
                      cursor = 'row-resize';
                    }
                  }
                  current = this.view.getState(this.model.getParent(current.cell));
  ...
  if (typeof mxVertexHandler !== 'undefined') {
    ...
    this.graphHandler.getCells = function (initialCell) {
      ...
      for (var i = 0; i < cells.length; i++) {
        ...
        var cell = this.graph.isTableCell(initialCell) && this.graph.isTableCell(cells[i]) && this.graph.isCellSelected(cells[i]) ? this.graph.model.getParent(cells[i]) : this.graph.isTableRow(initialCell) && this.graph.isTableRow(cells[i]) && this.graph.isCellSelected(cells[i]) ? cells[i] : this.graph.getCompositeParent(cells[i]);
    ...
    this.graphHandler.shouldRemoveCellsFromParent = function (parent, cells, evt) {
      if (this.graph && this.graph.isCellSelected(parent)) {
        return false;
      }
    ...
    this.updateMouseEvent = function (me) {
      ...
      if (me.state != null && this.isCellLocked(this.getLayerForCell(me.getCell()))) {
        if (this.getLinkForCell(me.getCell()) == null) {
          me.state = this.view.getState(this.getCellAt(me.getGraphX(), me.getGraphY(), null, null, null, mxUtils.bind(this, function (state, x, y) {
 ...
 */
Graph.touchStyle = mxClient.IS_TOUCH || mxClient.IS_FF && mxClient.IS_WIN || navigator.maxTouchPoints > 0 || navigator.msMaxTouchPoints > 0 || window.urlParams == null || urlParams['touch'] == '1';
 ...
 */
Graph.layoutNames = ['mxHierarchicalLayout', 'mxCircleLayout', 'mxCompactTreeLayout', 'mxEdgeLabelLayout', 'mxFastOrganicLayout', 'mxParallelEdgeLayout', 'mxPartitionLayout', 'mxRadialTreeLayout', 'mxStackLayout'];
...
Graph.createSvgImage = function (w, h, data, coordWidth, coordHeight) {
  var tmp = unescape(encodeURIComponent(Graph.svgDoctype + '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="' + w + 'px" height="' + h + 'px" ' + (coordWidth != null && coordHeight != null ? 'viewBox="0 0 ' + coordWidth + ' ' + coordHeight + '" ' : '') + 'version="1.1">' + data + '</svg>'));
...
Graph.createSvgDarkModeStyle = function (svgDoc, theme, cssClass) {
  ...
  if (theme == 'auto') {
    cssClass = cssClass != null ? '.' + cssClass : '';
    css = '@media (prefers-color-scheme: dark) {' + css + '\n' + '	svg' + cssClass + '[style^="background-color: rgb(255, 255, 255);"] {' + '		background-color: ' + Editor.darkColor + ' !important;' + '	}}';
  }
...
Graph.htmlToPng = function (html, w, h, fn) {
  ...
  img.src = 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100">' + '<foreignObject width="100%" height="100%"><div xmlns="http://www.w3.org/1999/xhtml"><style>em{color:red;}</style><em>I</em> lick <span>cheese</span></div></foreignObject></svg>');
};
...
Graph.decompress = function (data, inflate, checked) {
  if (data == null || data.length == 0 || typeof pako === 'undefined') {
    ...
    var inflated = decodeURIComponent(inflate ? pako.inflate(tmp, {
      to: 'string'
    }) : pako.inflateRaw(tmp, {
      to: 'string'
    }));
  ...
  function exploreFromHere(sourceGraph, selectionCell) {
    ...
    function load(graph, cell) {
      if (graph.getModel().isVertex(cell)) {
        ...
        try {
          ...
          var r = Math.max(minSize, Math.min(graph.container.scrollWidth / 3 - 80, graph.container.scrollHeight / 3 - 80));
    ...
    function rootChanged(graph, cell) {
      ...
      if (cell.startIndex > 0) {
        var backCell = graph.createVertex(null, null, mxResources.get('previousPage') + '...', 0, 0, 100, 30, btnStyle);
      ...
      if (edges.length > (cell.startIndex || 0) + pageSize) {
        var moreCell = graph.createVertex(null, null, mxResources.get('nextPage') + '...', 0, 0, 100, 30, btnStyle);
...
DOMPurify.addHook('afterSanitizeAttributes', function (node) {
  if (node.nodeName == 'use' && (node.getAttribute('xlink:href') != null && !node.getAttribute('xlink:href').startsWith('#') || node.getAttribute('href') != null && !node.getAttribute('href').startsWith('#'))) {
    node.remove();
  }
});
 ...
 */
Graph.linkPattern = new RegExp('^(https?:\\/\\/)?' +
  // protocol
 ...
 */
Graph.prototype.baseUrl = urlParams['base'] != null ? decodeURIComponent(urlParams['base']) : (window != window.top ? document.referrer : document.location.toString()).split('#')[0];
 ...
 */
Graph.prototype.roundableShapes = ['label', 'rectangle', 'internalStorage', 'corner', 'parallelogram', 'swimlane', 'triangle', 'trapezoid', 'ext', 'step', 'tee', 'process', 'link', 'rhombus', 'offPageConnector', 'loopLimit', 'hexagon', 'manualInput', 'card', 'curlyBracket', 'singleArrow', 'callout', 'doubleArrow', 'flexArrow', 'umlLifeline'];
...
Graph.prototype.createLayouts = function (list) {
  ...
  for (var i = 0; i < list.length; i++) {
    if (mxUtils.indexOf(Graph.layoutNames, list[i].layout) >= 0) {
      ...
      var layout = list[i].layout == 'mxOrgChartLayout' && list[i].config != null ? new window[list[i].layout](this, list[i].config['branchOptimizer']) : new window[list[i].layout](this);
...
Graph.prototype.isZoomWheelEvent = function (evt) {
  return Graph.zoomWheel && !mxEvent.isShiftDown(evt) && !mxEvent.isMetaDown(evt) && !mxEvent.isAltDown(evt) && (!mxEvent.isControlDown(evt) || mxClient.IS_MAC) || !Graph.zoomWheel && (mxEvent.isAltDown(evt) || mxEvent.isControlDown(evt));
};
...
Graph.prototype.isSplitTarget = function (target, cells, evt) {
  return !this.model.isEdge(cells[0]) && !mxEvent.isAltDown(evt) && !mxEvent.isShiftDown(evt) && !this.isCellLocked(this.getLayerForCell(target)) && mxGraph.prototype.isSplitTarget.apply(this, arguments);
};
...
/** setUseCssTransforms */
Graph.prototype.setUseCssTransforms = function (value) {
...
Graph.prototype.selectCellForEvent = function (cell, evt) {
  if ((cell === null || cell === void 0 ? void 0 : cell.vertex) == 1) {
    //** 由于mx的元素选择事件，在处理组内元素的点击事件的行为不符合预期，所以阻止mx原生事件，在点击事件中处理元素选择逻辑。
    return;
  }
...
Graph.prototype.connectVertex = function (source, direction, length, evt, forceClone, ignoreCellAt, createTarget, done) {
  ...
  var pt = source.geometry.relative && source.parent.geometry != null ? new mxPoint(source.parent.geometry.width * source.geometry.x, source.parent.geometry.height * source.geometry.y) : new mxPoint(composite.geometry.x, composite.geometry.y);
  ...
  var execute = mxUtils.bind(this, function (targetCell) {
    if (createTarget == null || targetCell != null || target == null && cloneSource) {
      ...
      try {
        ...
        var edge = mxEvent.isControlDown(evt) && mxEvent.isShiftDown(evt) && duplicate || target == null && cloneSource ? null : this.insertEdge(this.model.getParent(source), null, '', source, realTarget, this.createCurrentEdgeStyle());
        if (edge != null) {
          result.push(edge);
...
Graph.prototype.isCellConnectable = function (cell) {
  var style = this.getCurrentCellStyle(cell);
  return !this.isCellLocked(this.getLayerForCell(cell)) && (style['connectable'] != null ? style['connectable'] != '0' : mxGraph.prototype.isCellConnectable.apply(this, arguments));
};
...
Graph.prototype.isCellFoldable = function (cell) {
  var style = this.getCurrentCellStyle(cell);
  return this.foldingEnabled && mxUtils.getValue(style, mxConstants.STYLE_RESIZABLE, '1') != '0' && (style['treeFolding'] == '1' || !this.isCellLocked(cell) && (this.isContainer(cell) && style['collapsible'] != '0' || !this.isContainer(cell) && style['collapsible'] == '1'));
};
 ...
 * Up arrow.
 */
HoverIcons.prototype.triangleUp = !mxClient.IS_SVG ? new mxImage(IMAGE_PATH + '/triangle-up.png', 26, 14) : Graph.createSvgImage(18, 28, '<path d="m 6 26 L 12 26 L 12 12 L 18 12 L 9 1 L 1 12 L 6 12 z" ' + 'stroke="#fff" fill="' + HoverIcons.prototype.arrowFill + '"/>');

/**
 * Right arrow.
 */
HoverIcons.prototype.triangleRight = !mxClient.IS_SVG ? new mxImage(IMAGE_PATH + '/triangle-right.png', 14, 26) : Graph.createSvgImage(26, 18, '<path d="m 1 6 L 14 6 L 14 1 L 26 9 L 14 18 L 14 12 L 1 12 z" ' + 'stroke="#fff" fill="' + HoverIcons.prototype.arrowFill + '"/>');

/**
 * Down arrow.
 */
HoverIcons.prototype.triangleDown = !mxClient.IS_SVG ? new mxImage(IMAGE_PATH + '/triangle-down.png', 26, 14) : Graph.createSvgImage(18, 26, '<path d="m 6 1 L 6 14 L 1 14 L 9 26 L 18 14 L 12 14 L 12 1 z" ' + 'stroke="#fff" fill="' + HoverIcons.prototype.arrowFill + '"/>');

/**
 * Left arrow.
 */
HoverIcons.prototype.triangleLeft = !mxClient.IS_SVG ? new mxImage(IMAGE_PATH + '/triangle-left.png', 14, 26) : Graph.createSvgImage(28, 18, '<path d="m 1 9 L 12 1 L 12 6 L 26 6 L 26 12 L 12 12 L 12 18 z" ' + 'stroke="#fff" fill="' + HoverIcons.prototype.arrowFill + '"/>');

/**
 * Round target.
 */
HoverIcons.prototype.roundDrop = !mxClient.IS_SVG ? new mxImage(IMAGE_PATH + '/round-drop.png', 26, 26) : Graph.createSvgImage(26, 26, '<circle cx="13" cy="13" r="12" ' + 'stroke="#fff" fill="' + HoverIcons.prototype.arrowFill + '"/>');
...
HoverIcons.prototype.init = function () {
  ...
  this.graph.click = mxUtils.bind(this, function (me) {
    let cell = me.getCell();
    //**  点击了画布中的一个元素，并且是鼠标左键点击
    if (cell && cell.vertex && me.evt.button == 0) {
      if (mxEvent.isControlDown(me.evt)) {
        //	按下Ctrl表示多选
        // 多选模式下，如果元素已选中，再次点击时时表示取消选中
        if (this.graph.isCellSelected(cell)) {
...
HoverIcons.prototype.click = function (state, dir, me) {
  ...
  if (tmp != null && this.graph.model.isEdge(tmp.cell) && !this.graph.isCloneEvent(evt) && (tmp.getVisibleTerminalState(true) == state || tmp.getVisibleTerminalState(false) == state)) {
...
HoverIcons.prototype.execute = function (state, dir, me) {
  var evt = me.getEvent();
  this.graph.selectCellsForConnectVertex(this.graph.connectVertex(state.cell, dir, this.graph.defaultEdgeLength, evt, this.graph.isCloneEvent(evt), this.graph.isCloneEvent(evt)), evt, this);
};
...
HoverIcons.prototype.update = function (state, x, y) {
  if (!this.graph.connectionArrowsEnabled || this.graph.freehand != null && this.graph.freehand.isDrawing() || state != null && mxUtils.getValue(state.style, 'allowArrows', '1') == '0') {
    this.reset();
  } else {
...
Graph.prototype.createTable = function (rowCount, colCount, w, h, title, startSize, tableStyle, rowStyle, cellStyle) {
  ...
  rowStyle = rowStyle != null ? rowStyle : 'shape=tableRow;horizontal=0;startSize=0;swimlaneHead=0;swimlaneBody=0;strokeColor=inherit;' + 'top=0;left=0;bottom=0;right=0;collapsible=0;dropTarget=0;fillColor=none;points=[[0,0.5],[1,0.5]];portConstraint=eastwest;';
  cellStyle = cellStyle != null ? cellStyle : 'shape=partialRectangle;html=1;whiteSpace=wrap;connectable=0;strokeColor=inherit;' + 'overflow=hidden;fillColor=none;top=0;left=0;bottom=0;right=0;pointerEvents=1;';
...
(function () {
  ...
  mxShape.prototype.paint = function () {
    ...
    if (this.state != null && this.node != null && this.state.view.graph.enableFlowAnimation && this.state.view.graph.model.isEdge(this.state.cell) && mxUtils.getValue(this.state.style, 'flowAnimation', '0') == '1') {
          ...
          for (var e = 0; e < this.validEdges.length; e++) {
            ...
            if (pts2 != null && mxUtils.intersects(state, state2) && state2.style['noJump'] != '1') {
              ...
              for (var j = 0; j < pts2.length - 1; j++) {
                ...
                if (pt != null && (Math.abs(pt.x - p0.x) > thresh || Math.abs(pt.y - p0.y) > thresh) && (Math.abs(pt.x - p1.x) > thresh || Math.abs(pt.y - p1.y) > thresh)) {
                  var dx = pt.x - p0.x;
                  var dy = pt.y - p0.y;
                  var temp = {
                    distSq: dx * dx + dy * dy,
...
if (typeof mxVertexHandler !== 'undefined') {
  (function () {
    ...
    Graph.prototype.createCurrentEdgeStyle = function () {
      ...
      var keys = ['shape', 'curved', 'rounded', 'comic', 'sketch', 'fillWeight', 'hachureGap', 'hachureAngle', 'jiggle', 'disableMultiStroke', 'disableMultiStrokeFill', 'fillStyle', 'curveFitting', 'simplification', 'comicStyle', 'jumpStyle', 'jumpSize'];
    ...
    Graph.prototype.isValidDropTarget = function (cell, cells, evt) {
      ...
      //** 连线不可停靠（默认行为中，元素停靠在连线上会使这个连线切成两段） */
      if(cell.edge){
        return false
      }
      //** 组合元素，临时元素不可停靠 */
      if(cell.isTemporary == true){
        return false
      }
      //** Label元素不可停靠 */
      if(cell.type == 'Label'){
      ...
      //** 区域分割图元，不能离开父元素 */
      if(cells.length > 0 && cells[0].Object_Type == 'Partitions' && cells[0].parent.id != cell.id){
        return false
      }
      
      if (cells.every(x=>x.Stereotype == 'State')) {
        // 状态图图元可以调整父子关系
      }
      // 选中元素都允许通过拖曳修改父子关系，并且目标可以停靠
      else if ((cell.canBeDropParent == true || cell.canBeDropParent == 'true') && cells.every(x=>x.canMoveToUpdateParent == true)) {
        // 
      }else{
        //** 不允许通过停靠修改父子关系 */
        var handle =
        cells.filter((x) => x.parent.id !== cell.id).length > 0
        if(handle){
          return false
        }
      }

      return !this.isCellLocked(cell) && (this.isTargetShape(cell, cells, evt) || (mxUtils.getValue(style, 'part', '0') != '1' || this.isContainer(cell)) && mxUtils.getValue(style, 'dropTarget', '1') != '0' && (mxGraph.prototype.isValidDropTarget.apply(this, arguments) || this.isContainer(cell)) && !this.isTableRow(cell) && (!this.isTable(cell) || rows || tables));
    };
     ...
     * @param {number} dy Y-coordinate of the translation.
     */
    Graph.prototype.getSvg = function (background, scale, border, nocrop, crisp, ignoreSelection, showText, imgExport, linkTarget, hasShadow, incExtFonts, theme, exportType, cells) {
      var lookup = null;
      if (cells != null) {
      ...
      try {
        ...
        var bounds = exportType == 'page' ? this.view.getBackgroundPageBounds() : ignoreSelection && lookup == null || nocrop || exportType == 'diagram' ? this.getGraphBounds() : this.getBoundingBox(this.getSelectionCells());
        var vs = this.view.scale;
        if (exportType == 'diagram' && this.backgroundImage != null) {
          bounds = mxRectangle.fromRectangle(bounds);
          bounds.add(new mxRectangle((this.view.translate.x + this.backgroundImage.x) * vs, (this.view.translate.y + this.backgroundImage.y) * vs, this.backgroundImage.width * vs, this.backgroundImage.height * vs));
        }
        if (bounds == null) {
        ...
        imgExport.drawCellState = function (state, canvas) {
          ...
          canvas.stroke = function () {
            if (this.root != null && this.node != null && this.node.nodeName == 'path' && graph.enableFlowAnimation && graph.model.isEdge(state.cell) && mxUtils.getValue(state.style, 'flowAnimation', '0') == '1') {
    ...
    Graph.prototype.getSelectedTextBlocks = function () {
      ...
      for (var i = 0; i < nodes.length; i++) {
        var node = nodes[i];
        while (this.cellEditor.textarea != null && this.cellEditor.textarea.contains(node) && node != this.cellEditor.textarea && node.parentNode != null) {
    ...
    Graph.prototype.getSelectedEditingElement = function () {
      ...
      if (node != null) {
        ...
        if (node == this.cellEditor.textarea && this.cellEditor.textarea.children.length == 1 && this.cellEditor.textarea.firstChild.nodeType == mxConstants.NODETYPE_ELEMENT) {
    ...
    Graph.prototype.initTouch = function () {
      ...
      this.popupMenuHandler.mouseUp = mxUtils.bind(this, function (sender, me) {
        ...
        this.popupMenuHandler.popupTrigger = !this.isEditing() && this.isEnabled() && (me.getState() == null || !me.isSource(me.getState().control)) && (this.popupMenuHandler.popupTrigger || !menuShowing && !isMouseEvent && (selectionEmpty && me.getCell() == null && this.isSelectionEmpty() || cellSelected && this.isCellSelected(me.getCell())));
    ...
    mxVertexHandler.prototype.isGhostPreview = function () {
      return vertexHandlerIsGhostPreview.apply(this, arguments) && !this.graph.isTable(this.state.cell) && !this.graph.isTableRow(this.state.cell) && !this.graph.isTableCell(this.state.cell) && !this.graph.isSwimlane(this.state.cell);
    };
    ...
    mxVertexHandler.prototype.isRecursiveResize = function (state, me) {
      return false; //** 父元素尺寸变化，不影响子元素 */
      return this.graph.isRecursiveVertexResize(state) && !mxEvent.isAltDown(me.getEvent());
    };

    //** 阻止图元克隆 */
    mxGraph.prototype.isCellCloneable = function (state, me) {
      return false;
    };
    ...
    mxVertexHandler.prototype.refresh = function () {
      ...
      else if (this.graph.getSelectionCount() == 1 && this.graph.isCellMovable(this.state.cell) && (this.graph.isTableCell(this.state.cell) || this.graph.isTableRow(this.state.cell))) {
        this.cornerHandles = [];
        for (var i = 0; i < 4; i++) {
    ...
    mxVertexHandler.prototype.rowHandleImage = Graph.createSvgImage(14, 12, '<rect x="2" y="2" width="10" height="3" stroke-width="1" stroke="#ffffff" fill="' + HoverIcons.prototype.arrowFill + '"/>' + '<rect x="2" y="7" width="10" height="3" stroke-width="1" stroke="#ffffff" fill="' + HoverIcons.prototype.arrowFill + '"/>');
    ...
    mxVertexHandler.prototype.rotationEnabled = false; // 元素旋转（true启动 false禁用）
    ...
    mxRubberband.prototype.isSpaceEvent = function (me) {
      return this.graph.isEnabled() && !this.graph.isCellLocked(this.graph.getDefaultParent()) && (mxEvent.isControlDown(me.getEvent()) || mxEvent.isMetaDown(me.getEvent())) && mxEvent.isShiftDown(me.getEvent()) && mxEvent.isAltDown(me.getEvent());
    };
    ...
    mxEdgeHandler.prototype.createHandleShape = function (index, virtual, target) {
      ...
      var img = pt != null ? !target ? this.fixedHandleImage : this.endFixedHandleImage : c != null && terminalState != null ? !target ? this.terminalHandleImage : this.endTerminalHandleImage : !target ? this.handleImage : this.endHandleImage;
      if (img != null) {
