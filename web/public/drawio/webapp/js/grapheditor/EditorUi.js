...
EditorUi = function (editor, container, lightbox) {
  ...
  if (!graph.standalone) {
    ...
    var styles = ['rounded', 'shadow', 'glass', 'dashed', 'dashPattern', 'labelBackgroundColor', 'labelBorderColor', 'comic', 'sketch', 'fillWeight', 'hachureGap', 'hachureAngle', 'jiggle', 'disableMultiStroke', 'disableMultiStrokeFill', 'fillStyle', 'curveFitting', 'simplification', 'sketchStyle', 'pointerEvents', 'strokeColor', 'strokeWidth', 'align', 'verticalAlign', 'spacingLeft', 'spacingRight', 'spacingTop', 'spacingBottom', 'spacing'];
    var connectStyles = ['shape', 'edgeStyle', 'curved', 'rounded', 'elbow', 'jumpStyle', 'jumpSize', 'comic', 'sketch', 'fillWeight', 'hachureGap', 'hachureAngle', 'jiggle', 'disableMultiStroke', 'disableMultiStrokeFill', 'fillStyle', 'curveFitting', 'simplification', 'sketchStyle'];
    // Styles to be ignored if applyAll is false
    var ignoredEdgeStyles = ['curved', 'sourcePerimeterSpacing', 'targetPerimeterSpacing', 'startArrow', 'startFill', 'startSize', 'endArrow', 'endFill', 'endSize'];
    ...
    var keyGroups = [['startArrow', 'startFill', 'endArrow', 'endFill'], ['startSize', 'endSize'], ['sourcePerimeterSpacing', 'targetPerimeterSpacing'], ['fillColor', 'gradientColor', 'gradientDirection'], ['opacity'], ['html']];
      ...
      try {
        for (var i = 0; i < cells.length; i++) {
          ...
          if (Editor.simpleLabels) {
            newStyle = mxUtils.setStyle(mxUtils.setStyle(newStyle, 'html', null), 'whiteSpace', null);
          }
    ...
    graph.panningHandler.isForcePanningEvent = function (me) {
      ...
      return panningHandlerIsForcePanningEvent.apply(this, arguments) || ui.isSpaceDown() || mxEvent.isMouseEvent(me.getEvent()) && (this.usePopupTrigger || !mxEvent.isPopupTrigger(me.getEvent())) && (!mxEvent.isControlDown(me.getEvent()) && mxEvent.isRightMouseButton(me.getEvent()) || mxEvent.isMiddleMouseButton(me.getEvent()));
    };
...
EditorUi.prototype.convertDarkModeColors = function (cells, keys) {
  ...
  keys = keys != null ? keys : [mxConstants.STYLE_FONTCOLOR, mxConstants.STYLE_FILLCOLOR, mxConstants.STYLE_GRADIENTCOLOR, mxConstants.STYLE_STROKECOLOR, mxConstants.STYLE_IMAGE_BORDER, mxConstants.STYLE_IMAGE_BACKGROUND, mxConstants.STYLE_LABEL_BORDERCOLOR, mxConstants.STYLE_SWIMLANE_FILLCOLOR, mxConstants.STYLE_LABEL_BACKGROUNDCOLOR];
  var colorCache = {};
...
EditorUi.prototype.showShapePicker = function (x, y, source, callback, direction, hovering, getInsertLocationFn, showEdges, startEditing) {
  ...
  }), this.getCellsForShapePicker(source, hovering, showEdges), hovering, getInsertLocationFn, showEdges, startEditing);
  if (div != null) {
...
EditorUi.prototype.createShapePicker = function (x, y, source, callback, direction, afterClick, cells, hovering, getInsertLocationFn, showEdges, startEditing) {
  ...
  if (cells != null && cells.length > 0) {
    ...
    var addCell = mxUtils.bind(this, function (cell) {
      ...
      node.style.cssText = 'position:relative;display:inline-block;position:relative;' + 'width:30px;height:30px;cursor:pointer;overflow:hidden;padding:1px';
      div.appendChild(node);
      if (this.sidebar && style != null && urlParams['sketch'] != '1') {
        this.sidebar.graph.pasteStyle(style, [cell]);
      } else {
        if (this.sidebar) ui.insertHandler([cell], cell.value != '' && urlParams['sketch'] != '1', this.sidebar.graph.model);
      }
      var geo = cell.geometry;
      if (graph.model.isEdge(cell)) {
        var pt = geo.getTerminalPoint(false);
      ...
      if (this.sidebar && geo != null) {
        node.appendChild(this.sidebar.createVertexTemplateFromCells([cell], geo.width, geo.height, '', true, false, null, false, mxUtils.bind(this, function (evt) {
...
EditorUi.prototype.getCellsForShapePicker = function (cell, hovering, showEdges) {
  ...
  if (cell == null) {
    cell = createVertex('text;html=1;align=center;verticalAlign=middle;resizable=0;' + 'points=[];autosize=1;strokeColor=none;fillColor=none;', 40, 20, 'Text');
...
EditorUi.prototype.updatePasteActionStates = function () {
  ...
  paste.setEnabled(this.editor.graph.cellEditor.isContentEditing() || (!mxClient.IS_FF && navigator.clipboard != null || !mxClipboard.isEmpty()) && graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent()));
...
EditorUi.prototype.initCanvas = function () {
  ...
  if (this.editor.isChromelessView()) {
    ...
    if (urlParams['toolbar'] != '0') {
      ...
      if (!toolbarConfig.noCloseBtn && (toolbarConfig.closeBtn && window.self === window.top || graph.lightbox && (urlParams['close'] == '1' || this.container != document.body))) {
        addButton(mxUtils.bind(this, function (evt) {
  ...
  graph.lazyZoom = function (zoomIn, ignoreCursorPosition, delay, factor) {
    ...
    if (graph.isFastZoomEnabled()) {
      ...
      var cx = ignoreCursorPosition || cursorPosition == null ? graph.container.scrollLeft + graph.container.clientWidth / 2 : cursorPosition.x + graph.container.scrollLeft - graph.container.offsetLeft;
      var cy = ignoreCursorPosition || cursorPosition == null ? graph.container.scrollTop + graph.container.clientHeight / 2 : cursorPosition.y + graph.container.scrollTop - graph.container.offsetTop;
      ...
      if (graph.view.backgroundPageShape != null && graph.view.backgroundPageShape.node != null) {
        ...
        mxUtils.setPrefixedStyle(page.style, 'transform-origin', (ignoreCursorPosition || cursorPosition == null ? graph.container.clientWidth / 2 + graph.container.scrollLeft - page.offsetLeft + 'px' : cursorPosition.x + graph.container.scrollLeft - page.offsetLeft - graph.container.offsetLeft + 'px') + ' ' + (ignoreCursorPosition || cursorPosition == null ? graph.container.clientHeight / 2 + graph.container.scrollTop - page.offsetTop + 'px' : cursorPosition.y + graph.container.scrollTop - page.offsetTop - graph.container.offsetTop + 'px'));
...
EditorUi.prototype.updateActionStates = function () {
  ...
  // Updates action states
  var actions = ['cut', 'copy', 'bold', 'italic', 'underline', 'delete', 'duplicate', 'editStyle', 'editTooltip', 'editLink', 'backgroundColor', 'borderColor', 'edit', 'toFront', 'toBack', 'solid', 'dashed', 'pasteSize', 'dotted', 'fillColor', 'gradientColor', 'shadow', 'fontColor', 'formattedText', 'rounded', 'toggleRounded', 'strokeColor', 'sharp', 'snapToGrid'];
  ...
  this.actions.get('ungroup').setEnabled(!ss.row && !ss.cell && !ss.table && ss.vertices.length > 0 && (graph.isContainer(ss.vertices[0]) || graph.getModel().getChildCount(ss.vertices[0]) > 0));
...
EditorUi.prototype.refresh = function (sizeDidChange) {
  ...
  this.diagramContainer.style.position = 'relative';
  this.diagramContainer.style.left = contLeft + diagContOffset.x + 'px';
  this.diagramContainer.style.top = tmp + diagContOffset.y + 'px';
  this.diagramContainer.style.right = w + 'px';
  this.diagramContainer.style.bottom = this.footerHeight + off + h + 'px';
  this.diagramContainer.style.width = w - 1 + 'px';
  this.diagramContainer.style.height = h - 1 + 'px';
  ...
  var effHsplitPosition = Math.max(0, Math.min(this.hsplitPosition, w - this.splitSize - 40));
  var tmp = 0;
...
EditorUi.prototype.createDivs = function () {
  ...
  if (this.container == document.body) {
    ...
    this.diagramContainer = this.container;
  }
...
EditorUi.prototype.createDiagramUi = function () {
  if (this.container == document.body) {
...
EditorUi.prototype.parseHtmlData = function (data) {
  ...
  if (data != null && data.length > 0) {
    ...
    if (elt.firstChild != null && elt.firstChild.nodeType == mxConstants.NODETYPE_ELEMENT && elt.firstChild.nextSibling != null && elt.firstChild.nextSibling.nodeType == mxConstants.NODETYPE_ELEMENT && elt.firstChild.nodeName == 'META' && elt.firstChild.nextSibling.nodeName == 'A' && elt.firstChild.nextSibling.nextSibling == null) {
      var temp = elt.firstChild.nextSibling.innerText == null ? mxUtils.getTextContent(elt.firstChild.nextSibling) : elt.firstChild.nextSibling.innerText;
...
EditorUi.prototype.createOutline = function (wnd) {
  var outline = new mxOutline(this.editor.graph, wnd);
...
EditorUi.prototype.createKeyHandler = function (editor) {
  ...
  keyHandler.isEventIgnored = function (evt) {
    ...
    return !(mxEvent.isShiftDown(evt) && evt.keyCode == 9) && (!this.isControlDown(evt) || mxEvent.isShiftDown(evt) || evt.keyCode != 90 && evt.keyCode != 89 && evt.keyCode != 188 && evt.keyCode != 190 && evt.keyCode != 85) && (evt.keyCode != 66 && evt.keyCode != 73 || !this.isControlDown(evt) || this.graph.cellEditor.isContentEditing() && !mxClient.IS_FF && !mxClient.IS_SF) && (evt.keyCode != 109 && evt.keyCode != 107 || !this.isControlDown(evt) && !mxEvent.isShiftDown(evt) || !this.graph.cellEditor.isContentEditing() && !mxClient.IS_FF && !mxClient.IS_SF) && isEventIgnored.apply(this, arguments);
  };
...
EditorUi.prototype.destroy = function () {
  ...
  var c = [this.menubarContainer, this.toolbarContainer, this.sidebarContainer, this.formatContainer, this.diagramContainer, this.footerContainer, this.chromelessToolbar, this.hsplit, this.layersDialog];
