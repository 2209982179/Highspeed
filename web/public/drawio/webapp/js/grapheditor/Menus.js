...
Menus.prototype.init = function () {
  ...
  this.put('fontFamily', new Menu(mxUtils.bind(this, function (menu, parent) {
    var addItem = mxUtils.bind(this, function (fontFamily) {
      var tr = this.styleChange(menu, fontFamily, [mxConstants.STYLE_FONTFAMILY], [fontFamily], null, parent, function () {
  ...
  this.put('fontSize', new Menu(mxUtils.bind(this, function (menu, parent) {
    ...
    var selState = null;
    this.promptChange(menu, mxResources.get('custom') + '...', '(' + mxResources.get('points') + ')', this.defaultFontSize, mxConstants.STYLE_FONTSIZE, parent, true, mxUtils.bind(this, function (newValue) {
  ...
  this.put('line', new Menu(mxUtils.bind(this, function (menu, parent) {
    ...
    if (state != null) {
      ...
      menu.addSeparator(parent);
      this.styleChange(menu, '', [mxConstants.STYLE_SHAPE, mxConstants.STYLE_STARTSIZE, mxConstants.STYLE_ENDSIZE, 'width'], [null, null, null, null], 'geIcon geSprite geSprite-connection', parent, null, null, true).setAttribute('title', mxResources.get('line'));
      this.styleChange(menu, '', [mxConstants.STYLE_SHAPE, mxConstants.STYLE_STARTSIZE, mxConstants.STYLE_ENDSIZE, 'width'], ['link', null, null, null], 'geIcon geSprite geSprite-linkedge', parent, null, null, true).setAttribute('title', mxResources.get('link'));
      this.styleChange(menu, '', [mxConstants.STYLE_SHAPE, mxConstants.STYLE_STARTSIZE, mxConstants.STYLE_ENDSIZE, 'width'], ['flexArrow', null, null, null], 'geIcon geSprite geSprite-arrow', parent, null, null, true).setAttribute('title', mxResources.get('arrow'));
      this.styleChange(menu, '', [mxConstants.STYLE_SHAPE, mxConstants.STYLE_STARTSIZE, mxConstants.STYLE_ENDSIZE, 'width'], ['arrow', null, null, null], 'geIcon geSprite geSprite-simplearrow', parent, null, null, true).setAttribute('title', mxResources.get('simpleArrow'));
    }
  })));
  ...
  this.put('view', new Menu(mxUtils.bind(this, function (menu, parent) {
    this.addMenuItems(menu, (this.editorUi.format != null ? ['format'] : []).concat(['outline', 'layers', '-', 'pageView', 'pageScale', '-', 'tooltips', 'grid', 'guides', '-', 'connectionArrows', 'connectionPoints', '-', 'resetView', 'zoomIn', 'zoomOut'], parent));
  })));
...
Menus.prototype.addInsertTableCellItem = function (menu, parent) {
  ...
  if (parent != null || !isTable && !isStack) {
    this.addInsertTableItem(menu, mxUtils.bind(this, function (evt, rows, cols, title, container) {
      var table = container || mxEvent.isControlDown(evt) || mxEvent.isMetaDown(evt) ? graph.createCrossFunctionalSwimlane(rows, cols, null, null, title || mxEvent.isShiftDown(evt) ? 'Cross-Functional Flowchart' : null) : graph.createTable(rows, cols, null, null, title || mxEvent.isShiftDown(evt) ? 'Table' : null);
...
Menus.prototype.promptChange = function (menu, label, hint, defaultValue, key, parent, enabled, fn, sprite, beforeFn) {
  return menu.addItem(label, null, mxUtils.bind(this, function () {
    ...
    }), mxResources.get('enterValue') + (hint.length > 0 ? ' ' + hint : ''), null, null, null, null, function () {
...
Menus.prototype.addPopupMenuArrangeItems = function (menu, cell, evt) {
  ...
  // Shows group actions
  if (graph.getSelectionCount() > 1 || graph.getSelectionCount() > 0 && !graph.getModel().isEdge(cell) && !graph.isSwimlane(cell) && graph.getModel().getChildCount(cell) > 0 && graph.isCellEditable(cell)) {
  ...
  if (state != null) {
    ...
    if (graph.getModel().isEdge(cell) && mxUtils.getValue(state.style, mxConstants.STYLE_EDGE, null) != 'entityRelationEdgeStyle' && mxUtils.getValue(state.style, mxConstants.STYLE_SHAPE, null) != 'arrow') {
      ...
      if (hasWaypoints && handler instanceof mxEdgeHandler && handler.bends != null && handler.bends.length > 2) {
        ...
        if (index > 0 && index < handler.bends.length - 1 && (handler.bends[index] == null || handler.bends[index].node == null || handler.bends[index].node.style.opacity == '')) {
    ...
    if (graph.getSelectionCount() == 1 && this.isShowCellEditItems() && (hasWaypoints || graph.getModel().isVertex(cell) && graph.getModel().getEdgeCount(cell) > 0)) {
