...
var rough = function () {
  ...
  function Y(b, a, e, c, f, k, d, g, n, m) {
    ...
    w = [];
    var t, h, q;
    if (m) q = _slicedToArray(m, 4), t = q[0], g = q[1], h = q[2], q = q[3];else {
      t = I(b, a, -r);
    ...
    w = [b, a, e].concat(w);
    m = [];
    for (e = 0; e < w.length; e += 3) c = I(w[e][0], w[e][1], r), b = I(w[e + 1][0], w[e + 1][1], r), a = I(w[e + 2][0], w[e + 2][1], r), m.push([c[0], c[1], b[0], b[1], a[0], a[1]]);
    return m;
  }
  function N(b, a, e) {
    var c = (b || []).length;
  ...
  function ba(b, a, e, c, f, k, d, g) {
    ...
    return m.push([u(k, g) + a + c * Math.cos(r + 2 * Math.PI + .5 * d), u(k, g) + e + f * Math.sin(r + 2 * Math.PI + .5 * d)]), m.push([u(k, g) + a + .98 * c * Math.cos(r + d), u(k, g) + e + .98 * f * Math.sin(r + d)]), m.push([u(k, g) + a + .9 * c * Math.cos(r + .5 * d), u(k, g) + e + .9 * f * Math.sin(r + .5 * d)]), [m, n];
  }
  ...
  function ta(b, a, e, c, f, d, r, g) {
    for (var k = [], m = [g.maxRandomnessOffset || 1, (g.maxRandomnessOffset || 1) + .3], h, w = g.disableMultiStroke ? 1 : 2, t = g.preserveVertices, l = 0; l < w; l++) 0 === l ? k.push({
      op: "move",
      data: [r[0], r[1]]
    }) : k.push({
      op: "move",
      ...
      _createClass(b, [{
        ...
        value: function (a, e) {
          ...
          if (1 < b.length) {
            ...
            b = [];
            for (d = 0; d < c.length - 1; d += 2) r = [c[d], c[d + 1]], this.midPointInPolygon(a, r) && b.push(r);
            return b;
          }
