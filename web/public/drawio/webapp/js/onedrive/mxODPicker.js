...
function mxODPicker(container, previewFn, getODFilesList, getODFileInfo, getRecentList, addToRecent, pickedFileCallback, errorFn, foldersOnly, backFn, withSubmitBtn, withThumbnail, initFolderPath, acceptAllFiles) {
  var previewHtml = '';
  if (previewFn == null) {
      ...
      if (this.readyState == 4) {
        if (this.status >= 200 && this.status <= 299) {
          try {
            ...
            var node = doc.documentElement.nodeName == 'mxlibrary' ? doc.documentElement : Editor.extractGraphModel(doc.documentElement);
            if (node != null) {
    ...
    try {
      ...
      getDrawioFileDoc(file, function (doc) {
        ...
        if (loadingPreviewFile != file) {
          ...
          curViewer = AspectDialog.prototype.createViewer(prevDiv, diagrams.length == 0 ? doc.documentElement : diagrams[0], null, 'transparent');
        }
      }, function ()
      //If the file is not a draw.io diagram
      {
        selectedFile = null;
  ...
  function renderBreadcrumb() {
    ...
    if (breadcrumb[breadcrumb.length - 1] != null) {
      ...
      curr.innerHTML = mxUtils.htmlEntities(breadcrumb.length == 1 ? mxResources.get(foldersOnly ? 'selectFolder' : 'officeSelDiag') : breadcrumb[breadcrumb.length - 1].name || mxResources.get('home'));
  ...
  function fillFolderFiles(driveId, folderId, siteId, folderName, searchTxt) {
    ...
    function renderList(potentialDrawioFiles) {
      ...
      for (var i = 0; potentialDrawioFiles != null && i < potentialDrawioFiles.length; i++) {
        ...
        if (isSharepointSites) {
          item.folder = isSharepointSites == 2 ? {
            isRoot: true
          } : true;
        }
    ...
    switch (driveId) {
      ...
      case 'subsite':
        ...
        break;
      case 'search':
        //TODO search doesn't return any results, find out why then remove display: none from the searchBox
      ...
      getODFilesList(nextUrl ? nextUrl : url, function (resp) {
        ...
        if (acceptAllFiles || isSharepointSites) {
          ...
          for (var i = 0; i < list.length; i++) {
            ...
            if (file.folder || mimeType == 'text/html' || mimeType == 'text/xml' || mimeType == 'application/xml' || mimeType == 'image/png' || /\.svg$/.test(file.name) || /\.html$/.test(file.name) || /\.xml$/.test(file.name) || /\.png$/.test(file.name) || /\.drawio$/.test(file.name) || /\.drawiolib$/.test(file.name)) {
        ...
        errorFn(mxResources.get('errorFetchingFolder', null, 'Error fetching folder items') + (errMsg != null ? ' (' + errMsg + ')' : ''));
        requestInProgress = false;
        spinner.stop();
      }, nextUrl != null);
    }
