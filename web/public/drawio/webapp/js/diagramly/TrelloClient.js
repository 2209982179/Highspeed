...
TrelloClient.prototype.key = window.location.hostname == 'test.draw.io' ? 'e89d109082298ce91f6576f82f458551' : 'e89d109082298ce91f6576f82f458551';
  ...
  var callback = mxUtils.bind(this, function () {
    ...
    Trello.cards.get(ids[0] + '/attachments/' + ids[1], mxUtils.bind(this, function (meta) {
      ...
      if (acceptResponse) {
        ...
        if (/\.v(dx|sdx?)$/i.test(meta.name) || /\.gliffy$/i.test(meta.name) || !this.ui.useCanvasForExport && binary) {
          this.ui.convertFile(PROXY_URL + '?url=' + encodeURIComponent(meta.url), meta.name, meta.mimeType, this.extension, success, error, null, headers);
        } else {
          acceptResponse = true;
          ...
          }), binary || meta.mimeType != null && meta.mimeType.substring(0, 6) == 'image/', null, null, null, headers);
        }
      }
...
TrelloClient.prototype.showTrelloDialog = function (showFiles, fn) {
  ...
  var createLink = mxUtils.bind(this, function (label, fn, preview) {
    linkCounter++;
    var div = document.createElement('div');
    div.style = 'width:100%;text-overflow:ellipsis;overflow:hidden;vertical-align:middle;' + 'padding:2px 0 2px 0;background:' + (linkCounter % 2 == 0 ? Editor.isDarkMode() ? '#000' : '#eee' : Editor.isDarkMode() ? '' : '#fff');
  ...
  var selectAtt = mxUtils.bind(this, function () {
    ...
    var callback = mxUtils.bind(this, function () {
      Trello.cards.get(cardId + '/attachments', {
