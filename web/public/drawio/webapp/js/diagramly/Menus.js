...
(function () {
  ...
  Menus.prototype.init = function () {
    ...
    var googleEnabled = (urlParams['embed'] != '1' && urlParams['gapi'] != '0' || urlParams['embed'] == '1' && urlParams['gapi'] == '1') && mxClient.IS_SVG && isLocalStorage && (document.documentMode == null || document.documentMode >= 10);
    var dropboxEnabled = (urlParams['embed'] != '1' && urlParams['db'] != '0' || urlParams['embed'] == '1' && urlParams['db'] == '1') && mxClient.IS_SVG && (document.documentMode == null || document.documentMode > 9);
    var oneDriveEnabled = (window.location.hostname == 'www.draw.io' || window.location.hostname == 'test.draw.io' || window.location.hostname == 'drive.draw.io' || window.location.hostname == 'app.diagrams.net') && (urlParams['embed'] != '1' && urlParams['od'] != '0' || urlParams['embed'] == '1' && urlParams['od'] == '1') && !mxClient.IS_IOS && (navigator.userAgent.indexOf('MSIE') < 0 || document.documentMode >= 10);
    var trelloEnabled = urlParams['tr'] == '1' && mxClient.IS_SVG && (document.documentMode == null || document.documentMode > 9);
    ...
    editorUi.actions.put('insertTemplate', new Action(mxResources.get('template') + '...', function () {
      if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
        ...
        }, null, null, null, null, null, null, null, null, null, null, false, mxResources.get('insert'));
    ...
    var toggleSimpleModeAction = editorUi.actions.put('toggleSimpleMode', new Action(mxResources.get('simple'), function (e) {
      editorUi.setCurrentTheme(Editor.currentTheme == 'simple' ? !Editor.isDarkMode() ? 'kennedy' : 'dark' : 'simple');
    }));
    toggleSimpleModeAction.setToggleAction(true);
    toggleSimpleModeAction.visible = Editor.currentTheme != 'min' && Editor.currentTheme != 'sketch' && Editor.currentTheme != 'atlas';
      ...
      editorUi.actions.put('insertFreehand', new Action(mxResources.get('freehand') + '...', function () {
        if (graph.isEnabled()) {
          if (editorUi.freehandWindow == null) {
            ...
            editorUi.freehandWindow = new FreehandWindow(editorUi, document.body.offsetWidth - 420, 102, 176, withBrush ? 120 : 84, withBrush);
          }
    ...
    if (Editor.enableExportUrl) {
      editorUi.actions.put('exportUrl', new Action(mxResources.get('url') + '...', function () {
        editorUi.showPublishLinkDialog(mxResources.get('url'), true, null, null, function (linkTarget, linkColor, allPages, lightbox, editLink, layers, width, height, tags) {
          var params = [];
          if (tags) {
            params.push('tags=%7B%7D');
          }
          var dlg = new EmbedDialog(editorUi, editorUi.createLink(linkTarget, linkColor, allPages, lightbox, editLink, layers, null, true, params));
    ...
    editorUi.actions.put('exportHtml', new Action(mxResources.get('formatHtmlEmbedded') + '...', function () {
      if (editorUi.spinner.spin(document.body, mxResources.get('loading'))) {
        editorUi.getPublicUrl(editorUi.getCurrentFile(), function (url) {
          ...
          editorUi.showHtmlDialog(mxResources.get('export'), null, url, function (publicUrl, zoomEnabled, initialZoom, linkTarget, linkColor, fit, allPages, layers, tags, lightbox, editLink) {
    ...
    editorUi.actions.put('exportPdf', new Action(mxResources.get('formatPdf') + '...', function () {
      if (!EditorUi.isElectronApp && (editorUi.isOffline() || editorUi.printPdfExport)) {
        ...
        editorUi.showDialog(new PrintDialog(editorUi, mxResources.get('formatPdf')).container, 360, editorUi.pages != null && editorUi.pages.length > 1 && (editorUi.editor.editable || urlParams['hide-pages'] != '1') ? 470 : 390, true, true);
      } else {
        var noPages = editorUi.pages == null || editorUi.pages.length <= 1;
        var div = document.createElement('div');
        div.style.whiteSpace = 'nowrap';
        ...
        var isDrawioWeb = !mxClient.IS_CHROMEAPP && !EditorUi.isElectronApp && editorUi.getServiceName() == 'draw.io';
        var transparentBkg = null,
          include = null;
        if (EditorUi.isElectronApp || isDrawioWeb) {
          include = editorUi.addCheckbox(div, mxResources.get('includeCopyOfMyDiagram'), Editor.defaultIncludeDiagram);
          dlgH += 30;
        }
        if (isDrawioWeb) {
          transparentBkg = editorUi.addCheckbox(div, mxResources.get('transparentBackground'), false);
          dlgH += 30;
        }
        var dlg = new CustomDialog(editorUi, div, mxUtils.bind(this, function () {
          var pageRange = null;
          if (!noPages) {
            ...
            pageRange = !allPages.checked && (from != currentPage || to != currentPage) ? {
              from: Math.max(0, Math.min(pageCount - 1, from - 1)),
              to: Math.max(0, Math.min(pageCount - 1, to - 1))
            } : null;
          }
          editorUi.downloadFile('pdf', null, null, !selection.checked, noPages ? true : !allPages.checked && pageRange == null, !crop.checked, transparentBkg != null && transparentBkg.checked, null, null, grid.checked, include != null && include.checked, pageRange);
        }), null, mxResources.get('export'));
    ...
    editorUi.actions.addAction('upload...', function () {
      ...
      if (file != null) {
        ...
        editorUi.openLink(window.location.protocol + '//' + window.location.host + '/?create=drawdata&' + (editorUi.mode == App.MODE_DROPBOX ? 'mode=dropbox&' : '') + 'title=' + encodeURIComponent(filename), null, true);
      }
    });
    ...
    editorUi.actions.put('exportSvg', new Action(mxResources.get('formatSvg') + '...', function () {
      editorUi.showExportDialog(mxResources.get('formatSvg'), true, mxResources.get('export'), 'https://www.drawio.com/doc/faq/export-diagram', mxUtils.bind(this, function (scale, transparentBackground, ignoreSelection, addShadow, editable, embedImages, border, cropImage, currentPage, linkTarget, grid, theme, exportType, embedFonts, lblToSvg) {
        var val = parseInt(scale);
        editorUi.lastExportSvgEditable = editable;
        if (!isNaN(val) && val > 0) {
          if (lblToSvg) {
            editorUi.downloadFile('remoteSvg', null, null, ignoreSelection, null, cropImage, transparentBackground, scale, border, null, editable);
          } else {
            editorUi.exportSvg(val / 100, transparentBackground, ignoreSelection, addShadow, editable, embedImages, border, !cropImage, false, linkTarget, theme, exportType, embedFonts);
          }
        }
    ...
    function exportImage(format, defaultEditable, done) {
      if (editorUi.isExportToCanvas()) {
        editorUi.showExportDialog(mxResources.get('image'), false, mxResources.get('export'), 'https://www.drawio.com/doc/faq/export-diagram', mxUtils.bind(this, function (scale, transparentBackground, ignoreSelection, addShadow, editable, embedImages, border, cropImage, currentPage, dummy, grid, theme, exportType) {
          var val = parseInt(scale);
          if (!isNaN(val) && val > 0) {
            editorUi.exportImage(val / 100, transparentBackground && format == 'png', ignoreSelection, addShadow, editable && format == 'png', border, !cropImage, false, format, grid, null, theme, exportType);
            if (done != null) {
      ...
      editorUi.actions.addAction('configuration...', function () {
        ...
        if (editorUi.getServiceName() != 'atlassian' && urlParams['embed'] != '1') {
          buttons.push([mxResources.get('preferences'), function () {
            editorUi.showLocalStorageDialog(mxResources.get('preferences') + ':', Editor.settingsKey, [[mxResources.get('reset'), function () {
              editorUi.confirm(mxResources.get('areYouSure'), function () {
                try {
        ...
        editorUi.showLocalStorageDialog(mxResources.get('configuration') + ':', Editor.configurationKey, buttons, splashCb.parentNode, 'https://www.drawio.com/doc/faq/configure-diagram-editor', function () {
    ...
    if (mxClient.IS_CHROMEAPP || isLocalStorage) {
      ...
      Menus.prototype.createMenubar = function (container) {
        ...
        if ((urlParams['embed'] != '1' || urlParams['atlas'] == '1') && menubar != null && Editor.enableSimpleTheme && editorUi.getServiceName() != 'atlassian') {
          ...
          if (themeMenu != null) {
            ...
            var updateThemeElement = mxUtils.bind(this, function () {
              ...
              elt.style.display = Editor.currentTheme == 'atlas' || Editor.currentTheme == 'min' || Editor.currentTheme == 'sketch' ? 'none' : '';
              if (Editor.currentTheme == 'simple') {
                if (iw < 750) {
                  elt.style.backgroundImage = 'url(' + Editor.thinDoubleArrowRightImage + ')';
                } else {
                  elt.style.backgroundImage = 'url(' + (Editor.isDarkMode() || Editor.cssDarkMode ? Editor.thinDarkImage : Editor.thinLightImage) + ')';
                }
              } else {
                elt.style.backgroundImage = 'url(' + (Editor.isDarkMode() ? Editor.darkModeImage : Editor.lightModeImage) + ')';
              }
            });
    ...
    editorUi.actions.put('convertDarkModeColors', new Action('Dark Mode Colors...', function (evt) {
      if (Editor.enableCssDarkMode) {
        if (editorUi.darkModeColorsWindow == null) {
          editorUi.darkModeColorsWindow = new DarkModeColorsWindow(editorUi, document.body.offsetWidth - 520, 80, 200, 100);
        }
    ...
    layoutMenu.funct = function (menu, parent) {
      ...
      menu.addItem(mxResources.get('orgChart'), null, function () {
        ...
        var typesArr = [mxResources.get('linear'), mxResources.get('hanger2'), mxResources.get('hanger4'), mxResources.get('fishbone1'), mxResources.get('fishbone2'), mxResources.get('1ColumnLeft'), mxResources.get('1ColumnRight'), mxResources.get('smart')];
      ...
      if (!mxClient.IS_CHROMEAPP && editorUi.isOffline()) {
        ...
        mxEvent.addListener(input, 'keydown', mxUtils.bind(this, function (e) {
          ...
          if (e.keyCode == 13 && term.length > 0) {
            this.editorUi.openLink('https://www.drawio.com/search?src=' + (EditorUi.isElectronApp ? 'DESKTOP' : encodeURIComponent(location.host)) + '&search=' + encodeURIComponent(term));
            input.value = '';
            EditorUi.logEvent({
    ...
    if (urlParams['test'] == '1') {
      ...
      editorUi.actions.addAction('createSidebarEntry', mxUtils.bind(this, function () {
        if (!graph.isSelectionEmpty()) {
          ...
          editorUi.showTextDialog('Create Sidebar Entry', 'this.addDataEntry(\'tag1 tag2\', ' + bbox.width + ', ' + bbox.height + ', \'The Title\', \'' + Graph.compress(mxUtils.getXml(graph.encodeCells(cells))) + '\'),');
        }
      }));
      ...
      editorUi.actions.addAction('testDiff', mxUtils.bind(this, function () {
        if (editorUi.pages != null) {
          ...
          var dlg = new TextareaDialog(editorUi, 'Diff/Sync:', '', function (newValue) {
            var file = editorUi.getCurrentFile();
      ...
      this.put('testDevelop', new Menu(mxUtils.bind(this, function (menu, parent) {
        this.addMenuItems(menu, ['createSidebarEntry', 'showBoundingBox', '-', 'testCheckPages', 'testChecksum', 'testFixPages', '-', 'testCheckFile', 'testDiff', '-', 'testInspect', 'testOptimize', '-', 'testXmlImageExport', '-'], parent);
    ...
    editorUi.actions.put('embedHtml', new Action(mxResources.get('html') + '...', function () {
      if (editorUi.spinner.spin(document.body, mxResources.get('loading'))) {
        editorUi.getPublicUrl(editorUi.getCurrentFile(), function (url) {
          editorUi.spinner.stop();
          editorUi.showHtmlDialog(mxResources.get('create'), 'https://www.drawio.com/doc/faq/embed-html-options', url, function (publicUrl, zoomEnabled, initialZoom, linkTarget, linkColor, fit, allPages, layers, tags, lightbox, editLink) {
            editorUi.createHtml(publicUrl, zoomEnabled, initialZoom, linkTarget, linkColor, fit, allPages, layers, tags, lightbox, editLink, mxUtils.bind(this, function (html, scriptTag) {
    ...
    editorUi.actions.put('embedSvg', new Action(mxResources.get('formatSvg') + '...', function () {
      ...
      }, mxResources.get('formatSvg'), mxResources.get('image'), true, 'https://www.drawio.com/doc/faq/embed-svg.html');
    }));
    editorUi.actions.put('embedIframe', new Action(mxResources.get('iframe') + '...', function () {
      var bounds = graph.getGraphBounds();
      editorUi.showPublishLinkDialog(mxResources.get('iframe'), null, '100%', Math.ceil(bounds.height / graph.view.scale) + 2, function (linkTarget, linkColor, allPages, lightbox, editLink, layers, width, height, tags) {
        if (editorUi.spinner.spin(document.body, mxResources.get('loading'))) {
          editorUi.getPublicUrl(editorUi.getCurrentFile(), function (url) {
            editorUi.spinner.stop();
            var params = [];
            ...
            var dlg = new EmbedDialog(editorUi, '<iframe frameborder="0" style="width:' + width + ';height:' + height + ';" src="' + editorUi.createLink(linkTarget, linkColor, allPages, lightbox, editLink, layers, url, null, params) + '"></iframe>');
            editorUi.showDialog(dlg.container, 450, 240, true, true);
            dlg.init();
          });
        }
    ...
    editorUi.actions.put('embedNotion', new Action(mxResources.get('notion') + '...', function () {
      ...
      editorUi.showPublishLinkDialog(mxResources.get('notion'), null, null, null, function (linkTarget, linkColor, allPages, lightbox, editLink, layers, width, height, tags) {
        if (editorUi.spinner.spin(document.body, mxResources.get('loading'))) {
          editorUi.getPublicUrl(editorUi.getCurrentFile(), function (url) {
            editorUi.spinner.stop();
            var params = ['border=0'];
            ...
            var dlg = new EmbedDialog(editorUi, editorUi.createLink(linkTarget, linkColor, allPages, lightbox, editLink, layers, url, null, params));
            editorUi.showDialog(dlg.container, 450, 240, true, true);
            dlg.init();
          });
        }
    ...
    editorUi.actions.put('publishLink', new Action(mxResources.get('link') + '...', function () {
      editorUi.showPublishLinkDialog(null, null, null, null, function (linkTarget, linkColor, allPages, lightbox, editLink, layers, width, height, tags) {
        if (editorUi.spinner.spin(document.body, mxResources.get('loading'))) {
          editorUi.getPublicUrl(editorUi.getCurrentFile(), function (url) {
            editorUi.spinner.stop();
            var params = [];
    ...
    this.put('exportAs', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      if (editorUi.mode != App.MODE_ATLAS && urlParams['extAuth'] != '1' && (Editor.currentTheme == 'simple' || Editor.currentTheme == 'sketch' || Editor.currentTheme == 'min')) {
        menu.addSeparator(parent);
    ...
    this.put('dynamicAppearance', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      if (editorUi.isAutoDarkModeSupported()) {
        ...
        if (item != null) {
          item.setAttribute('title', mxResources.get('automatic') + ' (' + mxResources.get(Editor.cssDarkMode || Editor.isDarkMode() ? 'dark' : 'light') + ')');
        }
      }
      if (urlParams['embed'] != '1') {
    ...
    this.put('theme', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      if (theme != 'kennedy' && theme != 'atlas' && theme != 'dark' && theme != 'simple' && theme != 'sketch' && theme != 'min') {
        menu.addCheckmark(item, Editor.checkmarkImage);
      }
    ...
    var renameAction = this.editorUi.actions.addAction('rename...', mxUtils.bind(this, function () {
      ...
      if (file != null) {
        if (file.constructor == LocalFile && file.fileHandle != null) {
          ...
          }), file.constructor == DriveFile || file.constructor == StorageFile ? mxResources.get('diagramName') : null, function (name) {
    ...
    editorUi.actions.put('insertText', new Action(mxResources.get('text'), function (evt) {
      if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
        graph.startEditingAtCell(insertVertex('Text', 60, 30, 'text;strokeColor=none;align=center;' + 'fillColor=none;html=1;verticalAlign=middle;whiteSpace=wrap;rounded=0;', evt != null && !mxEvent.isControlDown(evt) && !mxEvent.isMetaDown(evt) && graph.isMouseInsertPoint() ? graph.getInsertPoint() : null));
      }
    ...
    editorUi.actions.put('insertRectangle', new Action(mxResources.get('rectangle'), function (evt) {
      if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
        insertVertex('', 120, 60, 'whiteSpace=wrap;html=1;', evt != null && !mxEvent.isControlDown(evt) && !mxEvent.isMetaDown(evt) && graph.isMouseInsertPoint() ? graph.getInsertPoint() : null);
      }
    }, null, null, 'D')).isEnabled = isGraphEnabled;
    editorUi.actions.put('insertNote', new Action(mxResources.get('note'), function (evt) {
      if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
        insertVertex('', 140, 160, 'shape=note;whiteSpace=wrap;html=1;backgroundOutline=1;' + 'fontColor=#000000;darkOpacity=0.05;fillColor=#FFF9B2;strokeColor=none;fillStyle=solid;' + 'direction=west;gradientDirection=north;gradientColor=#FFF2A1;shadow=1;size=20;pointerEvents=1;', evt != null && !mxEvent.isControlDown(evt) && !mxEvent.isMetaDown(evt) && graph.isMouseInsertPoint() ? graph.getInsertPoint() : null);
      }
    }, null, null, 'S')).isEnabled = isGraphEnabled;
    editorUi.actions.put('insertEllipse', new Action(mxResources.get('ellipse'), function (evt) {
      if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
        insertVertex('', 80, 80, 'ellipse;whiteSpace=wrap;html=1;', evt != null && !mxEvent.isControlDown(evt) && !mxEvent.isMetaDown(evt) && graph.isMouseInsertPoint() ? graph.getInsertPoint() : null);
      }
    }, null, null, 'F')).isEnabled = isGraphEnabled;
    editorUi.actions.put('insertRhombus', new Action(mxResources.get('rhombus'), function (evt) {
      if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
        insertVertex('', 80, 80, 'rhombus;whiteSpace=wrap;html=1;', evt != null && !mxEvent.isControlDown(evt) && !mxEvent.isMetaDown(evt) && graph.isMouseInsertPoint() ? graph.getInsertPoint() : null);
      }
    }, null, null, 'R')).isEnabled = isGraphEnabled;
    editorUi.actions.put('insertEdge', new Action(mxResources.get('line'), function (evt) {
      if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
        insertEdge('', graph.defaultEdgeLength, 'edgeStyle=none;orthogonalLoop=1;jettySize=auto;html=1;', evt != null && !mxEvent.isControlDown(evt) && !mxEvent.isMetaDown(evt) && graph.isMouseInsertPoint() ? graph.getInsertPoint() : null);
      }
    ...
    this.put('insert', new Menu(mxUtils.bind(this, function (menu, parent) {
      if (Editor.currentTheme == 'sketch') {
        ...
        this.addMenuItems(menu, ['insertRectangle', 'insertEllipse', 'insertRhombus', '-', 'insertEdge', 'insertNote', '-', 'insertText', 'insertLink', '-', 'createShape', 'insertFreehand', '-', 'insertImage'], parent);
        if (editorUi.insertTemplateEnabled && !editorUi.isOffline()) {
    ...
    this.put('insertLayout', new Menu(mxUtils.bind(this, function (menu, parent) {
      editorUi.addInsertMenuItems(menu, parent, ['horizontalFlow', 'verticalFlow', '-', 'horizontalTree', 'verticalTree', 'radialTree', '-', 'organic', 'circle']);
    })));
          ...
          var dlg = new FilenameDialog(editorUi, '', mxResources.get('open'), function (fileUrl) {
            if (fileUrl != null && fileUrl.length > 0) {
              if (editorUi.getCurrentFile() == null) {
                ...
                window.openWindow((mxClient.IS_CHROMEAPP ? 'https://www.draw.io/' : 'https://' + location.host + '/') + window.location.search + '#U' + encodeURIComponent(fileUrl));
              }
            }
          }, mxResources.get('url'));
          editorUi.showDialog(dlg.container, 300, 80, true, true);
    ...
    this.put('edit', new Menu(mxUtils.bind(this, function (menu, parent) {
      this.addMenuItems(menu, ['undo', 'redo', '-', 'cut', 'copy', 'copyAsImage', 'paste', 'delete', '-', 'duplicate', '-', 'findReplace', '-', 'editData', 'editTooltip', '-', 'editStyle', 'editGeometry', '-', 'edit', '-', 'editLink', 'openLink', '-', 'selectVertices', 'selectEdges', 'selectAll', 'selectNone', '-', 'lockUnlock']);
    })));
    ...
    this.put('view', new Menu(mxUtils.bind(this, function (menu, parent) {
      if (Editor.currentTheme == 'simple') {
        ...
        editorUi.menus.addMenuItems(menu, ['-', 'findReplace', 'layers', 'tags', 'chatWindowTitle', 'outline', '-'], parent);
        if (editorUi.commentsSupported()) {
          editorUi.menus.addMenuItems(menu, ['comments'], parent);
        }
        if (file != null && file.isRealtimeEnabled() && file.isRealtimeSupported()) {
        ...
        this.addMenuItems(menu, ['format', 'outline', 'layers', 'tags', 'chatWindowTitle'].concat(editorUi.commentsSupported() ? ['comments', '-'] : ['-']));
        ...
        this.addMenuItems(menu, ['tooltips', 'ruler', '-', 'grid', 'guides', '-', 'connectionArrows', 'connectionPoints', '-', 'resetView', 'zoomIn', 'zoomOut'], parent);
    ...
    this.put('editCell', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      this.addMenuItems(menu, ['editLink', 'editShape', 'editImage', 'crop', '-', 'editData', 'copyData', 'pasteData', '-', 'editConnectionPoints', 'editGeometry', '-', 'editTooltip', 'editStyle', '-', 'edit'], parent);
    })));
        ...
        for (var i = 0; i < editorUi.pages.length; i++) {
          mxUtils.bind(this, function (index) {
            ...
            if (editorUi.pages[index] == page && !editorUi.editor.graph.isLightboxView()) {
              ...
              item = menu.addItem(editorUi.getShortPageName(editorUi.pages[index]), null, mxUtils.bind(this, function () {
    ...
    this.put('extras', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      if (sketchTheme || uiTheme == 'min') {
        if ((urlParams['embed'] != '1' || urlParams['atlas'] == '1') && urlParams['extAuth'] != '1' && editorUi.mode != App.MODE_ATLAS && urlParams['embedInline'] != '1' && (Editor.isDarkMode() || !mxClient.IS_IE && !mxClient.IS_IE11)) {
        ...
        editorUi.menus.addMenuItems(menu, ['-', 'copyConnect', 'collapseExpand', 'tooltips', '-'], parent);
        var file = editorUi.getCurrentFile();
        if (Editor.currentTheme != 'simple') {
          if (file != null && file.isRealtimeEnabled() && file.isRealtimeSupported()) {
            this.addMenuItems(menu, ['showRemoteCursors'], parent);
        ...
        if ((urlParams['embed'] != '1' || urlParams['atlas'] == '1') && Editor.currentTheme != 'atlas' && (Editor.isDarkMode() || !mxClient.IS_IE && !mxClient.IS_IE11)) {
          editorUi.menus.addSubmenu('appearance', menu, parent);
        }
        if (urlParams['embed'] != '1' && urlParams['extAuth'] != '1' && editorUi.mode != App.MODE_ATLAS) {
    ...
    this.put('diagram', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      if (mxClient.IS_CHROMEAPP || EditorUi.isElectronApp) {
        editorUi.menus.addMenuItems(menu, ['new', 'open', '-', 'synchronize', 'properties', '-', 'save', 'saveAs', '-'], parent);
        ...
        if (Editor.currentTheme == 'min') {
          editorUi.menus.addMenuItems(menu, ['toggleShapes', 'format', 'layers', 'tags', 'chatWindowTitle', '-', 'findReplace'], parent);
          if (editorUi.commentsSupported()) {
            editorUi.menus.addMenuItems(menu, ['comments'], parent);
          }
          menu.addSeparator(parent);
      ...
      if (Editor.currentTheme != 'simple' && Editor.currentTheme != 'min') {
        ...
        editorUi.menus.addMenuItems(menu, ['toggleShapes', 'format', 'layers', 'tags', 'chatWindowTitle', '-'], parent);
    ...
    this.put('file', new Menu(mxUtils.bind(this, function (menu, parent) {
      ...
      if (urlParams['embed'] == '1') {
        ...
        menu.addSeparator(parent);
        if (file != null) {
          if (!mxClient.IS_CHROMEAPP && !EditorUi.isElectronApp && Editor.currentTheme == 'sketch') {
            this.addMenuItem(menu, 'publishLink', parent, null, null, mxResources.get('publish') + '...');
          }
    ...
    this.put('fontFamily', new Menu(mxUtils.bind(this, function (menu, parent) {
      var addItem = mxUtils.bind(this, function (fontName, fontUrl, deletable, fontLabel, tooltip) {
        var graph = editorUi.editor.graph;
        var tr = this.styleChange(menu, fontLabel || fontName, urlParams['ext-fonts'] != '1' ? [mxConstants.STYLE_FONTFAMILY, 'fontSource', 'FType'] : [mxConstants.STYLE_FONTFAMILY], urlParams['ext-fonts'] != '1' ? [fontName, fontUrl != null ? encodeURIComponent(fontUrl) : null, null] : [fontName], null, parent, function () {
          ...
          editorUi.fireEvent(new mxEventObject('styleChanged', 'keys', urlParams['ext-fonts'] != '1' ? [mxConstants.STYLE_FONTFAMILY, 'fontSource', 'FType'] : [mxConstants.STYLE_FONTFAMILY], 'values', urlParams['ext-fonts'] != '1' ? [fontName, fontUrl != null ? encodeURIComponent(fontUrl) : null, null] : [fontName], 'cells', [graph.cellEditor.getEditingCell()]));
        }, function () {
          ...
          mxEvent.addListener(img, mxClient.IS_POINTER ? 'pointerup' : 'mouseup', mxUtils.bind(this, function (evt) {
            if (urlParams['ext-fonts'] != '1') {
              ...
              for (var i = 0; i < this.customFonts.length; i++) {
                if (this.customFonts[i].name == fontName && this.customFonts[i].url == fontUrl) {
      ...
      if (urlParams['ext-fonts'] != '1') {
        ...
        if (entries.length > 0) {
          for (var i = 0; i < entries.length; i++) {
            addItem(entries[i].name, entries[i].url, true, entries[i].label, entries[i].url);
          }
          menu.addSeparator(parent);
        }
