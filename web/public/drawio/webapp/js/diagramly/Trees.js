...
(function () {
  ...
  Graph.prototype.foldCells = function (collapse, recurse, cells, checkFoldable, evt) {
    ...
    try {
      ...
      for (var i = 0; i < cells.length; i++) {
        if (mxUtils.getValue(this.getCurrentCellStyle(cells[i]), 'treeFolding', '0') == '1') {
  ...
  Graph.prototype.foldTreeCell = function (collapse, cell) {
    ...
    try {
      ...
      this.traverse(cell, true, mxUtils.bind(this, function (vertex, edge) {
        ...
        return (edge == null || treeEdge) && (vertex == cell || !this.model.isCollapsed(vertex));
      }));
  ...
  EditorUi.prototype.addTrees = function () {
    ...
    ui.actions.addAction('selectChildren', function (evt) {
      if (graph.isEnabled() && graph.getSelectionCount() == 1) {
        var cell = graph.getSelectionCell();
        var sib = mxEvent.isControlDown(evt) ? null : graph.getOutgoingTreeEdges(cell);
    ...
    ui.actions.addAction('selectDescendants', function (evt) {
      ...
      if (graph.isEnabled() && cell != null) {
        ...
        if (tmp.length == 0 || tmp.length == 1 && tmp[0] == cell) {
    ...
    ui.actions.addAction('selectParent', function (evt) {
      var cell = graph.getSelectionCell();
      if (graph.isEnabled() && cell != null) {
        var edges = mxEvent.isControlDown(evt) ? null : graph.getIncomingTreeEdges(cell);
    ...
    graph.moveCells = function (cells, dx, dy, clone, target, evt, mapping) {
      ...
      try {
        ...
        if (cells != null && isTreeVertex(target) && mxUtils.getValue(style, 'treeFolding', '0') == '1') {
          ...
          for (var i = 0; i < cells.length; i++) {
            if (isTreeVertex(cells[i]) || graph.model.isEdge(cells[i]) && graph.model.getTerminal(cells[i], true) == null) {
        ...
        if (result != null && cells != null && result.length == cells.length) {
          for (var i = 0; i < result.length; i++) {
            ...
            if (this.model.isEdge(result[i])) {
              if (isTreeVertex(newSource) && mxUtils.indexOf(result, this.model.getTerminal(result[i], true)) < 0) {
              ...
              if (edges.length > 0) {
                if (!clone) {
                  if (isTreeVertex(newSource) && mxUtils.indexOf(cells, this.model.getTerminal(edges[0], true)) < 0) {
                ...
                if (temp != cell && sibling != null) {
                  if (hor && after != sibling.getCenterX() < state.getCenterX() || !hor && after != sibling.getCenterY() < state.getCenterY()) {
                    if (mxUtils.intersects(bbox, sibling)) {
                      dx = spacing + Math.max(dx, (Math.min(bbox.x + bbox.width, sibling.x + sibling.width) - Math.max(bbox.x, sibling.x)) / s);
                      dy = spacing + Math.max(dy, (Math.min(bbox.y + bbox.height, sibling.y + sibling.height) - Math.max(bbox.y, sibling.y)) / s);
                    }
                  }
                }
              }
    ...
    function addChild(cell, direction) {
      ...
      try {
        ...
        if (edges.length == 0) {
          edges = [graph.createEdge(parent, null, '', null, null, graph.createCurrentEdgeStyle())];
          dir = direction;
        }
        var clones = graph.cloneCells([edges[0], cell]);
        ...
        var s = graph.view.scale;
        if (dir == mxConstants.DIRECTION_SOUTH) {
          clones[1].geometry.x = bbox == null ? cell.geometry.x + (cell.geometry.width - clones[1].geometry.width) / 2 : (bbox.x + bbox.width) / s - tr.x - pgeo.x + spacing;
          ...
          clones[1].geometry.y = bbox == null ? cell.geometry.y + (cell.geometry.height - clones[1].geometry.height) / 2 : (bbox.y + bbox.height) / s - tr.y + -pgeo.y + spacing;
        } else {
          clones[1].geometry.x += clones[1].geometry.width - pgeo.x + level;
          clones[1].geometry.y = bbox == null ? cell.geometry.y + (cell.geometry.height - clones[1].geometry.height) / 2 : (bbox.y + bbox.height) / s - tr.y + -pgeo.y + spacing;
        }
    ...
    function getOrderedTargets(cell, horizontal, ref) {
      ...
      if (state != null && sib != null) {
        for (var i = 0; i < sib.length; i++) {
          ...
          if (temp != null && (!horizontal && Math.min(temp.x + temp.width, state.x + state.width) >= Math.max(temp.x, state.x) || horizontal && Math.min(temp.y + temp.height, state.y + state.height) >= Math.max(temp.y, state.y))) {
            targets.push(temp);
          }
        }
    ...
    var altShiftActions = {
      67: ui.actions.get('selectChildren'),
      // Alt+Shift+C
      88: ui.actions.get('selectDescendants'),
      // Alt+Shift+X
      80: ui.actions.get('selectParent'),
      // Alt+Shift+P
    ...
    var altActions = {
      67: ui.actions.get('copyStyle'),
      // Alt+C
      86: ui.actions.get('pasteStyle')
    }; // Alt+V
    ...
    ui.onKeyDown = function (evt) {
      try {
        ...
        if (graph.isEnabled() && !graph.isEditing() && cell != null) {
          var action = mxEvent.isAltDown(evt) && mxEvent.isShiftDown(evt) ? altShiftActions[evt.keyCode] : mxEvent.isAltDown(evt) ? altActions[evt.keyCode] : null;
          if (action != null) {
            action.funct(evt);
            mxEvent.consume(evt);
            ...
            if (graph.getIncomingTreeEdges(cell).length > 0) {
              if (evt.which == 9)
                // Tab adds child
                {
                  cells = mxEvent.isShiftDown(evt) ? addParent(cell) : addChild(cell);
    ...
    graph.getSubtree = function (initialCell) {
      ...
      if ((isTreeMoving(initialCell) || isTreeVertex(initialCell)) && !hasLayoutParent(initialCell)) {
        ...
        graph.traverse(initialCell, true, function (vertex, edge) {
          ...
          if ((edge == null || treeEdge) && mxUtils.indexOf(cells, vertex) < 0) {
            cells.push(vertex);
          }
          return edge == null || treeEdge;
        });
    ...
    mxVertexHandler.prototype.init = function () {
      ...
      if ((isTreeMoving(this.state.cell) || isTreeVertex(this.state.cell)) && !hasLayoutParent(this.state.cell) && this.graph.getOutgoingTreeEdges(this.state.cell).length > 0) {
    ...
    mxVertexHandler.prototype.redrawHandles = function () {
      ...
      if (this.moveHandle != null) {
        this.moveHandle.style.left = this.state.x + this.state.width + (this.state.width < 40 ? 10 : 0) + 2 + 'px';
        this.moveHandle.style.top = this.state.y + this.state.height + (this.state.height < 40 ? 10 : 0) + 2 + 'px';
      }
    };
  ...
  if (typeof Sidebar !== 'undefined') {
    ...
    Sidebar.prototype.createAdvancedShapes = function () {
      ...
      var orgEdgeStyle = 'edgeStyle=elbowEdgeStyle;elbow=vertical;sourcePerimeterSpacing=0;' + 'targetPerimeterSpacing=0;startArrow=none;endArrow=none;rounded=0;curved=0;';
      var mmEdgeStyle = 'newEdgeStyle={"edgeStyle":"entityRelationEdgeStyle","startArrow":"none","endArrow":"none",' + '"segment":10,"curved":1,"sourcePerimeterSpacing":0,"targetPerimeterSpacing":0};';
      var treeEdgeStyle = 'newEdgeStyle={"edgeStyle":"elbowEdgeStyle","startArrow":"none","endArrow":"none"};';
      return result.concat([this.addEntry('tree container', function () {
        var cell = new mxCell('Tree Container', new mxGeometry(0, 0, 400, 320), 'swimlane;startSize=20;horizontal=1;containerType=tree;');
        cell.vertex = true;
        var cell2 = new mxCell('Parent', new mxGeometry(140, 60, 120, 40), 'whiteSpace=wrap;html=1;treeFolding=1;treeMoving=1;' + treeEdgeStyle);
        cell2.vertex = true;
        var cell3 = new mxCell('Child', new mxGeometry(140, 140, 120, 40), 'whiteSpace=wrap;html=1;treeFolding=1;treeMoving=1;' + treeEdgeStyle);
        cell3.vertex = true;
        ...
        cell.insert(cell2);
        cell.insert(cell3);
        return sb.createVertexTemplateFromCells([cell], cell.geometry.width, cell.geometry.height, cell.value);
      }), this.addEntry('tree mindmap mindmaps central idea branch topic', function () {
        var mindmap = new mxCell('Mindmap', new mxGeometry(0, 0, 420, 126), 'swimlane;startSize=20;horizontal=1;containerType=tree;');
        mindmap.vertex = true;
        var cell = new mxCell('Central Idea', new mxGeometry(160, 60, 100, 40), 'ellipse;whiteSpace=wrap;html=1;align=center;' + 'treeFolding=1;treeMoving=1;' + mmEdgeStyle);
        cell.vertex = true;
        var cell2 = new mxCell('Topic', new mxGeometry(320, 40, 80, 20), 'whiteSpace=wrap;html=1;rounded=1;arcSize=50;align=center;verticalAlign=middle;' + 'strokeWidth=1;autosize=1;spacing=4;treeFolding=1;treeMoving=1;' + mmEdgeStyle);
        cell2.vertex = true;
        var edge = new mxCell('', new mxGeometry(0, 0, 0, 0), 'edgeStyle=entityRelationEdgeStyle;' + 'startArrow=none;endArrow=none;segment=10;curved=1;sourcePerimeterSpacing=0;' + 'targetPerimeterSpacing=0;');
        edge.geometry.relative = true;
        edge.edge = true;
        cell.insertEdge(edge, true);
        cell2.insertEdge(edge, false);
        var cell3 = new mxCell('Branch', new mxGeometry(320, 80, 72, 26), 'whiteSpace=wrap;html=1;shape=partialRectangle;top=0;left=0;bottom=1;right=0;points=[[0,1],[1,1]];' + 'fillColor=none;align=center;verticalAlign=bottom;routingCenterY=0.5;' + 'snapToPoint=1;autosize=1;treeFolding=1;treeMoving=1;' + mmEdgeStyle);
        cell3.vertex = true;
        ...
        var cell4 = new mxCell('Topic', new mxGeometry(20, 40, 80, 20), 'whiteSpace=wrap;html=1;rounded=1;arcSize=50;align=center;verticalAlign=middle;' + 'strokeWidth=1;autosize=1;spacing=4;treeFolding=1;treeMoving=1;' + mmEdgeStyle);
        cell4.vertex = true;
        ...
        var cell5 = new mxCell('Branch', new mxGeometry(20, 80, 72, 26), 'whiteSpace=wrap;html=1;shape=partialRectangle;top=0;left=0;bottom=1;right=0;points=[[0,1],[1,1]];' + 'fillColor=none;align=center;verticalAlign=bottom;routingCenterY=0.5;' + 'snapToPoint=1;autosize=1;treeFolding=1;treeMoving=1;' + mmEdgeStyle);
        cell5.vertex = true;
        ...
        return sb.createVertexTemplateFromCells([mindmap], mindmap.geometry.width, mindmap.geometry.height, mindmap.value);
      }), this.addEntry('tree mindmap mindmaps central idea', function () {
        var cell = new mxCell('Central Idea', new mxGeometry(0, 0, 100, 40), 'ellipse;whiteSpace=wrap;html=1;align=center;' + mmEdgeStyle + 'treeFolding=1;treeMoving=1;');
        cell.vertex = true;
        return sb.createVertexTemplateFromCells([cell], cell.geometry.width, cell.geometry.height, cell.value);
      }), this.addEntry('tree mindmap mindmaps branch', function () {
        var cell = new mxCell('Branch', new mxGeometry(0, 0, 80, 20), 'whiteSpace=wrap;html=1;shape=partialRectangle;top=0;left=0;bottom=1;right=0;points=[[0,1],[1,1]];' + 'fillColor=none;align=center;verticalAlign=bottom;routingCenterY=0.5;' + 'snapToPoint=1;recursiveResize=0;autosize=1;treeFolding=1;treeMoving=1;' + mmEdgeStyle);
        cell.vertex = true;
        var edge = new mxCell('', new mxGeometry(0, 0, 0, 0), 'edgeStyle=entityRelationEdgeStyle;' + 'startArrow=none;endArrow=none;segment=10;curved=1;sourcePerimeterSpacing=0;' + 'targetPerimeterSpacing=0;');
        ...
        return sb.createVertexTemplateFromCells([cell, edge], cell.geometry.width, cell.geometry.height, cell.value);
      }), this.addEntry('tree mindmap mindmaps sub topic', function () {
        var cell = new mxCell('Sub Topic', new mxGeometry(0, 0, 80, 26), 'whiteSpace=wrap;html=1;rounded=1;arcSize=50;align=center;verticalAlign=middle;' + 'strokeWidth=1;autosize=1;spacing=4;treeFolding=1;treeMoving=1;' + mmEdgeStyle);
        cell.vertex = true;
        var edge = new mxCell('', new mxGeometry(0, 0, 0, 0), 'edgeStyle=entityRelationEdgeStyle;startArrow=none;' + 'endArrow=none;segment=10;curved=1;sourcePerimeterSpacing=0;' + 'targetPerimeterSpacing=0;');
        edge.geometry.setTerminalPoint(new mxPoint(-40, 40), true);
        edge.geometry.relative = true;
        ...
        var orgchart = new mxCell('Orgchart', new mxGeometry(0, 0, 280, 220), 'swimlane;startSize=20;horizontal=1;containerType=tree;' + treeEdgeStyle);
        orgchart.vertex = true;
        var cell = new mxCell('Organization', new mxGeometry(80, 40, 120, 60), 'whiteSpace=wrap;html=1;align=center;treeFolding=1;treeMoving=1;' + treeEdgeStyle);
        graph.setAttributeForCell(cell, 'treeRoot', '1');
        cell.vertex = true;
        var cell2 = new mxCell('Division', new mxGeometry(20, 140, 100, 60), 'whiteSpace=wrap;html=1;align=center;verticalAlign=middle;treeFolding=1;treeMoving=1;' + treeEdgeStyle);
        cell2.vertex = true;
        var edge = new mxCell('', new mxGeometry(0, 0, 0, 0), orgEdgeStyle);
        edge.geometry.relative = true;
        edge.edge = true;
        cell.insertEdge(edge, true);
        cell2.insertEdge(edge, false);
        var cell3 = new mxCell('Division', new mxGeometry(160, 140, 100, 60), 'whiteSpace=wrap;html=1;align=center;verticalAlign=middle;treeFolding=1;treeMoving=1;' + treeEdgeStyle);
        cell3.vertex = true;
        ...
        return sb.createVertexTemplateFromCells([orgchart], orgchart.geometry.width, orgchart.geometry.height, orgchart.value);
      }), this.addEntry('tree root', function () {
        var cell = new mxCell('Organization', new mxGeometry(0, 0, 120, 60), 'whiteSpace=wrap;html=1;align=center;treeFolding=1;treeMoving=1;' + treeEdgeStyle);
        graph.setAttributeForCell(cell, 'treeRoot', '1');
        cell.vertex = true;
        return sb.createVertexTemplateFromCells([cell], cell.geometry.width, cell.geometry.height, cell.value);
      }), this.addEntry('tree division', function () {
        var cell = new mxCell('Division', new mxGeometry(20, 40, 100, 60), 'whiteSpace=wrap;html=1;align=center;verticalAlign=middle;treeFolding=1;treeMoving=1;' + treeEdgeStyle);
        cell.vertex = true;
        var edge = new mxCell('', new mxGeometry(0, 0, 0, 0), orgEdgeStyle);
        ...
        var cell = new mxCell('Sub Section', new mxGeometry(0, 0, 100, 60), 'whiteSpace=wrap;html=1;align=center;verticalAlign=middle;treeFolding=1;treeMoving=1;');
        cell.vertex = true;
        var edge = new mxCell('', new mxGeometry(0, 0, 0, 0), 'edgeStyle=orthogonalEdgeStyle;sourcePerimeterSpacing=0;targetPerimeterSpacing=0;' + 'startArrow=none;endArrow=none;rounded=0;targetPortConstraint=eastwest;sourcePortConstraint=northsouth;curved=0;rounded=0;');
        edge.geometry.setTerminalPoint(new mxPoint(110, -40), true);
        edge.geometry.relative = true;
        edge.edge = true;
        cell.insertEdge(edge, false);
        var cell2 = new mxCell('Sub Section', new mxGeometry(120, 0, 100, 60), 'whiteSpace=wrap;html=1;align=center;verticalAlign=middle;treeFolding=1;treeMoving=1;');
        cell2.vertex = true;
        var edge2 = new mxCell('', new mxGeometry(0, 0, 0, 0), edge.style);
