...
DrawioFile = function (ui, data) {
  ...
  this.stats = {
    ...
    merged: 0,
    /* number of calls to merge */
    fileMerged: 0,
    /* number of calls to mergeFile */
    fileReloaded: 0,
    /* number of calls to mergeFile */
    conflicts: 0,
    /* number of write conflicts when saving a file */
    timeouts: 0,
    /* number of time we have given up to retry after a write conflict */
    saved: 0,
    /* number of calls to fileSaved */
    closed: 0,
    ...
    bytesSent: 0,
    /* number of bytes send in messages */
    bytesReceived: 0,
    /* number of bytes received in messages */
    msgSent: 0,
    /* number of messages sent */
    msgReceived: 0,
    /* number of messages received */
    cacheHits: 0,
    /* number of times the cache returned patches */
    cacheMiss: 0,
    /* number of times we have missed a cache entry */
...
DrawioFile.prototype.updateFile = function (success, error, abort, shadow, immediate) {
  if (abort == null || !abort()) {
    EditorUi.debug('DrawioFile.updateFile', [this], 'immediate', immediate, 'invalidChecksum', this.invalidChecksum);
    ...
    if (pages != null && pages.length > 0) {
      ...
      if (!ignored) {
        try {
          ...
          this.backupPatch = !this.isModified() ? null : this.ui.diffPages(shadow, this.isRealtime() ? this.ownPages : this.ui.pages);
          ...
          EditorUi.debug('File.mergeFile', [this], 'file', [file], 'shadow', shadow, 'pages', this.ui.pages, 'patches', patches, 'backup', this.backupPatch, 'checksum', checksum, 'current', current, 'valid', checksum == current, 'from', this.getCurrentRevisionId(), 'to', file.getCurrentRevisionId(), 'modified', this.isModified(), 'immediate', immediate);
          if (checksum != null && checksum != current) {
            ...
            if (this.sync != null) {
              var pending = this.sync.patchRealtime(patches, DrawioFile.LAST_WRITE_WINS ? this.backupPatch : null, null, immediate);
            ...
            // Patches the current document
            this.patch(patches, DrawioFile.LAST_WRITE_WINS ? this.backupPatch : null);
          }
  ...
  try {
    if (this.errorReportsEnabled) {
      ...
      var fn = mxUtils.bind(this, function (file) {
        var json = this.compressReportData(JSON.stringify(patches, null, 2));
        var remote = file == null ? 'n/a' : this.compressReportData(this.getAnonymizedXmlForPages(this.ui.getPagesForXml(file.data)), 25000);
        this.sendErrorReport('Checksum Error in ' + functionName + ' ' + this.getHash(), (details != null ? details : '') + '\n\nPatches:\n' + json + (remote != null ? '\n\nRemote:\n' + remote : ''), null, 70000);
      });
      if (etag == null) {
        fn(null);
      } else {
      ...
      this.getLatestVersion(mxUtils.bind(this, function (latestFile) {
        ...
        try {
          ...
          EditorUi.logError('Checksum ' + type + ' in ' + functionName + ' ' + id, null, this.getMode() + '.' + this.getId(), 'user_' + uid + (this.sync != null ? '-client_' + this.sync.clientId : '-nosync') + '-bytes_' + bytes + '-patches_' + patches.length + (data != null ? '-json_' + data : '') + '-size_' + this.getSize() + (checksum != null ? '-expected_' + checksum : '') + (current != null ? '-current_' + current : '') + (rev != null ? '-rev_' + this.ui.hashValue(rev) : '') + (latest != null ? '-latest_' + latest : '') + (latestFile != null ? '-latestRev_' + this.ui.hashValue(latestFile.getCurrentRevisionId()) : ''));
          EditorUi.logEvent({
...
DrawioFile.prototype.reloadFile = function (success, error) {
  try {
    ...
    var fn = mxUtils.bind(this, function () {
      ...
      if (this.getHash() == '') {
        this.mergeLatestVersion(this.backupPatch != null ? [this.backupPatch] : null, mxUtils.bind(this, function () {
          this.backupPatch = null;
          if (success != null) {
            success();
          }
...
DrawioFile.prototype.save = function (revision, success, error, unloading, overwrite, manual) {
  try {
    EditorUi.debug('DrawioFile.save', [this], 'revision', revision, 'unloading', unloading, 'overwrite', overwrite, 'manual', manual, 'saving', this.savingFile, 'editable', this.isEditable(), 'invalidChecksum', this.invalidChecksum);
    if (!this.isEditable()) {
      if (error != null) {
        error({
...
DrawioFile.prototype.share = function () {
  if (this.ui.drive != null) {
    this.ui.confirm(mxResources.get('saveItToGoogleDriveToCollaborate', [this.getTitle()]), mxUtils.bind(this, function () {
      this.ui.pickFolder(App.MODE_GOOGLE, mxUtils.bind(this, function (folderId) {
        var graph = this.ui.editor.graph;
        var selection = graph.getSelectionCells();
        var viewState = graph.getViewState();
        var page = this.ui.currentPage;
        this.ui.createFile(this.getTitle(), this.ui.getFileData(null, null, null, null, null, null, null, null, this), null, App.MODE_GOOGLE, null, true, folderId, null, null, mxUtils.bind(this, function () {
          this.ui.restoreViewState(page, viewState, selection);
...
DrawioFile.prototype.startSync = function () {
  if ((DrawioFile.SYNC == 'auto' || DrawioFile.SYNC == 'fast') && urlParams['stealth'] != '1' && (urlParams['rt'] == '1' || !this.ui.editor.chromeless || this.ui.editor.editable)) {
    if (this.sync == null) {
...
DrawioFile.prototype.addUnsavedStatus = function (err) {
  if (!this.inConflictState && this.ui.statusContainer != null && this.ui.getCurrentFile() == this) {
    if (err instanceof Error && err.message != null && err.message != '') {
      ...
      this.ui.editor.setStatus('<div title="' + status + '" data-title="' + mxUtils.htmlEntities(mxResources.get('unsavedChanges')) + '" data-message="' + mxUtils.htmlEntities(err.message) + '" class="geStatusAlert">' + status + ' (' + mxUtils.htmlEntities(err.message) + ')</div>');
    } else {
      ...
      this.ui.editor.setStatus('<div ' + action + ' title="' + status + '" class="geStatusAlert">' + status + ' <img class="geAdaptiveAsset" src="' + Editor.saveImage + '"/></div>');
...
DrawioFile.prototype.setConflictStatus = function (message, fn) {
  this.ui.editor.setStatus('<div title="' + message + '" ' + (fn != null ? 'data-action="statusFunction"' : '') + ' class="geStatusAlert">' + message + '<img data-link="https://www.drawio.com/doc/faq/synchronize" src="' + Editor.helpImage + '" style="cursor:help;"/></div>', fn);
};
...
DrawioFile.prototype.showRefreshDialog = function (success, error, message) {
  ...
  if (this.ui.editor.isChromelessView() && !this.ui.editor.editable) {
    ...
    this.ui.showError(mxResources.get('warning') + ' (' + message + ')', mxResources.get('fileChangedSyncDialog'), mxResources.get('makeCopy'), mxUtils.bind(this, function () {
...
DrawioFile.prototype.showCopyDialog = function (success, error, overwrite) {
  ...
  this.ui.showError(mxResources.get('externalChanges'), mxResources.get('fileChangedOverwriteDialog'), mxResources.get('makeCopy'), mxUtils.bind(this, function () {
    this.copyFile(success, error);
  }), null, mxResources.get('overwrite'), overwrite, mxResources.get('cancel'), mxUtils.bind(this, function () {
    this.ui.hideDialog();
  }), 380, 150);
};
...
DrawioFile.prototype.showConflictDialog = function (overwrite, synchronize) {
  this.ui.showError(mxResources.get('externalChanges'), mxResources.get('fileChangedSyncDialog'), mxResources.get('overwrite'), overwrite, null, mxResources.get('merge'), synchronize, mxResources.get('cancel'), mxUtils.bind(this, function () {
    this.ui.hideDialog();
...
DrawioFile.prototype.redirectToNewApp = function (error, details) {
  ...
  if (!this.redirectDialogShowing) {
    ...
    var url = window.location.protocol + '//' + window.location.host + '/' + this.ui.getSearch(['create', 'title', 'mode', 'url', 'drive', 'splash', 'state']) + '#' + this.getHash();
...
DrawioFile.prototype.handleFileSuccess = function (saved) {
  ...
  if (this.ui.getCurrentFile() == this) {
    EditorUi.debug('DrawioFile.handleFileSuccess', [this], 'saved', saved, 'modified', this.isModified(), 'remoteFileChanged', this.sync == null ? 'n/a' : this.sync.remoteFileChanged);
    if (this.isModified()) {
      this.fileChanged();
    } else if (saved) {
      if (this.isTrashed()) {
  ...
  if (this.ui.getCurrentFile() == this) {
    if (this.inConflictState) {
      ...
      if (manual) {
        ...
        this.ui.editor.setStatus('<div class="geStatusAlert">' + mxUtils.htmlEntities(mxResources.get('error')) + (msg != null ? ' (' + mxUtils.htmlEntities(msg) + ')' : '') + '</div>');
      }
    }
  }
};
...
DrawioFile.prototype.autosave = function (delay, maxDelay, success, error) {
  ...
  EditorUi.debug('DrawioFile.autosave', [this], 'thread', thread, 'delay', tmp, 'saving', this.savingFile);
