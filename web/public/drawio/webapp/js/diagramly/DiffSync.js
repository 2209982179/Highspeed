 ...
 * Shared codec.
 */
EditorUi.transientViewStateProperties = ['defaultParent', 'currentRoot', 'scrollLeft', 'scrollTop', 'scale', 'translate', 'lastPasteXml', 'pasteCounter'];
...
EditorUi.prototype.applyPatches = function (pages, patches, markPages, resolver, updateEdgeParents) {
  if (patches != null) {
    for (var i = 0; i < patches.length; i++) {
      if (patches[i] != null) {
        pages = this.patchPages(pages, patches[i], markPages, resolver, updateEdgeParents);
      }
    }
  }
  return pages;
...
EditorUi.prototype.patchPages = function (pages, diff, markPages, resolver, updateEdgeParents) {
  ...
  if (pages != null) {
    ...
    for (var i = 0; i < pages.length; i++) {
      ...
      if (moved[prev] == null && !removed[pageId] && (diff[EditorUi.DIFF_UPDATE] == null || diff[EditorUi.DIFF_UPDATE][pageId] == null || diff[EditorUi.DIFF_UPDATE][pageId].previous == null)) {
        moved[prev] = pageId;
      }
      prev = pageId;
    }
  ...
  var addPage = mxUtils.bind(this, function (page) {
    ...
    if (page != null && !added[id]) {
      added[id] = true;
      newPages.push(page);
      var pageDiff = diff[EditorUi.DIFF_UPDATE] != null ? diff[EditorUi.DIFF_UPDATE][id] : null;
      if (pageDiff != null) {
        ...
        if (pageDiff.cells != null) {
          this.patchPage(page, pageDiff.cells, resolverLookup[page.getId()], updateEdgeParents);
        }
        if (markPages && (pageDiff.cells != null || pageDiff.view != null)) {
  ...
  var insertPage = mxUtils.bind(this, function (ins) {
    ...
    if (page == null) {
      ...
      this.patchPage(page, this.diffPages([page], [newPage]), resolverLookup[page.getId()], updateEdgeParents);
      if (markPages) {
        page.needsUpdate = true;
      }
    }
...
EditorUi.prototype.patchPage = function (page, diff, resolver, updateEdgeParents) {
  ...
  try {
    ...
    if (diff[EditorUi.DIFF_UPDATE] != null) {
      var res = resolver != null && resolver.cells != null ? resolver.cells[EditorUi.DIFF_UPDATE] : null;
      for (var id in diff[EditorUi.DIFF_UPDATE]) {
        this.patchCell(model, model.getCell(id), diff[EditorUi.DIFF_UPDATE][id], res != null ? res[id] : null);
      }
    }
...
EditorUi.prototype.patchCellRecursive = function (page, model, cell, parentLookup, diff) {
  if (cell != null) {
    ...
    for (var i = 0; i < childCount; i++) {
      ...
      if (moved[prev] == null && (diff[EditorUi.DIFF_UPDATE] == null || diff[EditorUi.DIFF_UPDATE][cellId] == null || diff[EditorUi.DIFF_UPDATE][cellId].previous == null && diff[EditorUi.DIFF_UPDATE][cellId].parent == null)) {
        moved[prev] = cellId;
      }
      prev = cellId;
    }
    var addCell = mxUtils.bind(this, function (child, insert) {
      ...
      if (child != null) {
        ...
        this.patchCellRecursive(page, model, child, parentLookup, diff);
        index++;
      }
      return id;
    });
    ...
    while (children.length > 0) {
      ...
      if (children.length == 0) {
        ...
        for (var id in inserted) {
          children.push({
            child: this.getCellForJson(inserted[id]),
            insert: true
          });
          delete inserted[id];
        }
...
EditorUi.prototype.patchCell = function (model, cell, diff, resolve) {
  if (cell != null && diff != null) {
    ...
    if (resolve == null || resolve.xmlValue == null && (resolve.value == null || resolve.value == '')) {
      if ('value' in diff) {
    ...
    if (diff.geometry != null) {
      model.setGeometry(cell, this.codec.decode(mxUtils.parseXml(diff.geometry).documentElement));
    }
    if (diff.source != null) {
  ...
  if (oldPages != null && newPages != null) {
    ...
    for (var i = 0; i < oldPages.length; i++) {
      ...
      if (newPage == null) {
        ...
        if ((newPage.prev != null ? prev == null : prev != null) || prev != null && newPage.prev != null && prev.getId() != newPage.prev.getId()) {
        ...
        if (newPage.page.getName() != null && oldPages[i].getName() != newPage.page.getName()) {
    ...
    for (var id in lookup) {
      var newPage = lookup[id];
      inserted.push({
        id: newPage.page.getId(),
        data: mxUtils.getXml(newPage.page.node),
        previous: newPage.prev != null ? newPage.prev.getId() : ''
      });
    }
...
EditorUi.prototype.diffCellRecursive = function (cell, prev, lookup, diff, removed) {
  ...
  if (newCell == null) {
    ...
    if (temp.parent != null || (newCell.prev != null ? prev == null : prev != null) || prev != null && newCell.prev != null && prev.getId() != newCell.prev.getId()) {
...
EditorUi.prototype.getViewStateProperty = function (viewState, key) {
  ...
  if (key == 'backgroundImage' && result != null && result.originalSrc != null) {
    result = {
...
EditorUi.prototype.getCellForJson = function (json) {
  var geometry = json.geometry != null ? this.codec.decode(mxUtils.parseXml(json.geometry).documentElement) : null;
...
EditorUi.prototype.getJsonForCell = function (cell, previous) {
  ...
  if (cell.value != null) {
    if (typeof cell.value === 'object' && typeof cell.value.nodeType === 'number' && typeof cell.value.nodeName === 'string' && typeof cell.value.getAttribute === 'function') {
  ...
  for (var key in cell) {
    if (!this.cellProperties[key] && typeof cell[key] !== 'function') {
      result[key] = cell[key];
    }
  }
  return result;
...
EditorUi.prototype.diffCell = function (oldCell, newCell) {
  ...
  if ((oldCell.parent != null ? newCell.parent == null : newCell.parent != null) || oldCell.parent != null && newCell.parent != null && oldCell.parent.getId() != newCell.parent.getId()) {
    diff.parent = newCell.parent != null ? newCell.parent.getId() : '';
  }
  if ((oldCell.source != null ? newCell.source == null : newCell.source != null) || oldCell.source != null && newCell.source != null && oldCell.source.getId() != newCell.source.getId()) {
    diff.source = newCell.source != null ? newCell.source.getId() : '';
  }
  if ((oldCell.target != null ? newCell.target == null : newCell.target != null) || oldCell.target != null && newCell.target != null && oldCell.target.getId() != newCell.target.getId()) {
    diff.target = newCell.target != null ? newCell.target.getId() : '';
  }
  function isNode(value) {
    return value != null && typeof value === 'object' && typeof value.nodeType === 'number' && typeof value.nodeName === 'string' && typeof value.getAttribute === 'function';
  }
  ;
  ...
  for (var key in oldCell) {
    if (!this.cellProperties[key] && typeof oldCell[key] !== 'function' && typeof newCell[key] !== 'function' && oldCell[key] != newCell[key]) {
  ...
  for (var key in newCell) {
    if (!(key in oldCell) && !this.cellProperties[key] && typeof oldCell[key] !== 'function' && typeof newCell[key] !== 'function' && oldCell[key] != newCell[key]) {
      diff[key] = newCell[key] === undefined ? null : newCell[key];
    }
  }
  return diff;
...
EditorUi.prototype.resolveCrossReferences = function (ownDiff, theirDiff) {
  ...
  EditorUi.debug('EditorUi.resolveCrossReferences', [this], 'ownDiff', ownDiff, 'theirDiff', theirDiff, 'resolve', resolve);
  return resolve;
};
...
EditorUi.prototype.adoptTheirPages = function (ownDiff, theirDiff, resolve) {
  ...
  if (theirDiff[EditorUi.DIFF_INSERT] != null) {
    for (var i = 0; i < theirDiff[EditorUi.DIFF_INSERT].length; i++) {
      theirInsertedPages[theirDiff[EditorUi.DIFF_INSERT][i].id] = theirDiff[EditorUi.DIFF_INSERT][i];
    }
  }
  for (var id in ownDiff[EditorUi.DIFF_UPDATE]) {
    if (theirInsertedPages[id] != null) {
      ...
      resolve[EditorUi.DIFF_INSERT].push(theirInsertedPages[id]);
      resolve[EditorUi.DIFF_UPDATE][id] = ownDiff[EditorUi.DIFF_UPDATE][id];
...
EditorUi.prototype.adoptTheirCellsFromPage = function (ownPageUpdate, theirDiff, pageId, resolve) {
  var theirPageUpdate = theirDiff[EditorUi.DIFF_UPDATE] != null ? theirDiff[EditorUi.DIFF_UPDATE][pageId] : null;
  if (theirPageUpdate != null && theirPageUpdate.cells != null && theirPageUpdate.cells[EditorUi.DIFF_INSERT] != null) {
    ...
    pageDiff.inserted = {};
    this.resolveOwnInsertedCells(ownPageUpdate.cells[EditorUi.DIFF_INSERT], theirInsertedCells, pageDiff);
    this.resolveOwnUpdatedCells(ownPageUpdate.cells[EditorUi.DIFF_UPDATE], theirInsertedCells, theirUpdatedCells, pageDiff);
    if (resolve[EditorUi.DIFF_UPDATE] == null) {
      resolve[EditorUi.DIFF_UPDATE] = {};
    }
...
EditorUi.prototype.resolveOwnInsertedCells = function (ownInsertedCells, theirInsertedCells, pageDiff) {
  if (ownInsertedCells != null) {
    for (var i = 0; i < ownInsertedCells.length; i++) {
      ...
      if (cell != null) {
        this.adoptParentCell(cell.id, null, theirInsertedCells, pageDiff);
        this.adoptTerminalCell(cell.id, cell, theirInsertedCells, true, pageDiff);
        this.adoptTerminalCell(cell.id, cell, theirInsertedCells, false, pageDiff);
      }
    }
  }
};
  ...
  if (ownUpdatedCells != null) {
    for (var id in ownUpdatedCells) {
      ...
      if (cell != null) {
        ...
        pageDiff.cells[EditorUi.DIFF_UPDATE][id] = ownUpdatedCells[id];
      } else if (theirUpdatedCells != null) {
        // Adds their referenced terminals
        // and parents to own cells
        var theirCell = theirUpdatedCells[id];
        if (theirCell != null) {
          this.adoptParentCell(id, theirCell, theirInsertedCells, pageDiff);
          this.adoptTerminalCell(id, theirCell, theirInsertedCells, true, pageDiff, id);
          this.adoptTerminalCell(id, theirCell, theirInsertedCells, false, pageDiff, id);
        }
      }
    }
  }
...
EditorUi.prototype.adoptParentCell = function (cellId, cellDiff, theirInsertedCells, pageDiff) {
  ...
  var parentId = cellDiff != null ? cellDiff.parent : cell != null ? cell.parent : null;
  if (parentId != null) {
...
EditorUi.prototype.adoptTerminalCell = function (cellId, cell, theirInsertedCells, source, pageDiff) {
  ...
  if (terminal != null) {
    ...
    pageDiff.cells[EditorUi.DIFF_UPDATE][cellId][source ? 'source' : 'target'] = terminalId;
  }
};
