      ...
      window.onunload = mxUtils.bind(this, function () {
        ...
        if (file != null && file.isModified()) {
          var evt = {
            ...
            action: (file.savingFile ? 'saving' : '') + (file.savingFile && file.savingFileTime != null ? '_' + Math.round((Date.now() - file.savingFileTime.getTime()) / 1000) : '') + (file.saveLevel != null ? '-sl_' + file.saveLevel : '') + '-age_' + (file.ageStart != null ? Math.round((Date.now() - file.ageStart.getTime()) / 1000) : 'x') + (this.editor.autosave ? '' : '-nosave') + (file.isAutosave() ? '' : '-noauto') + '-open_' + (file.opened != null ? Math.round((Date.now() - file.opened.getTime()) / 1000) : 'x') + '-save_' + (file.lastSaved != null ? Math.round((Date.now() - file.lastSaved.getTime()) / 1000) : 'x') + '-change_' + (file.lastChanged != null ? Math.round((Date.now() - file.lastChanged.getTime()) / 1000) : 'x') + '-alive_' + Math.round((Date.now() - App.startTime.getTime()) / 1000),
            label: file.sync != null ? 'client_' + file.sync.clientId : 'nosync'
          };
          if (file.constructor == DriveFile && file.desc != null && this.drive != null) {
            evt.label += (this.drive.user != null ? '-user_' + this.drive.user.id : '-nouser') + '-rev_' + file.desc.headRevisionId + '-mod_' + file.desc.modifiedDate + '-size_' + file.getSize() + '-mime_' + file.desc.mimeType;
          }
 ...
 */
App.DROPBOX_URL = BASE_DRAW_APP_PATH + 'js/dropbox/Dropbox-sdk.min.js';
 ...
 */
App.ONEDRIVE_URL = mxClient.IS_IE11 ? 'https://js.live.net/v7.2/OneDrive.js' : BASE_DRAW_APP_PATH + 'js/onedrive/OneDrive.js';
...
App.publicPlugin = ['ex', 'tips', 'svgdata', 'number', 'sql', 'props', 'text', 'anim', 'update', 'trees',
//	'import',
'replay', 'anon', 'webcola',
//	'rnd', 'page', 'gd',
'tags'];
...
App.loadScripts = function (scripts, onload, onerror) {
  ...
  for (var i = 0; i < scripts.length; i++) {
    var basePath = window.document.location.origin + '/drawio/webapp/';
    mxscript(basePath + scripts[i], function () {
      ...
      if (urlParams['embed'] != '1') {
        if (typeof window.DriveClient === 'function') {
          if (urlParams['gapi'] != '0' && isSvgBrowser && (document.documentMode == null || document.documentMode >= 10)) {
            ...
            if (App.mode == App.MODE_GOOGLE || urlParams['state'] != null && window.location.hash == '' || window.location.hash != null && window.location.hash.substring(0, 2) == '#G') {
...
App.isErrorHandle = false;
App.handleError = function () {
  if (App.isErrorHandle) {
    return;
  }
  App.isErrorHandle = true;
...
App.main = function (callback, createUi) {
  try {
    ...
    if (window.mxscript != null) {
      ...
      if (urlParams['dev'] == '1' && !mxClient.IS_CHROMEAPP && !EditorUi.isElectronApp && CryptoJS != null && App.mode != App.MODE_DROPBOX && App.mode != App.MODE_TRELLO) {
      ...
      try {
        ...
        if (Editor.enableServiceWorker && (urlParams['offline'] == '0' || /www\.draw\.io$/.test(window.location.hostname) || urlParams['offline'] != '1' && urlParams['dev'] == '1')) {
      ...
      // Loads Pusher API
      if ('ArrayBuffer' in window && !mxClient.IS_CHROMEAPP && !EditorUi.isElectronApp && DrawioFile.SYNC == 'auto' && (urlParams['embed'] != '1' || urlParams['embedRT'] == '1') && urlParams['local'] != '1' && (urlParams['chrome'] != '0' || urlParams['rt'] == '1') && urlParams['stealth'] != '1' && urlParams['offline'] != '1') {
      ...
      if (typeof window.DriveClient === 'function' && typeof gapi === 'undefined' && (urlParams['embed'] != '1' && urlParams['gapi'] != '0' || urlParams['embed'] == '1' && urlParams['gapi'] == '1') && isSvgBrowser && isLocalStorage && (document.documentMode == null || document.documentMode >= 10)) {
        ...
        function realMain() {
          try {
            ...
            if (window.mxscript != null) {
              // Loads dropbox for all browsers but IE8 and below (no CORS) if not disabled or if enabled and in embed mode
              // KNOWN: Picker does not work in IE11 (https://dropbox.zendesk.com/requests/1650781)
              if (typeof window.DropboxClient === 'function' && window.Dropbox == null && window.DrawDropboxClientCallback != null && (urlParams['embed'] != '1' && urlParams['db'] != '0' || urlParams['embed'] == '1' && urlParams['db'] == '1') && isSvgBrowser && (document.documentMode == null || document.documentMode > 9)) {
                mxscript(App.DROPBOX_URL, function () {
                  // Must load this after the dropbox SDK since they use the same namespace
              ...
              // Loads OneDrive for all browsers but IE6/IOS if not disabled or if enabled and in embed mode
              if (typeof window.OneDriveClient === 'function' && typeof OneDrive === 'undefined' && window.DrawOneDriveClientCallback != null && (urlParams['embed'] != '1' && urlParams['od'] != '0' || urlParams['embed'] == '1' && urlParams['od'] == '1') && (navigator.userAgent == null || navigator.userAgent.indexOf('MSIE') < 0 || document.documentMode >= 10)) {
                //Editor.oneDriveInlinePicker can be set with configuration which is done later, so load it all time
              ...
              // Loads Trello for all browsers but < IE10 if not disabled or if enabled and in embed mode
              if (typeof window.TrelloClient === 'function' && !mxClient.IS_IE11 && typeof window.Trello === 'undefined' && window.DrawTrelloClientCallback != null && urlParams['tr'] == '1' && (navigator.userAgent == null || navigator.userAgent.indexOf('MSIE') < 0 || document.documentMode >= 10)) {
                mxscript(App.TRELLO_JQUERY_URL, function () {
                  // Must load this after the dropbox SDK since they use the same namespace
...
App.prototype.init = function () {
   ...
   */
  this.gitHub = (!mxClient.IS_IE || document.documentMode == 10 || mxClient.IS_IE11 || mxClient.IS_EDGE) && urlParams['gh'] != '0' && (urlParams['embed'] != '1' || urlParams['gh'] == '1') ? new GitHubClient(this) : null;
  if (this.gitHub != null) {
   ...
   */
  this.gitLab = (!mxClient.IS_IE || document.documentMode == 10 || mxClient.IS_IE11 || mxClient.IS_EDGE) && urlParams['gl'] != '0' && (urlParams['embed'] != '1' || urlParams['gl'] == '1') ? new GitLabClient(this) : null;
  if (this.gitLab != null) {
  ...
  if (urlParams['embed'] != '1') {
    ...
    if (this.isOwnDomain() && (!this.editor.chromeless || this.editor.editable)) {
      this.editor.addListener('fileLoaded', mxUtils.bind(this, function () {
        ...
        if (!mxClient.IS_CHROMEAPP && !mxClient.IS_ANDROID && !mxClient.IS_IOS && !EditorUi.isElectronApp && !this.isOffline() && urlParams['open'] == null && urlParams['extAuth'] != '1' && (mode == App.MODE_DEVICE || mode == App.MODE_BROWSER)) {
          this.showDownloadDesktopBanner();
        }
      }));
    }
    if (!mxClient.IS_CHROMEAPP && !EditorUi.isElectronApp && DrawioFile.SYNC == 'auto' && urlParams['local'] != '1' && urlParams['stealth'] != '1' && !this.isOffline() && Editor.enableRealtimeCache && (!this.editor.chromeless || this.editor.editable)) {
  ...
  try {
    if (window.location.ancestorOrigins && window.location.hostname && window.location.ancestorOrigins.length && window.location.ancestorOrigins.length > 0) {
      ...
      if (hostname.endsWith('embed.diagrams.net') && window.location.ancestorOrigins.length > 0) {
        var img = new Image();
        img.src = 'https://log.diagrams.net/images/1x1.png?src=EditorEmbedAncestorFrames' + '&v=' + encodeURIComponent(EditorUi.VERSION) + '&data=' + encodeURIComponent(message);
      }
    }
  } catch (e) {
    // ignore
...
App.prototype.sanityCheck = function () {
  ...
  if (file != null && file.isModified() && file.isAutosave() && file.isOverdue()) {
    ...
    this.spinner.stop();
    this.showError(mxResources.get('unsavedChanges'), msg, mxResources.get('ignore'), mxUtils.bind(this, function () {
      this.hideDialog();
...
App.prototype.isOwnDomain = function () {
  return window.location.hostname == 'test.draw.io' || window.location.hostname == 'www.draw.io' || window.location.hostname == 'drive.draw.io' || window.location.hostname == 'preprod.diagrams.net' || window.location.hostname == 'app.diagrams.net';
};
...
App.prototype.showRatingBanner = function () {
  if (!this.bannerShowing && !this['hideBanner' + 'ratingFooter'] && (!isLocalStorage || mxSettings.settings == null || mxSettings.settings['close' + 'ratingFooter'] == null)) {
    ...
    banner.style.cssText = 'position:absolute;bottom:10px;left:50%;max-width:90%;padding:18px 34px 12px 20px;' + 'font-size:16px;font-weight:bold;white-space:nowrap;cursor:pointer;z-index:' + mxPopupMenu.prototype.zIndex + ';';
    ...
    var star = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAABHNCSVQICAgIfAhkiAAAAAlwSFlzAAALEgAACxIB0t1+/AAAABx0RVh0U29mdHdhcmUAQWRvYmUgRmlyZ' + 'XdvcmtzIENTM5jWRgMAAAQRdEVYdFhNTDpjb20uYWRvYmUueG1wADw/eHBhY2tldCBiZWdpbj0iICAgIiBpZD0iVzVNME1wQ2VoaUh6cmVTek5UY3prYzlkIj8+Cjx4OnhtcG1ldGEgeG1sbnM6eD0iYWRvYmU6bnM6bWV0YS8i' + 'IHg6eG1wdGs9IkFkb2JlIFhNUCBDb3JlIDQuMS1jMDM0IDQ2LjI3Mjk3NiwgU2F0IEphbiAyNyAyMDA3IDIyOjExOjQxICAgICAgICAiPgogICA8cmRmOlJERiB4bWxuczpyZGY9Imh0dHA6Ly93d3cudzMub3JnLzE5OTkvMDI' + 'vMjItcmRmLXN5bnRheC1ucyMiPgogICAgICA8cmRmOkRlc2NyaXB0aW9uIHJkZjphYm91dD0iIgogICAgICAgICAgICB4bWxuczp4YXA9Imh0dHA6Ly9ucy5hZG9iZS5jb20veGFwLzEuMC8iPgogICAgICAgICA8eGFwOkNyZW' + 'F0b3JUb29sPkFkb2JlIEZpcmV3b3JrcyBDUzM8L3hhcDpDcmVhdG9yVG9vbD4KICAgICAgICAgPHhhcDpDcmVhdGVEYXRlPjIwMDgtMDItMTdUMDI6MzY6NDVaPC94YXA6Q3JlYXRlRGF0ZT4KICAgICAgICAgPHhhcDpNb2RpZ' + 'nlEYXRlPjIwMDktMDMtMTdUMTQ6MTI6MDJaPC94YXA6TW9kaWZ5RGF0ZT4KICAgICAgPC9yZGY6RGVzY3JpcHRpb24+CiAgICAgIDxyZGY6RGVzY3JpcHRpb24gcmRmOmFib3V0PSIiCiAgICAgICAgICAgIHhtbG5zOmRjPSJo' + 'dHRwOi8vcHVybC5vcmcvZGMvZWxlbWVudHMvMS4xLyI+CiAgICAgICAgIDxkYzpmb3JtYXQ+aW1hZ2UvcG5nPC9kYzpmb3JtYXQ+CiAgICAgIDwvcmRmOkRlc2NyaXB0aW9uPgogICA8L3JkZjpSREY+CjwveDp4bXBtZXRhPgo' + 'gICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgCiAgICAgICAgICAgICAgICAgICAgICAgICAgIC' + 'AgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAKICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgI' + 'CAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgIIImu8AAAAAVdEVYdENyZWF0aW9uIFRpbWUAMi8xNy8wOCCcqlgAAAHuSURBVDiNlZJBi1JRGIbfk+fc0ZuMXorJe4XujWoMdREaA23HICj6AQeLINr0C4I27ab2' + '7VqOI9+q/sH8gMDceG1RkIwgClEXFMbRc5zTZgZURmG+5fu9PN/7Hg6wZohoh4h21nn4uqXW+q0xZgzg+SrPlTXX73uet+26bp6ICpcGaK1fua57M5vN3tZav7gUgIiSqVTqcRAEm0EQbCaTyQoRXb3Iy4hoG8CT6XSaY4xtMMa' + 'SQohMPp8v+r7vAEC3243CMGwqpfoApsaYE8uyfgM45ABOjDEvXdfNlMvlzFINAIDneY7neZVzvdlsDgaDQYtzfsjOIjtKqU+e5+0Wi0V3VV8ACMOw3+/3v3HOX0sp/7K53te11h/S6fRuoVAIhBAL76OUOm2320dRFH0VQuxJKf' + '8BAFu+UKvVvpRKpWe2bYt5fTweq0ajQUKIN1LK43N94SMR0Y1YLLYlhBBKqQUw51wkEol7WmuzoC8FuJtIJLaUUoii6Ljb7f4yxpz6vp9zHMe2bfvacDi8BeDHKkBuNps5rVbr52QyaVuW9ZExttHpdN73ej0/Ho+nADxYCdBaV' + '0aj0RGAz5ZlHUgpx2erR/V6/d1wOHwK4CGA/QsBnPN9AN+llH+WkqFare4R0QGAO/M6M8Ysey81/wGqa8MlVvHPNAAAAABJRU5ErkJggg==';
...
App.prototype.checkLicense = function () {
  ...
  if (!this.isOffline() && !this.editor.chromeless && email != null && driveUser.id != null) {
    ...
    mxUtils.post('/license', 'domain=' + encodeURIComponent(domain) + '&id=' + encodeURIComponent(userId) + '&ts=' + new Date().getTime(), mxUtils.bind(this, function (req) {
      try {
...
App.prototype.onBeforeUnload = function () {
  if (urlParams['embed'] == '1' && this.editor.modified) {
    ...
    if (file != null) {
      ...
      if (file.constructor == LocalFile && file.getHash() == '' && !file.isModified() && urlParams['nowarn'] != '1' && !this.isDiagramEmpty() && urlParams['url'] == null && !this.editor.isChromelessView() && file.fileHandle == null) {
  ...
  try {
    ...
    if (mxClient.IS_CHROMEAPP || this.useCanvasForExport) {
      this.editor.exportToCanvas(mxUtils.bind(this, function (canvas) {
        ...
        success();
      }, null, null, null, null, null, null, graph, null, null, null, null, 'diagram', null);
      result = true;
...
App.prototype.showRefreshDialog = function (title, message) {
  if (!this.showingRefreshDialog) {
    ...
    this.showError(title || mxResources.get('externalChanges'), message || mxResources.get('redirectToNewApp'), mxResources.get('refresh'), mxUtils.bind(this, function () {
      var file = this.getCurrentFile();
      if (file != null) {
  ...
  try {
    ...
    window.onerror = function (message, url, linenumber, colno, err) {
      ...
      if (message != 'ResizeObserver loop limit exceeded') {
        ...
        ui.handleError({
          message: message
        }, mxResources.get('unknownError'), null, null, null, null, true);
      }
    };
    ...
    if ((window.location.hash == null || window.location.hash.length <= 1) && urlParams['desc'] != null) {
      try {
        this.loadDescriptor(JSON.parse(Graph.decompress(urlParams['desc'])), null, mxUtils.bind(this, function (e) {
    ...
    // Redirects old url URL parameter to new #U format
      ...
      if ((window.location.hash == null || window.location.hash.length <= 1) && value != null && value.length > 0 && this.spinner.spin(document.body, mxResources.get('loading'))) {
        ...
        var showCreateDialog = mxUtils.bind(this, function (xml) {
          ...
          if (urlParams['splash'] != '0') {
            ...
            }), null, null, null, null, urlParams['browser'] == '1', null, null, true, rowLimit, null, null, null, this.editor.fileExtensions);
            this.showDialog(dlg.container, 420, serviceCount > rowLimit ? 390 : 280, true, false, mxUtils.bind(this, function (cancel) {
      ...
      for (var i = 0; i < items.length; i++) {
        try {
          ...
          if (key != null && key.substring(0, 7) == '.draft_') {
            ...
            if (obj != null && obj.type == 'draft' && obj.aliveCheck != guid && (filePath == null && obj.fileObject == null || obj.fileObject != null && obj.fileObject.path == filePath)) {
              obj.key = key;
              drafts.push(obj);
            }
          }
    ...
    window.setTimeout(mxUtils.bind(this, function () {
      ...
      this.filterDrafts(null, guid, mxUtils.bind(this, function (drafts) {
        if (drafts.length == 1) {
          ...
          var dlg = new DraftDialog(this, drafts.length > 1 ? mxResources.get('selectDraft') : mxResources.get('draftFound', [ts.toLocaleDateString() + ' ' + ts.toLocaleTimeString()]), drafts.length > 1 ? null : drafts[0].data, mxUtils.bind(this, function (index) {
            this.hideDialog();
            index = index != '' ? index : 0;
...
App.prototype.showSplash = function (force) {
  ...
  var showSecondDialog = mxUtils.bind(this, function () {
    var newBlankFile = mxUtils.bind(this, function (cancel) {
...
App.prototype.pickLibrary = function (mode) {
  ...
  if (mode == App.MODE_GOOGLE || mode == App.MODE_DROPBOX || mode == App.MODE_ONEDRIVE || mode == App.MODE_GITHUB || mode == App.MODE_GITLAB || mode == App.MODE_TRELLO) {
    var peer = mode == App.MODE_GOOGLE ? this.drive : mode == App.MODE_ONEDRIVE ? this.oneDrive : mode == App.MODE_GITHUB ? this.gitHub : mode == App.MODE_GITLAB ? this.gitLab : mode == App.MODE_TRELLO ? this.trello : this.dropbox;
    if (peer != null) {
    ...
    if (!forceDialog && file.getTitle() != null && file.invalidFileHandle == null && this.mode != null) {
          ...
          if (/(\.pdf)$/i.test(name)) {
            ...
            if (prev == null && mode == App.MODE_DEVICE) {
              ...
              var createFile = mxUtils.bind(this, function (folderId) {
                ...
                this.createFile(name, this.getFileData(/(\.xml)$/i.test(name) || name.indexOf('.') < 0 || /(\.drawio)$/i.test(name), /(\.svg)$/i.test(name), /(\.html)$/i.test(name)), null, mode, done, this.mode == null, folderId, null, null, mxUtils.bind(this, function () {
      ...
      if (urlParams['save-dialog'] != '0') {
        ...
        var dlg = new CreateDialog(this, filename, saveFunction, mxUtils.bind(this, function () {
          this.hideDialog();
        }), mxResources.get('saveAs'), mxResources.get('download'), null, null, allowTab, null, true, rowLimit, null, null, null, this.editor.fileExtensions, false);
...
App.prototype.loadTemplate = function (url, onload, onerror, templateFilename, asLibrary) {
  ...
  var isVisioFilename = /(\.v(dx|sdx?))($|\?)/i.test(filterFn) || /(\.vs(x|sx?))($|\?)/i.test(filterFn);
  ...
  }), onerror, /(\.png)($|\?)/i.test(filterFn) || /(\.v(dx|sdx?))($|\?)/i.test(filterFn) || /(\.vs(x|sx?))($|\?)/i.test(filterFn), null, null, base64);
};
...
App.prototype.loadFile = function (id, sameWindow, file, success, force) {
  ...
  if (id == null || id.length == 0) {
    ...
    this.showDialog(new PopupDialog(this, this.getUrl() + '#' + id, null, fn).container, 320, 140, true, true);
  } else {
    fn();
  }
};
...
App.prototype.updateButtonContainer = function () {
  if (this.buttonContainer != null) {
    var file = this.getCurrentFile();
    if (urlParams['embed'] == '1' && Editor.currentTheme != 'simple' && Editor.currentTheme != 'sketch') {
      this.buttonContainer.style.paddingRight = urlParams['atlas'] == '1' ? '32px' : '8px';
    }

    // Comments
    if (this.commentsSupported() && Editor.currentTheme != 'simple' && Editor.currentTheme != 'atlas' && Editor.currentTheme != 'sketch') {
      if (this.commentButton == null) {
        ...
        this.commentButton.style.cssText = 'display:inline-block;position:relative;box-sizing:border-box;' + 'width:24px;height:24px;background-size:24px 24px;background-position:center center;cursor:pointer;' + 'background-repeat:no-repeat;background-image:url(' + Editor.commentImage + ');';
    ...
    // Share
    if (this.getServiceName() == 'draw.io' && urlParams['embed'] != '1' && !this.isStandaloneApp()) {
      if (file != null) {
        ...
        if (this.shareButton != null) {
          this.shareButton.style.display = Editor.currentTheme == 'simple' || Editor.currentTheme == 'sketch' || Editor.currentTheme == 'min' ? 'none' : 'inline-block';

          // Hides parent element if empty for flex layout gap to work
          if (Editor.currentTheme == 'simple' || Editor.currentTheme == 'sketch') {
            this.shareButton.parentNode.style.display = this.shareButton.parentNode.clientWidth == 0 ? 'none' : '';
          }
        }
      ...
      if (this.isStandaloneApp() && (Editor.currentTheme == 'simple' || Editor.currentTheme == 'sketch')) {
    ...
    if (this.commentButton != null) {
      ...
      if (Editor.currentTheme != 'simple' && Editor.currentTheme != 'sketch' && Editor.currentTheme != 'min' && urlParams['embed'] != '1') {
    ...
    if (this.notificationBtn != null) {
      if (Editor.currentTheme != 'simple' && Editor.currentTheme != 'sketch' && Editor.currentTheme != 'atlas' && Editor.currentTheme != 'min' && urlParams['embed'] != '1') {
    ...
    mxUtils.get(NOTIFICATIONS_URL, mxUtils.bind(this, function (req) {
      if (req.getStatus() >= 200 && req.getStatus() <= 299) {
        try {
          ...
          if (isLocalStorage) {
            localStorage.setItem(cachedNotifKey, JSON.stringify({
              ts: Date.now(),
              notifs: notifs
            }));
          }
  ...
  if (notifListEl == null) {
    ...
    for (var i = 0; i < notifs.length; i++) {
      ...
      (function (editorUi, notif) {
        ...
        notifEl.innerHTML = '<div class="circle' + (notif.isNew ? ' active' : '') + '"></div><span class="time">' + mxUtils.htmlEntities(mxResources.get('timeAgo', [str], '{1} ago')) + '</span>' + '<p>' + mxUtils.htmlEntities(notif.content) + '</p>';
        if (notif.link) {
...
App.prototype.updateHeader = function () {
  if (this.menubar != null) {
    ...
    // Prevents focus
    mxEvent.addListener(this.fname, mxClient.IS_POINTER ? 'pointerdown' : 'mousedown', mxUtils.bind(this, function (evt) {
      evt.preventDefault();
    }));
    ...
    // Prevents focus
    mxEvent.addListener(this.toggleFormatElement, mxClient.IS_POINTER ? 'pointerdown' : 'mousedown', mxUtils.bind(this, function (evt) {
      evt.preventDefault();
    }));
    ...
    // Prevents focus
    mxEvent.addListener(this.fullscreenElement, mxClient.IS_POINTER ? 'pointerdown' : 'mousedown', mxUtils.bind(this, function (evt) {
      evt.preventDefault();
    }));
    ...
    if (urlParams['embed'] != '1' && Editor.currentTheme != 'atlas') {
      ...
      // Prevents focus
      mxEvent.addListener(this.toggleElement, mxClient.IS_POINTER ? 'pointerdown' : 'mousedown', mxUtils.bind(this, function (evt) {
        evt.preventDefault();
      }));
...
App.prototype.toggleCompactMode = function (visible) {
  ...
  if (this.toggleElement != null) {
    this.toggleElement.style.backgroundImage = "url('" + (visible ? this.chevronUpImage : this.chevronDownImage) + "')";
  }
  this.refresh();
...
App.prototype.updateUserElementStyle = function () {
  ...
  if (elt != null) {
    if (Editor.currentTheme == 'simple' || Editor.currentTheme == 'sketch' || Editor.currentTheme == 'min') {
      ...
      elt.style.width = '';
      elt.style.height = '';
      elt.style.right = Editor.currentTheme == 'atlas' || this.darkModeElement != null ? '12px' : '26px';
