 ...
 */ ;(function () {
  ...
  ;(function () {
    ...
    EditorUi.prototype.webpSupported = false // Checks if canvas is supported
    try {
    ...
    try {
      ...
      var img = new Image() // LATER: Capability check should not be async
      img.onload = function () {
        try {
          var ctx = canvas.getContext('2d')
          ctx.drawImage(img, 0, 0) // Works in Chrome, Firefox, Edge, Safari and Opera
   ...
   */ EditorUi.prototype.replaceFileData = function (data) {
    ...
    var node =
      data != null && data.length > 0
        ...
        : null // Some nodes must be extracted here to find the mxfile node
    ...
    if (node != null) {
      ...
      try {
        ...
        if (
          ...
          this.fileNode = node
          this.pages = this.pages != null ? this.pages : [] // Wraps page nodes
          for (var i = nodes.length - 1; i >= 0; i--) {
            var page = this.updatePageRoot(new DiagramPage(nodes[i])) // Checks for invalid page names
          ...
          this.editor.setGraphXml(node) // Avoids duplicate parsing of the XML stored in the node
   ...
   */ EditorUi.prototype.createFileData = function (
    ...
    if (node == null) {
      ...
      var fileNode = node // Ignores case for possible HTML or XML nodes
      if (fileNode.nodeName.toLowerCase() != 'mxfile') {
        if (uncompressed) {
          ...
          var data = Graph.compress(text) // Fallback to plain XML for invalid compression
      ...
      var xml = uncompressed
        ...
        : mxUtils.getXml(fileNode) // Writes the file as an embedded HTML file
      if (
        !forceSvg &&
        !forceXml &&
   ...
   */ EditorUi.prototype.getXmlFileData = function (
    ...
    uncompressed =
      uncompressed != null ? uncompressed : !Editor.defaultCompressed // Generats graph model XML node for single page export
    ...
    if (ignoreSelection && this.fileNode != null && this.currentPage != null) {
      ...
      mxUtils.setTextContent(this.currentPage.node, Graph.compressNode(node)) // Creates a clone of the file node for processing
      node = this.fileNode.cloneNode(false) // Appends the node of the page and applies compression
        ...
        for (var i = 0; i < this.pages.length; i++) {
          ...
          if (page != this.currentPage) {
            if (page.needsUpdate) {
              ...
              mxUtils.setTextContent(currNode, Graph.compressNode(temp)) // Marks the page as up-to-date
              delete page.needsUpdate
            } else if (resolveReferences) {
              this.updatePageRoot(page) // Forces update of background page image in offscreen page
   ...
   */ EditorUi.prototype.getFileData = function (
    ...
    uncompressed =
      uncompressed != null ? uncompressed : !Editor.defaultCompressed
    var graph = this.editor.graph // Forces compression of embedded XML
    if (
      forceSvg ||
      (!forceXml && file != null && /(\.svg)$/i.test(file.getTitle()))
    ) {
      var darkTheme =
        graph.themes != null && graph.defaultThemeName == 'darkTheme' // Exports SVG for first page while other page is visible by creating a graph
   ...
   */ EditorUi.prototype.getHtml = function (
    ...
    ignoreSelection = ignoreSelection != null ? ignoreSelection : true
    var bg = null
    var js = EditorUi.drawHost + '/js/embed-static.min.js' // LATER: Merge common code with EmbedDialog
    if (graph != null) {
      ...
      bg = graph.background // Embed script only used if no redirect
      if (redirect == null) {
    ...
    if (node != null) {
      ...
      node.setAttribute('border', '20') // Hidden attributes
    ...
    var text = node != null ? Graph.zapGremlins(mxUtils.getXml(node)) : '' // Double compression for mxfile not fixed since it may cause imcompatibilites with
    // embed clients that rely on this format. HTML files and export use getHtml2.
    var data = Graph.compress(text) // Fallback to URI encoded XML for invalid compression
    if (Graph.decompress(data) != text) {
   ...
   */ EditorUi.prototype.getHtml2 = function (
    ...
    var js =
      window.DRAWIO_VIEWER_URL || EditorUi.drawHost + '/js/viewer-static.min.js' // Makes XHTML compatible
    if (redirect != null) {
      redirect = redirect.replace(/&/g, '&amp;')
    }
    var data = {
   ...
   */ EditorUi.prototype.setFileData = function (data) {
    ...
    if (cause) {
      ...
      if (node != null && node.nodeName == 'mxfile') {
        ...
        if (nodes.length > 0) {
          ...
          for (var i = 0; i < nodes.length; i++) {
            ...
            var page = new DiagramPage(nodes[i]) // Checks for invalid page names
            if (page.getName() == null) {
              page.setName(mxResources.get('pageWithNumber', [i + 1]))
            }
      ...
      this.editor.setGraphXml(node) // Avoids duplicate parsing of the XML stored in the node
      if (this.currentPage != null) {
        this.currentPage.root = this.editor.graph.model.root // Scrolls to current page
        this.scrollToPage()
      }
      if (urlParams['layer-ids'] != null) {
        try {
          ...
          var children = model.getChildren(model.root) // handle layers visibility
      ...
      if (format == 'xml') {
        ...
        this.saveRequest(
          ...
          mxUtils.bind(this, function (newTitle, base64) {
            try {
              var prev = this.editor.graph.pageVisible //Only override if page is actually visible
   ...
   */ EditorUi.prototype.downloadRequestBuilder = function (
    ...
    var graph = this.editor.graph
    var bounds = graph.getGraphBounds() // Exports only current page for images that does not contain file data, but for
   ...
   */ EditorUi.prototype.getDiagramId = function () {
    var id = window.location.hash // Strips the hash sign
   ...
   */ EditorUi.prototype.loadDescriptor = function (desc, success, error) {
    ...
    if (desc.url != null && desc.url.length > 0) {
      var url = this.editor.getProxiedUrl(desc.url) // LATER: Remove cache-control header
      this.editor.loadUrl(
        url,
              ...
              if (cell != null) {
                ...
                try {
                  ...
                  if (geo != null) {
                    ...
                    if (curr != null) {
                      curr = curr.clone() // Partially overwrites geometry
                      for (key in geo) {
   ...
   */ EditorUi.prototype.getCopyFilename = function (file, timestamp) {
    var title =
      file != null && file.getTitle() != null
        ? file.getTitle()
        : this.defaultFilename // Handles extension
    var extension = ''
   ...
   */ EditorUi.prototype.fileLoaded = function (file, noDialogs, success) {
    ...
    var noFile = mxUtils.bind(this, function () {
      ...
      this.setCurrentFile(null) // Keeps initial title if no file existed before
      if (oldFile != null) {
        this.updateDocumentTitle()
      } // File might have been loaded halfway
      this.editor.graph.model.clear()
      this.editor.undoManager.clear()
      this.setBackgroundImage(null) // Avoids empty hash with no value
      if (
        !noDialogs &&
    ...
    if (file != null) {
      try {
        ...
        file.open()
        delete this.openingFile // DescriptorChanged updates the enabled state of the graph
        this.setGraphEnabled(true)
        this.setMode(file.getMode())
        this.editor.graph.model.prefix = Editor.guid() + '-'
        this.editor.undoManager.clear()
        this.descriptorChanged()
        this.updateUi() // Realtime files have a valid status message
        if (!file.isEditable()) {
          this.editor.setStatus(
          ...
          file.addUnsavedStatus() // Restores unsaved data
        ...
        if (!this.editor.isChromelessView() || this.editor.editable) {
          ...
          this.restoreLibraries() // Workaround for no initial focus in FF
        ...
        EditorUi.debug('File.opened', [file]) //Notify users that editing is disabled within mobile apps (mainly for MS Teams)
        ...
        this.fileLoadedError = e // Disconnects file from UI
        if (file != null) {
          try {
            file.close()
          } catch (e2) {
   ...
   */ EditorUi.prototype.getHashValueForPages = function (pages, details) {
    ...
    for (var i = 0; i < pages.length; i++) {
      ...
      var diagram = pages[i].node.cloneNode(false) // FIXME: Check why names can be null in newer files
      // ignore in hash and do not diff null names for now
      diagram.removeAttribute('name') // Model is only a holder for the root
      model.root = pages[i].root
      var xmlNode = codec.encode(model)
      this.editor.graph.saveViewState(pages[i].viewState, xmlNode, true) // Local defaults may be different in files so ignore
   ...
   */ EditorUi.prototype.hashValue = function (obj, replacer, details) {
    var hash = 0 // Checks for XML nodes
    if (
      obj != null &&
      typeof obj === 'object' &&
   ...
   */ EditorUi.prototype.libraryLoaded = function (
    ...
    var nextSibling = elts != null ? elts[elts.length - 1].nextSibling : null // Removes existing sidebar entry for this library
    this.removeLibrarySidebar(file.getHash())
    var dropTarget = null
    ...
    this.repositionLibrary(nextSibling) // Adds tooltip for backend
    ...
    if (file.isEditable()) {
      ...
      mxEvent.addListener(btn2, 'click', addSelection) // Hack to add selection via context menu
      if (file.title == '.scratchpad') {
   ...
   */ EditorUi.prototype.handleError = function (
    ...
    var resume =
      this.spinner != null && this.spinner.pause != null
        ? this.spinner.pause()
        : function () {}
    var e = resp != null && resp.error != null ? resp.error : resp // Logs errors and writes stack to console
    if (
      resp != null &&
        ...
        if (e.code == 404 || e.status == 404 || e.code == 403) {
          ...
          var id =
            notFoundMessage != null
              ...
              : fileHash != null
              ? fileHash
              : window.location.hash // #U handles case where we tried to fallback to Google File and
          ...
          if (
            ...
            id =
              id.substring(0, 2) == '#U'
                ...
                : id.substring(2) // Special case where the button must have a different label and function
            this.showError(
              title,
              msg,
              ...
              mxUtils.bind(this, function () {
                ...
                usersSelect.style.width = '100%' //TODO This code is similar to Dialogs.js change user part in SplashDialog
                function fillUsersSelect() {
                  ...
                  for (var i = 0; i < driveUsers.length; i++) {
                    ...
                    option.value = i
                    usersSelect.appendChild(option) //More info (email) about the user in a disabled option
  ...
  /**
   *
   */ EditorUi.prototype.createImageDataUri = function (
    canvas,
    xml,
    format,
    dpi
  ) {
    var data = canvas.toDataURL('image/' + format) // Checks for valid output
   ...
   */ EditorUi.prototype.doSaveLocalFile = function (
    ...
    if (window.Blob && navigator.msSaveOrOpenBlob) {
      ...
      var a = document.createElement('a') // Workaround for mxXmlRequest.simulate no longer working in PaleMoon
      ...
      var useDownload =
        ...
        typeof a.download !== 'undefined' // Workaround for Chromium 65 cross-domain anchor download issue
        ...
        mxUtils.bind(this, function (evt) {
          ...
          if (this.tagsDialog != null) {
            ...
            this.tagsDialog.style.bottom =
              ...
              4 +
              'px' // Puts the dialog on top of the container z-index
              ...
              var doSave = mxUtils.bind(this, function (folderId) {
                mimeType =
                  mimeType != null
                    ...
                    : format == 'pdf'
                    ? 'application/pdf'
                    : 'image/' + format // Workaround for no roundtrip required if data is available on client-side
    ...
    if (this.spinner.spin(document.body, mxResources.get('export'))) {
      try {
        ...
        if (bg == null && transparentBackground == false) {
          bg =
            theme == 'dark' && !Editor.enableSvgDarkMode
              ? Editor.darkColor
              : '#ffffff'
        } // Sets or disables alternate text for foreignObjects. Disabling is needed
        // because PhantomJS seems to ignore switch statements and paint all text.
   ...
   */ EditorUi.prototype.createLink = function (
    ...
    if (url != null) {
      ...
      var file = this.getCurrentFile() // Fallback to non-public URL for Drive files
   ...
   */ EditorUi.prototype.showPublishLinkDialog = function (
    ...
    var edit = editSection.getEditInput() // Cannot disable lightbox in iframes
    if (showFrameOption) {
   ...
   */ EditorUi.prototype.showExportDialog = function (
    ...
    selfOption.setAttribute('value', 'self')
    mxUtils.write(selfOption, mxResources.get('openInThisWindow'))
    linkSelect.appendChild(selfOption) //Inkscape doesn't support links from pdf to svg. Related to https://gitlab.com/inkscape/inbox/-/issues/583
   ...
   */ EditorUi.prototype.createEmbedImage = function (
    ...
    function doUpdate(dataUri) {
      var onclick = ' '
      var css = '' // Adds double click handling
      if (lightbox) {
   ...
   */ EditorUi.prototype.createEmbedSvg = function (
    ...
    if (image) {
      ...
      var css = '' // Adds double click handling
      if (lightbox) {
        var page = this.getSelectedPageIndex() // KNOWN: Message passing does not seem to work in IE11
        var js =
          '(function(svg){var src=window.event.target||window.event.srcElement;' + // Ignores link events
          "while (src!=null&&src.nodeName.toLowerCase()!='a'){src=src.parentNode;}if(src==null)" + // Focus existing lightbox
          '{if(svg.wnd!=null&&!svg.wnd.closed){svg.wnd.focus();}else{var r=function(evt){' + // Message handling
          ...
          "window.addEventListener('message',r);" + // Opens lightbox window
          "svg.wnd=window.open('" +
          EditorUi.lightboxHost +
          '/?client=1' +
          (page != null ? '&page=' + page : '') +
   ...
   */ EditorUi.prototype.getEmbeddedPng = function (
    ...
    try {
      ...
      var diagramData = null // Exports PNG for given optional data
      ...
      this.editor.exportToCanvas(
        mxUtils.bind(this, function (canvas) {
          try {
            ...
            success(data.substring(data.lastIndexOf(',') + 1)) // Removes temporary graph from DOM
            if (graph != this.editor.graph) {
              graph.container.parentNode.removeChild(graph.container)
            }
          } catch (e) {
   ...
   */ EditorUi.prototype.exportImage = function (
    ...
    if (this.spinner.spin(document.body, mxResources.get('exporting'))) {
      var selectionEmpty = this.editor.graph.isSelectionEmpty()
      ignoreSelection =
        ignoreSelection != null ? ignoreSelection : selectionEmpty // Caches images
      if (this.thumbImageCache == null) {
        this.thumbImageCache = new Object()
      }
      try {
    ...
    try {
      ...
      if (xml != null && xml.length > 0) {
        ...
        try {
          ...
          var mapping = {} // Checks for mxfile with multiple pages
          var node = this.editor.extractGraphModel(
            doc.documentElement,
            this.pages != null
          )
            ...
            if (diagrams.length == 1 && !addNewPage) {
              ...
              if (this.currentPage != null) {
                mapping[diagrams[0].getAttribute('id')] =
                  this.currentPage.getId() // Renames page if diagram has one blank page with default name
                if (this.isBlankFile()) {
              ...
              var pages = []
              var i0 = 0 // Adds first page to current page if current page is only page and empty
              if (
                this.pages != null &&
                this.pages.length == 1 &&
                this.isDiagramEmpty()
              ...
              for (var i = i0; i < diagrams.length; i++) {
                ...
                var index = this.pages.length // Checks for invalid page names
                if (page.getName() == null) {
        ...
        if (type == 'txt') {
          ...
          reader.onloadend = function (e) {
            ...
            img.onload = function () {
              try {
                var w = img.width
                var h = img.height // Workaround for 0 image size in IE11
                if (w == 0 && h == 0) {
   ...
   */ EditorUi.prototype.insertTextAt = function (
    ...
    resizeImages = resizeImages != null ? resizeImages : true // Handles special case for Gliffy data which requires async server-side for parsing
    if (text != null) {
      if (
        Graph.fileSupport &&
        new XMLHttpRequest().upload &&
        ...
        var graph = this.editor.graph // Checks for embedded XML in PDF
        ...
        this.loadImage(
          ...
          mxUtils.bind(this, function () {
            var cell = null // Inserts invalid data URIs as text
            graph.getModel().beginUpdate()
            try {
              cell = graph.insertVertex(
        ...
        if (this.isCompatibleString(text)) {
          ...
          if (this.isLucidChartData(text)) {
            ...
            try {
              ...
              cell.value = text
              graph.updateCellSize(cell) // Adds wrapping for large text blocks
              if (
                this.maxTextWidth > 0 &&
        ...
        mxEvent.addListener(
          ...
          mxUtils.bind(this, function () {
            if (input.files != null) {
              ...
              this.importFiles(input.files, null, null, this.maxImageSize) // Resets input to force change event for same file (type reset required for IE)
              input.type = ''
              input.type = 'file'
              input.value = ''
            }
   ...
   */ EditorUi.prototype.importFile = function (
    ...
    if (mimeType.substring(0, 5) == 'image') {
      ...
      if (!containsModel) {
        var graph = this.editor.graph // Strips encoding bit (eg. ;base64,) for cell style
      ...
      if (this.isOffline()) {
        ...
        //  LATER: done and async are a hack before making this asynchronous
        async = true // Returns empty cells array as it is aysynchronous
   ...
   */ EditorUi.prototype.importFiles = function (
    ...
    x = x != null ? x : 0
    y = y != null ? y : 0 // Checks if large images are imported
    ...
    var doImportFiles = mxUtils.bind(this, function () {
      ...
      if (this.spinner.spin(document.body, mxResources.get('loading'))) {
        ...
        var remain = count
        var queue = [] // Barrier waits for all files to be loaded asynchronously
    ...
    if (
      ...
      try {
        ...
        if (factor > 1) {
          ...
          var tmp = canvas.toDataURL() // Uses new image if smaller
  ...
  EditorUi.prototype.init = function () {
    ...
    var ui = this
    var graph = this.editor.graph // Stops panning while freehand is active
    if (Graph.touchStyle) {
    ...
    if (this.menus != null) {
      var menusAddPopupMenuItems = Menus.prototype.addPopupMenuItems // Inserts zoomIn/zoomOut into popup menu
      this.menus.addPopupMenuItems = function (menu, cell, evt) {
        ...
        menusAddPopupMenuItems.apply(this, arguments) // Shows add to scratchpad option
      ...
      var menusAddPopupMenuEditItems = Menus.prototype.addPopupMenuEditItems // Inserts copyAsImage into popup menu
        ...
        if (ui.editor.graph.isSelectionEmpty()) {
          ...
          if (!this.isShowStyleItems()) {
            if (
              ...
              menu.addSeparator() // Shows line submenu for edges
    ...
    this.defaultFilename = mxResources.get('untitledDiagram') // Adds export for %page%, %pagenumber% and %pagecount% placeholders
    ...
    graph.labelLinkClicked = function (state, elt, evt) {
      ...
      if (
        ...
        if (
          ...
          graph.customLinkClicked(href) // Resets rubberband after click on locked cell
          graph.getRubberband().reset()
        }
        mxEvent.consume(evt)
      } else {
    ...
    printAction.visible = printAction.isEnabled() // Installs additional keyboard shortcuts for editor
    if (!this.editor.chromeless || this.editor.editable) {
    ...
    this.spinner = this.createSpinner(null, null, 24) // Installs drag and drop handler for rich text editor
    if (Graph.fileSupport) {
      graph.addListener(
    ...
    if (urlParams['styledev'] == '1') {
      ...
      if (footer != null) {
        ...
        this.styleInput.style.left = '2px' // Workaround for ignore right CSS property in FF
          ...
          if (graph.isEnabled()) {
            ...
            if (files.length > 0) {
              ...
              if (data != null) {
                ...
                var div = document.createElement('div')
                div.innerHTML = Graph.sanitizeHtml(html) // The default is based on the extension
                var asImage = null // Extracts single image
        ...
        mxUtils.bind(this, function (evt) {
          if (!mxEvent.isConsumed(evt)) {
            try {
              ...
              var containsText = false // Workaround for asynchronous paste event processing in textInput
   ...
   */ EditorUi.prototype.installNativeClipboardHandler = function () {
    var graph = this.editor.graph // Focused but invisible textarea during control or meta key events
    ...
    textInput.innerHTML = '&nbsp;'
    var restoreFocus = false // Disables built-in cut, copy and paste shortcuts
    this.keyHandler.bindControlKey(88, null)
    this.keyHandler.bindControlKey(67, null)
    this.keyHandler.bindControlKey(86, null) // Shows a textare when control/cmd is pressed to handle native clipboard actions
    mxEvent.addListener(
      document,
      'keydown',
      mxUtils.bind(this, function (evt) {
        ...
        if (
          ...
          graph.container.offsetParent != null && // keydown事件在graph不显示时不触发
          graph.isEnabled() &&
          !graph.isMouseDown &&
          !graph.isEditing() &&
          this.dialog == null &&
          ...
          if (
            ...
            if (!restoreFocus) {
              ...
              restoreFocus = true
              textInput.focus()
              document.execCommand('selectAll', false, null) // Workaround for Safari 16 scroll after CMD key press
    ...
    mxEvent.addListener(
      ...
      mxUtils.bind(this, function (evt) {
        ...
        var keyCode = evt.keyCode // Asynchronous workaround for scroll to origin after paste if the
        ...
        window.setTimeout(
          mxUtils.bind(this, function () {
            if (
              ...
              textInput.parentNode.removeChild(textInput) // Workaround for lost cursor in focused element
              if (this.dialog == null) {
   ...
   */ EditorUi.prototype.installStatusMinimizer = function (parent) {
    ...
    var statusChanged = mxUtils.bind(this, function () {
      if (Editor.currentTheme == 'sketch') {
        ...
        if (!visible && elt != null) {
          ...
          var key =
            file != null
              ...
              : DrawioFile.prototype.savingStatusKey // Shows animated spinner while saving
        ...
        this.statusContainer.style.display = 'none'
        var empty = parent.clientWidth <= 32 // Hides container if empty and no status
        parent.style.visibility =
          empty && this.editor.getStatus() == '' ? 'hidden' : '' // Shows status if container empty or status relevant
        if (empty || visible) {
          this.statusContainer.style.display = 'inline-flex'
          visible = true
        }
   ...
   */ EditorUi.prototype.switchTheme = function (value) {
    ...
    this.createMenubarForTheme(value) // TODO: Check what hides sidebarContainer
    ...
    if (value == 'sketch') {
      ...
      this.formatContainer.style.border = 'none' // Shapes window
      var libs =
    ...
    if (value == 'simple' || value == 'sketch') {
      if (this.sketchStyleElt == null) {
        ...
        document
          ...
          .appendChild(this.sketchStyleElt)
      }
    } else {
      if (this.sketchStyleElt != null) {
   ...
   */ EditorUi.prototype.createWrapperForTheme = function (value) {
    if (value == 'simple' || value == 'sketch') {
      if (this.sketchWrapperElt == null) {
        ...
        this.sketchWrapperElt.style.cssText =
          'position:absolute;' +
   ...
   */ EditorUi.prototype.createMainMenuForTheme = function (value) {
    if (value == 'simple' || value == 'sketch') {
      if (this.sketchMainMenuElt == null) {
        ...
        this.sketchMainMenuElt.style.cssText =
          'position:absolute;' +
          'padding:9px 12px;overflow:hidden;white-space:nowrap;' +
          'user-select:none;box-sizing:border-box;'
        var elt = this.createMenu(
      ...
      if (this.sketchFooterMenuElt == null) {
        ...
        if (value != 'simple') {
          ...
          pageMenu.style.cssText =
            'display:inline-block;cursor:pointer;overflow:hidden;padding:4px 16px 4px 4px;' +
          ...
          updatePageName() // Page menu only visible for multiple pages
          var pagesVisibleChanged = mxUtils.bind(this, function () {
            pageMenu.style.display = this.isPageMenuVisible()
              ? 'inline-block'
              : 'none'
          })
          this.addListener(
        ...
        if (value == 'simple') {
          ...
          elt.style.backgroundImage =
            'url(' + mxWindow.prototype.minimizeImage + ')'
        ...
        if (value == 'simple') {
          ...
          footer.appendChild(redoElt) // Page menu only visible for multiple pages
        ...
        mxUtils.bind(this, function (elt) {
          ...
          this.editor.addListener('pageSelected', updateZoom) // Zoom Preview
          this.editor.graph.addListener(
            'zoomPreview',
            mxUtils.bind(this, function (sender, evt) {
        ...
        if (value == 'simple') {
          this.sketchFooterMenuElt.style.cssText =
            'position:relative;white-space:nowrap;gap:6px;' +
          ...
          this.sketchFooterMenuElt.style.cssText =
            'position:absolute;right:12px;bottom:12px;height:44px;' +
   ...
   */ EditorUi.prototype.createPickerMenuForTheme = function (value) {
    console.log('createPickerMenuForTheme')
    if (value == 'simple' || value == 'sketch') {
      if (this.sketchPickerMenuElt == null) {
        ...
        var freehandElt = this.createMenuItem(
          'insertFreehand',
          value == 'simple' ? Editor.thinGestureImage : Editor.freehandImage,
          true
        )
        ...
        var insertElt = this.createMenu(
          'insert',
          value == 'simple' ? Editor.thinAddCircleImage : Editor.addBoxImage
        )
        ...
        shapesElt.style.backgroundImage =
          'url(' +
          (value == 'simple' ? Editor.thinShapesImage : Editor.shapesImage) +
          ')'
        ...
        var initPicker = mxUtils.bind(this, function (force) {
          if (
            force ||
            (document.body != null && document.body.contains(picker))
          ) {
            ...
            if (!collapsed) {
              var iw =
                window.innerWidth ||
                document.documentElement.clientWidth ||
                document.body.clientWidth // Thinner previews in simple toolbar
              if (value == 'simple') {
              ...
              if (value != 'simple' || iw >= 660) {
                var textElt = this.sidebar.createVertexTemplate(
                  ...
                  tw + 10,
                  th + 10,
                  value == 'simple' ? Editor.thinTextImage : null,
                  true
                )
                if (value == 'simple') {
                ...
                addElt(
                  ...
                  'A',
                  32
                ).style.margin = value == 'simple' ? '0 -8px 0 0' : '0 0 0 -2px'
              }
              ...
              if (value == 'simple') {
                ...
                cell.geometry.setTerminalPoint(
                  new mxPoint(cell.geometry.width, 0),
                  false
                )
        ...
        if (value == 'simple') {
          ...
          this.sketchPickerMenuElt.style.cssText =
            'position:absolute;left:10px;border-radius:4px;' +
   ...
   */ EditorUi.prototype.createMenubarForTheme = function (value) {
    if (value == 'simple' || value == 'sketch') {
      if (this.sketchMenubarElt == null) {
        ...
        var css =
          'display:flex;white-space:nowrap;user-select:none;justify-content:flex-end;' +
          'align-items:center;flex-wrap:nowrap;gap:6px;'
        if (value == 'simple') {
          this.sketchMenubarElt.style.cssText =
            'position:relative;flex-grow:0.5;' +
            'overflow:visible;' +
            (urlParams['embed'] != '1' ? 'flex-shrink:0;' : 'min-width:0;') +
            css
          ...
          if (this.formatElt == null) {
            ...
            this.formatElt.className =
              cls + (this.formatWidth == 0 ? '' : ' geActiveItem')
          ...
          this.sketchMenubarElt.style.cssText =
            'position:absolute;right:12px;top:10px;height:44px;' +
          ...
          var refreshMenu = mxUtils.bind(this, function () {
            if (Editor.currentTheme == 'sketch') {
              ...
              var iw =
                ...
                document.body.clientWidth
              if (this.commentElt != null) {
                this.commentElt.style.display =
                  iw > 560 && this.commentsSupported() ? '' : 'none'
              }
      ...
      if (elt != null) {
        ...
        elt.style.opacity = ''
        this.menubarContainer.parentNode.insertBefore(
          elt,
   ...
   */ EditorUi.prototype.createFormatWindow = function () {
    if (this.formatWindow == null) {
      ...
      this.formatWindow.window.minimumSize = new mxRectangle(0, 0, 240, 80) // Sets initial state for format window
      if (Editor.currentTheme == 'sketch') {
        var iw =
          window.innerWidth ||
          document.documentElement.clientWidth ||
   ...
   */ EditorUi.prototype.toggleShapesPanel = function (visible) {
    if (this.isShapesPanelVisible() != visible) {
      var size = EditorUi.prototype.hsplitPosition // On smaller screens this is set to 0
      if (size == 0) {
   ...
   */ EditorUi.prototype.setSketchMode = function (value) {
    if (this.spinner.spin(document.body, mxResources.get('working') + '...')) {
      window.setTimeout(
        mxUtils.bind(this, function () {
          this.spinner.stop()
          this.doSetSketchMode(value) // Persist setting
    ...
    return (
      (Editor.isDarkMode()
        ? 'html body .geMenubarContainer .geMenuItem .geMenuItem, html body .geMenubarContainer a.geMenuItem { color: #353535; }' +
          ...
          'html body .mxCellEditor { color: #f0f0f0; }' // Non-dark mode styles
        : 'div.diagramContainer button.gePrimaryBtn, .mxWindow button.gePrimaryBtn, .geDialog button.gePrimaryBtn, html body .gePrimaryBtn ' +
          ...
          'html body .gePrimaryBtn:hover:not([disabled]) { background: #12a2e0 !important; }') + // End of custom styles
   ...
   */ EditorUi.prototype.setAndPersistLanguage = function (value) {
    ...
    mxSettings.save() // Shows dialog in new language
  ...
  EditorUi.setGraphDarkMode = function (graph, container, darkMode) {
    ...
    graph.loadStylesheet() // Sets global vars
    ...
    if (Editor.enableCssDarkMode) {
      ...
      var delayed = mxUtils.bind(this, function () {
        if (Editor.darkMode != value) {
          var graph = this.editor.graph
          Editor.darkMode = value // Sets instance vars and graph stylesheet
          this.spinner.opts.color = Editor.isDarkMode() ? '#c0c0c0' : '#000'
          EditorUi.setGraphDarkMode(graph, document.body, Editor.isDarkMode()) // Destroys windows with code for dark mode
          if (this.actions.layersWindow != null) {
          ...
          Editor.checkmarkImage =
            Editor.isDarkMode() && mxClient.IS_SVG
              ...
              : Editor.lightCheckmarkImage // Updates CSS
          if (this.sketchStyleElt != null) {
            this.sketchStyleElt.innerHTML = Editor.createMinimalCss()
   ...
   */ EditorUi.prototype.setPagesVisible = function (value) {
    if (Editor.pagesVisible != value) {
      Editor.pagesVisible = value // Persist setting
   ...
   */ EditorUi.prototype.setSidebarTitles = function (value, remember) {
    if (this.sidebar.sidebarTitles != value) {
      ...
      this.sidebar.refresh() // Persist setting
   ...
   */ EditorUi.prototype.installSettings = function () {
    if (Editor.isSettingsEnabled()) {
      ...
      Editor.pagesVisible = mxSettings.settings.pagesVisible // Gets recent colors from settings
      ColorDialog.recentColors = mxSettings.getRecentColors() // Avoids overridden values for changes in
          ...
          window.addEventListener(
            ...
            mxUtils.bind(this, function (evt) {
              if (evt.key == mxSettings.key) {
                mxSettings.load() // Updates values
      ...
      if (
        ...
        this.editor.exportToCanvas(
          mxUtils.bind(this, function (canvas, svgRoot) {
            try {
              this.spinner.stop() // KNOWN: SVG and delayed content currently not supported
   ...
   */ EditorUi.prototype.pasteXml = function (
    ...
    if (compat || this.isCompatibleString(xml)) {
      ...
      if (graph.isMouseInsertPoint()) {
        dx = 0 // No offset for insert at mouse position
   ...
   */ EditorUi.prototype.pasteCells = function (
    ...
    if (!mxEvent.isConsumed(evt)) {
      ...
      /*else if (spans != null && spans.length > 0 && spans[0].hasAttribute('data-meta')
        ...
        var compat = false // Workaround for junk after XML in VM
        try {
      ...
      for (var i = 0; i < elts.length; i++) {
        ...
        mxEvent.addListener(
          ...
          mxUtils.bind(this, function (evt) {
            ...
            if (this.editor.graph.isEnabled() || urlParams['embed'] != '1') {
              if (evt.dataTransfer.files.length > 0) {
                this.hideDialog() // Never open files in embed mode
                if (urlParams['embed'] == '1') {
                  this.importFiles(
                    evt.dataTransfer.files,
                    0,
                ...
                var data = this.extractGraphModelFromEvent(evt) // Tries additional and async parsing of text content such as HTML, Gliffy data
                if (data == null) {
                  var provider =
                  ...
                  if (provider != null) {
                    if (
                      ...
                      if (data != null && data.length > 0) {
                        var div = document.createElement('div')
                        div.innerHTML = Graph.sanitizeHtml(data) // Extracts single image
                        var imgs = div.getElementsByTagName('img')
                        if (imgs.length > 0) {
  ...
  var dropCallback = null //** 设置拖曳停靠回调 */
  EditorUi.prototype.setDropCallback = function (action) {
    dropCallback = action
  }
  /**
   ...
   */ EditorUi.prototype.openFileHandle = function (
    ...
    if (name != null && name.length > 0) {
      ...
      if (
        ...
        this.spinner.stop() // Creates new temporary file if library is dropped in splash screen
        if (this.getCurrentFile() == null && urlParams['embed'] != '1') {
          this.openLocalFile(this.emptyDiagramXml, this.defaultFilename, temp)
        }
        try {
   ...
   */ EditorUi.prototype.addBasenamesForCell = function (cell, basenames) {
    ...
    addName(mxStencilRegistry.getBasenameForStencil(shape)) // Adds package names for markers in edges
      ...
      if (
        ...
        this.installMessageHandler(
          mxUtils.bind(this, function (xml, evt, modified, convertToSketch) {
            ...
            this.setFileData(xml) // TODO: Check if cellsInserted should be fired instead here
            if (convertToSketch) {
              try {
                ...
                var cells = graph.model.cells //Add sketch style and font to all cells
                for (var id in cells) {
                  var cell = cells[id]
            ...
            this.updateUi() // Workaround for no initial focus in FF
   ...
   */ EditorUi.prototype.installMessageHandler = function (fn) {
    ...
    this.editor.graph.model.addListener(mxEvent.CHANGE, updateStatus) // Receives XML message from opener and puts it into the graph
    mxEvent.addListener(
      window,
      'message',
      mxUtils.bind(this, function (evt) {
            ...
            if (data == null) {
              ...
              var dlg = new NewDialog(
                ...
                mxUtils.bind(this, function (xml, name, url, libs) {
                  xml = xml || this.emptyDiagramXml // LATER: Add autosave option in template message
                  if (data.callback != null) {
                    ...
                    fn(xml, evt, xml != this.emptyDiagramXml, data.toSketch) // Workaround for status updated before modified applied
                    if (!this.editor.modified) {
                      this.editor.setStatus('')
                    }
                  }
              ...
              if (data.format == 'png' || data.format == 'xmlpng') {
                ...
                var graphReady = mxUtils.bind(this, function () {
                  var msg = this.createLoadMessage('export') // Attaches incoming message
                  msg.message = data // Forces new HTML format if pages exists
                  if (
                    data.format == 'html2' ||
                    (data.format == 'html' &&
                      (urlParams['pages'] != '0' ||
              ...
              if (data.rect != null) {
                ...
                this.diagramContainer.style.top =
                  Math.max(40, data.rect.top) + 'px' // Inline min width and height
                this.minInlineWidth = data.minWidth
                this.minInlineHeight = data.minHeight
                this.diagramContainer.style.border = '2px solid #295fcc'
                this.diagramContainer.style.bottom = ''
                this.diagramContainer.style.right = '' // Data is extracted diagram in async code
                ...
                afterLoad = mxUtils.bind(this, function () {
                  ...
                  window.setTimeout(
                    mxUtils.bind(this, function () {
                      ...
                      graph.container.focus() // Moves format window to top of graph
                      if (
                        this.formatWindow != null &&
        ...
        var doLoad = mxUtils.bind(this, function (data, evt) {
          ...
          if (autosave && changeListener == null) {
            ...
            this.editor.graph.model.addListener(mxEvent.CHANGE, changeListener) // Some options trigger autosave
          ...
          if (
            ...
            var resp = this.createLoadMessage('load') // Attaches XML to response
            resp.xml = data
    ...
    parent.postMessage(msg, '*') // Adds JSON event for opening links
    if (urlParams['proto'] == 'json') {
   ...
   */ EditorUi.prototype.doImportCsv = function (text, done) {
    try {
      ...
      if (lines.length > 0) {
        ...
        var padding = 0 // Delayed after optional layout
        ...
        var x0 = pt.x
        var y0 = pt.y
        var y = y0 // Default label value depends on column names
        var label = null // Default layout to run.
        var layout = 'auto' // Name of the attribute that contains the parent reference
        var parent = null // Name of the attribute that contains the references for creating edges
        var edges = [] // Name of the column for hyperlinks
        var link = null // String array of names to remove from metadata
        var ignore = null // Read processing instructions first
        var index = 0
        ...
        try {
          for (var i = 0; i < arrays.length; i++) {
            ...
            if (id != null) {
              cell = graph.model.getCell(id) // Bypasses update of cells inserted during this run
            ...
            if (link != null && link != 'link') {
              graph.setLinkForCell(cell, cell.getAttribute(link)) // Removes attribute
          ...
          for (var e = 0; e < edges.length; e++) {
            ...
            for (var i = 0; i < allCells.length; i++) {
              ...
              insertEdge(cell, cell, edge) // Checks more entries
          ...
          if (cells.length > 0) {
            ...
            if (layout.charAt(0) == '[') {
              ...
              var circleLayoutIsVertexIgnored = circleLayout.isVertexIgnored // Ignore other cells
              ...
              this.executeLayout(
                function () {
                  flowLayout.execute(graph.getDefaultParent(), select) // Workaround for flow layout moving cells to origin
              ...
              var orgChartLayoutIsVertexIgnored = orgChartLayout.isVertexIgnored // Ignore other cells
              ...
              var organicLayoutIsVertexIgnored = organicLayout.isVertexIgnored // Ignore other cells
   ...
   */ EditorUi.prototype.updateUi = function () {
    this.updateButtonContainer()
    this.updateActionStates() // Action states that only need update for new files
    var file = this.getCurrentFile()
    var active =
      file != null ||
    ...
    this.menus.get('embed').setEnabled(!restricted) // Disables libraries and extras menu in embed mode
    ...
    this.menus.get('arrange').setEnabled(editable) // Disables connection drop downs in toolbar
  ...
  EditorUi.prototype.handleRemoteInvoke = function (msg, origin) {
    ...
    try {
      ...
      if (functionInfo != null && typeof this[funtionName] === 'function') {
        ...
        var functionArgs = msg.functionArgs //Confirm functionArgs are not null and is array, otherwise, discard it
                      ...
                      try {
                        this.setDatabaseItem(
                          ...
                          [
                            ...
                            { title: file.title, data: file.data },
                          ],
                          next,
                          next /* Ignore errors */,
                          ['filesInfo', 'files']
   ...
   */ EditorUi.prototype.getDatabaseItems = function (
    success,
    error,
    storeName
  ) {
 ...
 */ var CommentsWindow = function (editorUi, x, y, w, h, saveCallback) {
  ...
  refresh()
  this.refreshComments = refresh //Refresh the modified date of each comment if the window is visible
