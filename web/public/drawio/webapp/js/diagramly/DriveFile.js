...
DriveFile.prototype.isRestricted = function () {
  return DrawioFile.RESTRICT_EXPORT || this.desc.userPermission != null && this.desc.labels != null && this.desc.userPermission.role == 'reader' && this.desc.labels.restricted;
};
...
DriveFile.prototype.getFolderUrl = function () {
  if (this.desc.labels != null && this.desc.labels.trashed) {
    ...
    return this.desc.parents != null && this.desc.parents.length > 0 ? 'https://drive.google.com/drive/folders/' + this.desc.parents[0].id : null;
  }
};
  ...
  this.ui.drive.executeRequest({
    ...
    if (resp != null && resp.items != null) {
      for (var i = 0; i < resp.items.length; i++) {
        if (resp.items[i].id === 'anyoneWithLink' || resp.items[i].id === 'anyone') {
            ...
            this.ui.drive.saveFile(this, realRevision, mxUtils.bind(this, function (resp, savedData, pages, checksum) {
              try {
                ...
                if (resp != false) {
                  ...
                  this.autosaveDelay = Math.round(Math.min(10000, Math.max(DriveFile.prototype.autosaveDelay, this.saveDelay)));
                  this.desc = resp;
...
DriveFile.prototype.setRealtimeEnabled = function (value, success, error) {
  if (this.sync != null) {
    this.ui.drive.executeRequest({
      ...
      'params': {
        'key': 'collaboration',
        'value': value ? 'enabled' : urlParams['fast-sync'] != '0' ? 'disabled' : ''
      }
    }, mxUtils.bind(this, function () {
...
DriveFile.prototype.isRealtimeEnabled = function () {
  ...
  return DrawioFile.prototype.isRealtimeEnabled.apply(this, arguments) && collab != 'disabled' || Editor.enableRealtime && collab == 'enabled';
};
    ...
    for (var i = 0; i < resp.items.length; i++) {
      mxUtils.bind(this, function (item) {
        ...
        item.getUrl = mxUtils.bind(this, function (page) {
          return this.ui.getUrl(window.location.pathname + '?rev=' + item.id + '&chrome=0&nav=1&layers=1&edit=_blank' + (page != null ? '&page=' + page : '')) + window.location.hash;
        });
      })(resp.items[i]);
    }
    success(resp.items);
...
DriveFile.prototype.getComments = function (success, error) {
  ...
  function driveCommentToDrawio(file, gComment, pCommentId) {
    ...
    var comment = new DriveComment(file, gComment.commentId || gComment.replyId, gComment.content, gComment.modifiedDate, gComment.createdDate, gComment.status == 'resolved', gComment.author.isAuthenticatedUser ? currentUser : new DrawioUser(gComment.author.permissionId, gComment.author.emailAddress, gComment.author.displayName, gComment.author.picture.url), pCommentId);
