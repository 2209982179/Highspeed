...
(function () {
      ...
      var auth = mxUtils.bind(this, function () {
        ...
        if (authRemembered != null) {
          ...
          this.ui.showAuthDialog(this, true, mxUtils.bind(this, function (remember, authSuccess) {
            var win = window.open('https://www.dropbox.com/oauth2/authorize?client_id=' + this.clientId + (remember ? '&token_access_type=offline' : '') + '&redirect_uri=' + encodeURIComponent(this.redirectUri) + '&response_type=code&state=' + encodeURIComponent('cId=' + this.clientId +
  ...
  DropboxClient.prototype.executePromise = function (promiseFn, success, error) {
    var doExecute = mxUtils.bind(this, function (failOnAuth) {
      ...
      promise['catch'](mxUtils.bind(this, function (err) {
        window.clearTimeout(timeoutThread);
        if (acceptResponse) {
          if (err != null && (err.status == 500 || err.status == 400 || err.status == 401)) {
            this.setUser(null);
            this.client.setAccessToken(null);
            _token = null;
            if (!failOnAuth) {
            ...
            error({
              message: mxResources.get('error') + ' ' + err.status
            });
          }
        }
      }));
  ...
  DropboxClient.prototype.getFile = function (path, success, error, asLibrary) {
    ...
    if (/^https:\/\//i.test(path) || /\.v(dx|sdx?)$/i.test(path) || /\.gliffy$/i.test(path) || /\.pdf$/i.test(path) || !this.ui.useCanvasForExport && binary) {
      ...
      this.readFile(arg, mxUtils.bind(this, function (data, response) {
        ...
        success(file != null ? file : asLibrary ? new DropboxLibrary(this.ui, data, response) : new DropboxFile(this.ui, data, response));
      }), error, binary);
    }
  };
  ...
  DropboxClient.prototype.readFile = function (arg, success, error, binary) {
    var doExecute = mxUtils.bind(this, function (failOnAuth) {
      ...
      var checkPromise = this.client.filesGetMetadata({
        path: '/' + arg.path.substring(1),
  ...
  DropboxClient.prototype.saveFile = function (filename, data, success, error, folder) {
    if (/[\\\/:\?\*"\|]/.test(filename)) {
      ...
      error({
        message: mxResources.get('drawingTooLarge') + ' (' + this.ui.formatFileSize(data.length) + ' / 150 MB)'
      });
    } else {
      ...
      success: mxUtils.bind(this, function (files) {
        if (this.ui.spinner.spin(document.body, mxResources.get('loading'))) {
          ...
          if (tmp > 0) {
            ...
            this.readFile({
              path: rel
            }, mxUtils.bind(this, function (data, stat) {
        ...
        success: mxUtils.bind(this, function (files) {
          if (this.ui.spinner.spin(document.body, mxResources.get('loading'))) {
            ...
            if (readOnly) {
              ...
              if (/\.vsdx$/i.test(files[0].name) || /\.gliffy$/i.test(files[0].name) || !this.ui.useCanvasForExport && binary) {
