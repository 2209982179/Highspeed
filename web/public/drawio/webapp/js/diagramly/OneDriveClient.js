...
(function () {
   ...
   */
  OneDriveClient.prototype.clientId = window.DRAWIO_MSGRAPH_CLIENT_ID || (window.location.hostname == 'test.draw.io' ? '95e4b4ed-ed5c-4a05-935b-b411b4562ef2' : '24b129a6-117b-4394-bdc8-3b9955e5cdef');
  OneDriveClient.prototype.clientId = window.location.hostname == 'app.diagrams.net' ? 'b5ff67d6-3155-4fca-965a-59a3655c4476' : OneDriveClient.prototype.clientId;
  OneDriveClient.prototype.clientId = window.location.hostname == 'viewer.diagrams.net' ? '417a451a-a343-4788-b6c1-901e63182565' : OneDriveClient.prototype.clientId;
  /**
   ...
   */
  OneDriveClient.prototype.rootId = {
    id: 'root',
    name: 'root',
    parentReference: {
      driveId: 'me'
   ...
   */
  OneDriveClient.prototype.invalidFilenameRegExs = [/[~"#%\*:<>\?\/\\{\|}]/, /^\.lock$/i, /^CON$/i, /^PRN$/i, /^AUX$/i, /^NUL$/i, /^COM\d$/i, /^LPT\d$/i, /^desktop\.ini$/i, /_vti_/i];
      ...
      var auth = mxUtils.bind(this, function () {
        ...
        if (authInfo != null) {
          ...
          this.ui.showAuthDialog(this, true, mxUtils.bind(this, function (remember, authSuccess) {
            var url = this.authUrl + '/oauth2/v2.0/authorize' + '?client_id=' + this.clientId + '&response_type=code' + '&redirect_uri=' + encodeURIComponent(this.redirectUri) + '&scope=' + encodeURIComponent(this.scopes + (remember ? ' offline_access' : '')) + '&state=' + encodeURIComponent('cId=' + this.clientId + '&domain=' + window.location.host + '&token=' + state); //To identify which app/domain is used
  ...
  OneDriveClient.prototype.getAccountTypeAndEndpoint = function (success, error) {
    this.get(this.baseUrl + '/me/drive/root', mxUtils.bind(this, function (req) {
      ...
      error({
        message: mxResources.get('unknownError') + ' (Code: ' + req.getStatus() + ')'
      });
    }), error);
  };
  ...
  OneDriveClient.prototype.getFile = function (id, success, error, denyConvert, asLibrary) {
    ...
    this.executeRequest(this.getItemURL(id), mxUtils.bind(this, function (req) {
      if (req.getStatus() >= 200 && req.getStatus() <= 299) {
        ...
        if (/\.v(dx|sdx?)$/i.test(meta.name) || /\.gliffy$/i.test(meta.name) || /\.pdf$/i.test(meta.name) || !this.ui.useCanvasForExport && binary) {
          var mimeType = meta.file != null ? meta.file.mimeType : null;
          this.ui.convertFile(meta['@microsoft.graph.downloadUrl'], meta.name, mimeType, this.extension, success, error);
        } else {
          var acceptResponse = true;
          var timeoutThread = window.setTimeout(mxUtils.bind(this, function () {
            acceptResponse = false;
          ...
          }), binary || meta.file != null && meta.file.mimeType != null && (meta.file.mimeType.substring(0, 6) == 'image/' && meta.file.mimeType.substring(0, 9) != 'image/svg' || meta.file.mimeType == 'application/pdf'));
        }
      } else {
        if (this.isExtAuth) {
          error({
  ...
  OneDriveClient.prototype.renameFile = function (file, filename, success, error) {
    if (file != null && filename != null) {
      if (!this.isValidFilename(filename)) {
        error({
          message: this.invalidFilenameRegExs[0].test(filename) ? mxResources.get('oneDriveCharsNotAllowed') : mxResources.get('oneDriveInvalidDeviceName')
        });
        return;
      }
      ...
      this.checkExists(file.getParentId(), filename, false, mxUtils.bind(this, function (checked) {
        if (checked) {
          this.writeFile(this.getItemURL(file.getId()), JSON.stringify({
            name: filename
          }), 'PATCH', 'application/json', success, error);
        } else {
          error();
        }
      }));
  ...
  OneDriveClient.prototype.moveFile = function (id, folderId, success, error) {
    ...
    if (folderInfo.driveId != fileInfo.driveId) {
      ...
      this.writeFile(this.getItemURL(id), JSON.stringify({
        parentReference: folderInfo
      }), 'PATCH', 'application/json', success, error);
    }
  };
      ...
      if (data != null) {
        ...
        var uploadPart = mxUtils.bind(this, function (uploadUrl, index, byteIndex, retryCount) {
          try {
            ...
            req.setRequestHeaders = mxUtils.bind(this, function (request, params) {
              request.setRequestHeader('Content-Range', 'bytes ' + byteIndex + '-' + (byteIndex + partByteLength - 1) + '/' + dataByteLength);
            });
            req.send(mxUtils.bind(this, function (req) {
              window.clearTimeout(timeoutThread);
              if (acceptResponse) {
                ...
                } else if (status >= 500 && status <= 599 && retryCount < 2)
  ...
  OneDriveClient.prototype.pickFolder = function (fn, direct) {
    ...
    var odSaveDlg = mxUtils.bind(this, function (direct) {
      var openSaveDlg = this.inlinePicker ? this.createInlinePicker(fn, true) : mxUtils.bind(this, function () {
        OneDrive.save({
          ...
          advanced: {
            ...
            //IE11 doen't work with our modified version, so, setting endpointHint disable using our token BUT will force relogin!
  ...
  OneDriveClient.prototype.pickFile = function (fn, acceptAllFiles) {
    ...
    var odOpenDlg = this.inlinePicker ? this.createInlinePicker(fn, null, acceptAllFiles) : mxUtils.bind(this, function () {
      OneDrive.open({
        clientId: this.clientId,
        action: 'query',
        multiSelect: false,
        advanced: {
          ...
          //We can also get @microsoft.graph.downloadUrl within this request but it will break the normal process
          'accessToken': _token,
          isConsumerAccount: false
        },
        success: mxUtils.bind(this, function (files) {
