...
(function () {
      ...
      var auth = mxUtils.bind(this, function () {
        ...
        if (authRemembered != null) {
          ...
          this.ui.showAuthDialog(this, true, mxUtils.bind(this, function (remember, authSuccess) {
            var win = window.open(DRAWIO_GITLAB_URL + '/oauth/authorize?client_id=' + this.clientId + '&scope=' + this.scope + '&redirect_uri=' + encodeURIComponent(this.redirectUri) + '&response_type=code&state=' + encodeURIComponent('cId=' + this.clientId +
    ...
    if (knownRefPos != null) {
      ...
      var checkUrl = mxUtils.bind(this, function () {
        if (refPos < 2) {
          ...
          var url = this.baseUrl + '/projects/' + encodeURIComponent(org + '/' + repo) + '/repository/' + (!isFolder ? 'files/' + encodeURIComponent(path) + '?ref=' + ref : checkRepo ? 'branches?per_page=1&page=1&ref=' + ref : 'tree?path=' + path + '&ref=' + ref);
    ...
    this.getRefIndex(path.split('/'), false, mxUtils.bind(this, function (tokens, refPos) {
      ...
      if (!checkExists && (/\.v(dx|sdx?)$/i.test(path) || /\.gliffy$/i.test(path) || /\.pdf$/i.test(path) || !this.ui.useCanvasForExport && binary)) {
        ...
        if (_token != null) {
          ...
          var url = this.baseUrl + '/projects/' + encodeURIComponent(org + '/' + repo) + '/repository/files/' + encodeURIComponent(path) + '?ref=' + ref;
        ...
        // Adds random parameter to bypass cache
        var rnd = '&t=' + new Date().getTime();
        url = this.baseUrl + '/projects/' + encodeURIComponent(org + '/' + repo) + '/repository/files/' + encodeURIComponent(path) + '?ref=' + ref;
            ...
            this.showCommitDialog(filename, true, mxUtils.bind(this, function (message) {
              this.writeFile(org, repo, ref, path, message, data, last_commit_id, mxUtils.bind(this, function (req) {
                try {
                  ...
                  success(this.createGitLabFile(org, repo, ref, msg.content != null ? msg.content : msg, asLibrary, refPos));
                } catch (e) {
                  error(e);
                }
              }), error);
  ...
  GitLabClient.prototype.writeFile = function (org, repo, ref, path, message, data, last_commit_id, success, error) {
    if (data.length >= this.maxFileSize) {
      error({
        message: mxResources.get('drawingTooLarge') + ' (' + this.ui.formatFileSize(data.length) + ' / 10 MB)'
      });
    } else {
      var method = 'POST';
      var entity = {
  ...
  GitLabClient.prototype.showGitLabDialog = function (showFiles, fn, hideNoFilesError) {
    ...
    var selectFile = mxUtils.bind(this, function (page) {
      ...
      var req = new mxXmlRequest(this.baseUrl + '/projects/' + encodeURIComponent(org + '/' + repo) + '/repository/tree?path=' + path + '&ref=' + ref + '&per_page=' + pageSize + '&page=' + page, null, 'GET');
    ...
    var selectRef = mxUtils.bind(this, function (page, auto) {
      ...
      var req = new mxXmlRequest(this.baseUrl + '/projects/' + encodeURIComponent(org + '/' + repo) + '/repository/branches?per_page=' + pageSize + '&page=' + page, null, 'GET');
    ...
    var selectRepo = mxUtils.bind(this, function (page) {
      ...
      listGroups(mxUtils.bind(this, function (groups) {
        if (this.user == null) {
          ...
          spinnerRequestStarted();
          var req = new mxXmlRequest(this.baseUrl + '/users/' + this.user.id + '/projects?per_page=' + pageSize + '&page=' + page, null, 'GET');
          this.executeRequest(req, mxUtils.bind(this, function (req) {
