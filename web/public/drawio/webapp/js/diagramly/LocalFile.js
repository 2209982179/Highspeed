...
LocalFile.prototype.saveFile = function (title, revision, success, error, useCurrentData) {
    ...
    if (this.fileHandle != null) {
      ...
      if (this.ui.isOfflineApp() || this.ui.isLocalFileSave()) {
        ...
        if (data.length < MAX_REQUEST_SIZE) {
          ...
          new mxXmlRequest(SAVE_URL, 'format=' + format + '&xml=' + encodeURIComponent(data) + '&filename=' + encodeURIComponent(title) + (binary ? '&binary=1' : '')).simulate(document, '_blank');
        } else {
          this.ui.handleError({
  ...
  if (binary) {
    ...
    this.ui.getEmbeddedPng(mxUtils.bind(this, function (imageData) {
      doSave(imageData);
    }), error, this.ui.getCurrentFile() != this ? savedData : null, p.scale, p.border);
  } else {
    doSave(savedData);
  }
};
