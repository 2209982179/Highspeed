...
(async function () {
  ...
  PrintDialog.electronPrint = function (editorUi, allPages, pagesFrom, pagesTo, fit, sheetsAcross, sheetsDown, zoom, pageScale, pageFormat) {
    ...
    if (file) {
      file.updateFileData();
      xml = editorUi.getFileData(true, null, null, null, null, false, null, null, null, false, true);
      title = file.title;
    }
    var extras = {
      globalVars: editorUi.editor.graph.getExportVariables()
    };
    ...
    this.actions.addAction('import...', mxUtils.bind(this, async function () {
      if (editorUi.getCurrentFile() != null) {
        ...
        if (paths !== undefined && paths[0] != null) {
          var path = paths[0];
          localStorage.setItem('.lastImpDir', await requestSync({
            action: 'dirname',
            path: path
          }));
      ...
      if (file.fileObject != null) {
        ...
        await requestSync({
          ...
          listener: mxUtils.bind(this, function (curr, prev) {
            EditorUi.debug('EditorUi.watchFile', [this], 'file', [file], 'stat', [file.stat], 'curr', [curr], 'prev', [prev], 'inConflictState', file.inConflictState, 'unwatchedSaves', file.unwatchedSaves);
  ...
  App.prototype.chooseFileEntry = async function (fn) {
    ...
    if (paths !== undefined && paths[0] != null) {
      localStorage.setItem('.lastOpenDir', await requestSync({
        action: 'dirname',
        path: paths[0]
      }));
  ...
  App.prototype.readGraphFile = function (fn, fnErr, path, noDraftCheck) {
    ...
    var checkDrafts = mxUtils.bind(this, function () {
      ...
      electron.request({
        ...
        if (drafts.length > 0) {
          var dlg = new DraftDialog(this, mxResources.get('unsavedChanges'), drafts.length > 1 ? null : drafts[0].data, mxUtils.bind(this, async function (index) {
            index = index || 0;
            this.hideDialog();
  ...
  */
    ...
    if (!this.savingFile) {
      ...
      try {
        if (this.fileObject == null) {
          ...
          if (path != null) {
            localStorage.setItem('.lastSaveDir', await requestSync({
              action: 'dirname',
              path: path
            }));
  ...
  EditorUi.prototype.createDownloadRequest = function (filename, format, ignoreSelection, base64, transparent, currentPage, scale, border, grid, includeXml, pageRange, w, h) {
    var params = this.downloadRequestBuilder(filename, format, ignoreSelection, base64, transparent, currentPage, scale, border, grid, includeXml, pageRange, w, h);
  ...
  EditorUi.prototype.saveData = async function (filename, format, data, mimeType, base64Encoded) {
    ...
    window.setTimeout(mxUtils.bind(this, async function () {
      ...
      if (path != null) {
        localStorage.setItem('.lastExpDir', await requestSync({
          action: 'dirname',
          path: path
        }));
