...
function MiroImporter() {
  ...
  var typeStylesMap = {
    ...
    'stencil': function (vertex, obj, tags, graph) {
      ...
      try {
        ...
        if (texts) {
          vertex.value = texts.caption ? texts.caption.text : texts.name ? texts.name.text : '';
        }
  ...
  function colorNum2Hex(color) {
    return color == -1 ? 'none' : '#' + ('0' + (color >> 16).toString(16)).slice(-2) + ('0' + (color >> 8 & 0xFF).toString(16)).slice(-2) + ('0' + (color & 0xFF).toString(16)).slice(-2);
  }
  ;
  fontNameMap = {
    0: 'Arial',
  ...
  function getTypeStyle(type, obj) {
    var style = '';
    if (type == 'paint') {
      var drawPoints = obj.points,
        w = obj.size.width,
        h = obj.size.height;
      var drawShape = '<shape strokewidth="inherit"><foreground>';
      for (var i = 0; i < drawPoints.length; i++) {
        var p = drawPoints[i];
        drawShape += (i == 0 ? '<path><move' : '<line') + ' x="' + (p.x / w * 100).toFixed(2) + '" y="' + (p.y / h * 100).toFixed(2) + '"/>';
      }
    ...
    try {
      ...
      for (var key in styleMap) {
        ...
        switch (key) {
          ...
          case 'ta':
            //Text Align
            style['align'] = val == 't' ? 'left' : val == 'r' ? 'right' : 'center';
            break;
          case 'tav':
            //Text Vertical Align
            style['verticalAlign'] = val == 't' ? 'top' : val == 'b' ? 'bottom' : 'middle';
            break;
          case 'taw':
            //Text Wrap?
            break;
  ...
  function importVertex(obj, type, tags, graph) {
    try {
      ...
      var scale = obj.scale.scale,
        //TODO Check this is the correct use of the scale
        w = obj.size.width,
        h = obj.size.height,
        x = obj.position.x,
        y = obj.position.y;
      var vertex = new mxCell(obj.text, new mxGeometry(x - w / 2 * scale, y - h / 2 * scale, w * scale, h * scale), '');
      if (typeof style == 'function') {
  ...
  function importEdge(obj, vertexes, tags, graph) {
    try {
      ...
      var pp = obj.primary.point,
        sp = obj.secondary.point;
      ...
      if (lbls.length > 0) {
        ...
        edgeStyle += 'labelBackgroundColor=none;fontSize=' + lbls[0].fontSize + ';fontColor=' + colorNum2Hex(lbls[0].color) + ';';
      }
  ...
  this.importMiroJson = function (data) {
    try {
      ...
      for (var i = 0; i < objects.length; i++) {
        var o = objects[i];
        if (o.type == 13)
          //Tags (assuming they are always at the top)
          {
            tags.push({
              color: colorNum2Hex(o.color),
              text: o.text
            });
          } else if (o.type == 14) {
