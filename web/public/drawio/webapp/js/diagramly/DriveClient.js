...
(function () {
  ...
  window.DriveClient = function (editorUi, isExtAuth) {
    ...
    if (this.ui.editor.chromeless && !this.ui.editor.editable && urlParams['rt'] != '1' && urlParams['extAuth'] != '1') {
      ...
      this.scopes = ['https://www.googleapis.com/auth/drive.readonly', 'https://www.googleapis.com/auth/userinfo.profile'];
    } else {
   ...
   */
  DriveClient.prototype.scopes = ['https://www.googleapis.com/auth/drive.file', 'https://www.googleapis.com/auth/drive.install', 'https://www.googleapis.com/auth/userinfo.profile'];
      ...
      this.ui.showAuthDialog(this, true, mxUtils.bind(this, function (remember, success) {
        this.authorize(false, mxUtils.bind(this, function () {
          ...
          if (resp != null && resp.error != null) {
            if (resp.error.code == 403 && resp.error.data != null && resp.error.data.length > 0 && resp.error.data[0].reason == 'domainPolicy') {
            ...
            try {
              ...
              if (acceptResponse) {
                ...
                if (req.getStatus() >= 200 && req.getStatus() <= 299) {
                  ...
                  var data = resp != null && resp.error != null ? resp.error.data != null ? resp.error.data : resp.error.errors : null;
                  ...
                  if (error != null && resp != null && resp.error != null && (resp.error.code == -1 || resp.error.code == 403 && (reason == 'domainPolicy' || resp.error.message == 'The requested mime type change is forbidden.'))) {
                    error(resp);
                  }
                  // Handles authentication error
                  else if (resp != null && resp.error != null && (resp.error.code == 401 || resp.error.code == 403 && reason != 'rateLimitExceeded')) {
                    // Shows an error if re-authenticated but the server still doesn't allow it
                    if (resp.error.code == 403 && this.retryAuth || resp.error.code == 401 && this.retryAuth && reason == 'authError') {
                      if (error != null) {
                        error(resp);
                      }
                  ...
                  else if (resp != null && resp.error != null && resp.error.code != 412 && resp.error.code != 404 && resp.error.code != 400 && this.currentRequest == reqObj && retryCount < this.maxRetries) {
                    retryCount++;
                    var jitter = 1 + 0.1 * (Math.random() - 0.5);
                    this.requestThread = window.setTimeout(fn, Math.round(Math.pow(2, retryCount) * jitter * this.coolOff));
                  } else if (error != null) {
                    error(resp);
                  }
                }
  ...
  DriveClient.prototype.authorizeStep2 = function (state, immediate, success, error, remember, popup) {
    try {
      ...
      if (immediate && this.userId == null) {
        ...
        if (immediate)
          //Note, we checked refresh token is not null above
          {
            //state is used to identify which app/domain is used
            var req = new mxXmlRequest(this.redirectUri + '?state=' + encodeURIComponent('cId=' + this.clientId + '&domain=' + window.location.host + '&token=' + state) + '&userId=' + this.userId, null, 'GET');
            req.send(mxUtils.bind(this, function (req) {
              try {
                if (req.getStatus() >= 200 && req.getStatus() <= 299) {
          ...
          var url = 'https://accounts.google.com/o/oauth2/v2/auth?client_id=' + this.clientId + '&redirect_uri=' + encodeURIComponent(this.redirectUri) + '&response_type=code&include_granted_scopes=true' + (remember ? '&access_type=offline&prompt=consent%20select_account' : '') +
          //Ask for consent again to get a new refresh token
          '&scope=' + encodeURIComponent(this.scopes.join(' ')) + '&state=' + encodeURIComponent('cId=' + this.clientId + '&domain=' + window.location.host + '&token=' + state + (
              ...
              try {
                if (newAuthInfo == null) {
                  if (error != null) {
                    ...
                    }); //TODO Check this error handling is correct
                  }
                } else {
  ...
  DriveClient.prototype.copyFile = function (id, title, success, error) {
    if (id != null && title != null) {
      this.executeRequest({
        url: '/files/' + id + '/copy?fields=' + encodeURIComponent(this.allFields) + '&supportsAllDrives=true&enforceSingleParent=true',
        //&alt=json
        method: 'POST',
        params: {
          'title': title,
  ...
  DriveClient.prototype.listFiles = function (searchStr, afterDate, mineOnly, success, error) {
    this.executeRequest({
      url: '/files?supportsAllDrives=true&includeItemsFromAllDrives=true&q=' + encodeURIComponent('(mimeType contains \'' + this.xmlMimeType + '\') ' + (searchStr ? ' and (title contains \'' + searchStr + '\')' : '') + (afterDate ? ' and (modifiedDate > \'' + afterDate.toISOString() + '\')' : '') + (mineOnly ? ' and (\'me\' in owners)' : '')) + '&orderBy=modifiedDate desc,title'
    }, success, error);
  };
      ...
      this.loadDescriptor(id, mxUtils.bind(this, function (resp) {
        try {
          if (this.user != null) {
            ...
            if (/\.v(dx|sdx?)$/i.test(resp.title) || /\.gliffy$/i.test(resp.title) || !this.ui.useCanvasForExport && binary) {
              ...
              if (readXml || readLibrary || resp.mimeType == this.libraryMimeType || resp.mimeType == this.xmlMimeType) {
    ...
    try {
      ...
      if (url == null) {
        ...
        var fn = mxUtils.bind(this, function () {
                  ...
                  if (index > 0) {
                    ...
                    if (xml != null && xml.length > 0) {
                      ...
                      try {
                        ...
                        var temp = window.atob && !mxClient.IS_IE && !mxClient.IS_IE11 ? atob(xml) : Base64.decode(xml);
                        var node = this.ui.editor.extractGraphModel(mxUtils.parseXml(temp).documentElement, true);
          ...
          }), resp.mimeType != null && resp.mimeType.substring(0, 6) == 'image/' && resp.mimeType.substring(0, 9) != 'image/svg' || /\.png$/i.test(resp.title) || /\.jpe?g$/i.test(resp.title) || /\.pdf$/i.test(resp.title), null, null, null, headers);
        });
        fn();
      }
    } catch (e) {
  ...
  DriveClient.prototype.saveFile = function (file, revision, success, errFn, noCheck, unloading, overwrite, properties, secret) {
    try {
      ...
      var error = mxUtils.bind(this, function (e) {
        ...
        try {
          if (!file.isConflict(e)) {
            var err = 'sl_' + file.saveLevel + '-error_' + (file.getErrorMessage(e) || 'unknown');
            if (e != null && e.error != null && e.error.code != null) {
              err += '-code_' + e.error.code;
            }
            EditorUi.logEvent({
              category: 'ERROR-SAVE-FILE-' + file.getHash() + '-rev_' + file.desc.headRevisionId + '-mod_' + file.desc.modifiedDate + '-size_' + file.getSize() + '-mime_' + file.desc.mimeType + (this.ui.editor.autosave ? '' : '-nosave') + (file.isAutosave() ? '' : '-noauto') + (file.changeListenerEnabled ? '' : '-nolisten') + (file.inConflictState ? '-conflict' : '') + (file.invalidChecksum ? '-invalid' : ''),
              action: err,
              label: (this.user != null ? 'user_' + this.user.id : 'nouser') + (file.sync != null ? '-client_' + file.sync.clientId : '-nosync')
            });
          }
        } catch (ex) {
          // ignore
      ...
      if (file.isEditable() && file.desc != null) {
        ...
        else if (meta.mimeType == 'application/octet-stream' || urlParams['override-mime'] == '1' && meta.mimeType != this.xmlMimeType) {
          meta.mimeType = this.xmlMimeType;
        }
          ...
          try {
                ...
                if (prevDesc != null) {
                  ...
                  try {
                    EditorUi.logEvent({
                      ...
                      action: 'from_' + prevDesc.id + '.' + prevDesc.headRevisionId + '-to_' + file.desc.id + '.' + file.desc.headRevisionId,
                      label: this.user != null ? 'user_' + this.user.id : 'nouser' + (file.sync != null ? '-client_' + file.sync.clientId : 'nosync')
                    });
                  } catch (e) {
                    // ignore
            ...
            var doExecuteRequest = mxUtils.bind(this, function (data, binary) {
              ...
              try {
                ...
                var etag = !overwrite && file.constructor == DriveFile && (DrawioFile.SYNC == 'manual' || DrawioFile.SYNC == 'auto') ? file.getCurrentEtag() : null;
                var doExecuteSave = mxUtils.bind(this, function (realOverwrite) {
                  file.saveLevel = 5;
                  try {
                    var unknown = file.desc.mimeType != this.xmlMimeType && file.desc.mimeType != this.mimeType && file.desc.mimeType != this.libraryMimeType;
                    var acceptResponse = true;
                    var timeoutThread = null;
                    ...
                    this.executeRequest(this.createUploadRequest(file.getId(), meta, data, revision || realOverwrite || unknown, binary, realOverwrite ? null : etag, pinned), mxUtils.bind(this, function (resp) {
                      window.clearTimeout(timeoutThread);
                      if (acceptResponse) {
                        ...
                        if (delta <= 0 || etag0 == resp.etag || revision && head0 == resp.headRevisionId) {
                          ...
                          if (retryCount < this.staleEtagMaxRetries) {
                            ...
                            if (urlParams['test'] == '1') {
                              EditorUi.debug('DriveClient: Invalid response', 'retry', retryCount, 'delay', delay, 'rev', resp != null ? resp.headRevisionId : 'null', 'errors', temp);
                            }
                          } else {
                            file.saveLevel = 12;
                            error({
                              message: mxResources.get('error') + ': ' + temp
                            }, resp);

                            // Logs failed save
                            try {
                              EditorUi.logError('Critical: Saving to Google Drive failed ' + file.desc.id, null, 'from-' + head0 + '.' + mod0 + '-' + this.ui.hashValue(etag0) + '-to-' + resp.headRevisionId + '.' + resp.modifiedDate + '-' + this.ui.hashValue(resp.etag) + (temp.length > 0 ? '-errors-' + temp : ''), 'user-' + (this.user != null ? this.user.id : 'nouser') + (file.sync != null ? '-client_' + file.sync.clientId : '-nosync') + '-retries-' + retryCount);
                            } catch (e) {
                              // ignore
                            }
                          }
                              ...
                              try {
                                ...
                                if (resp != null && resp.etag == etag) {
                                  if (retryCount < this.staleEtagMaxRetries) {
                                    ...
                                    if (urlParams['test'] == '1') {
                                      EditorUi.debug('DriveClient: Stale Etag Detected', 'retry', retryCount, 'delay', delay);
                                    }
                                  } else {
                                    executeSave(true);
                                    ...
                                    try {
                                      EditorUi.logEvent({
                                        ...
                                        action: 'rev_' + file.desc.headRevisionId + '-mod_' + file.desc.modifiedDate + '-size_' + file.getSize() + '-mime_' + file.desc.mimeType + (this.ui.editor.autosave ? '' : '-nosave') + (file.isAutosave() ? '' : '-noauto') + (file.changeListenerEnabled ? '' : '-nolisten') + (file.inConflictState ? '-conflict' : '') + (file.invalidChecksum ? '-invalid' : ''),
                                        label: (this.user != null ? 'user_' + this.user.id : 'nouser') + (file.sync != null ? '-client_' + file.sync.clientId : '-nosync')
                                      });
                                    } catch (e) {
                                      // ignore
                                  ...
                                  if (urlParams['test'] == '1' && resp.headRevisionId == head0) {
                                    EditorUi.debug('DriveClient: Remote Etag Changed', 'local', etag, 'remote', resp.etag, 'rev', file.desc.headRevisionId, 'response', [resp], 'file', [file]);
                                  }
                                  error(err, resp);
                                }
                              } catch (e) {
                        ...
                        try {
                          ...
                          if (desc2 != null && desc2.headRevisionId == head0) {
                            if (urlParams['test'] == '1' && etag != desc2.etag) {
                              EditorUi.debug('DriveClient: Preflight Etag Update', 'from', etag, 'to', desc2.etag, 'rev', file.desc.headRevisionId, 'response', [desc2], 'file', [file]);
                            }
                            etag = desc2.etag;
                            doExecuteSave(realOverwrite);
                          } else {
            ...
            if (saveAsPng) {
              ...
              }), error, this.ui.getCurrentFile() != file ? savedData : null, p.scale, p.border);
            } else {
        ...
        try {
          ...
          if (unloading || saveAsPng || file.constructor == DriveLibrary || !this.enableThumbnails || urlParams['thumb'] == '0' || meta.mimeType != null && meta.mimeType.substring(0, 29) != 'application/vnd.jgraph.mxfile' || !this.ui.getThumbnail(this.thumbnailWidth, mxUtils.bind(this, function (canvas) {
  ...
  DriveClient.prototype.createUploadRequest = function (id, metadata, data, revision, binary, etag, pinned) {
    ...
    var reqObj = {
      'fullUrl': 'https://content.googleapis.com/upload/drive/v2/files' + (id != null ? '/' + id : '') + '?uploadType=multipart&supportsAllDrives=true&enforceSingleParent=true&fields=' + this.allFields,
      'method': id != null ? 'PUT' : 'POST',
      'headers': headers,
      'params': delim + 'Content-Type: application/json\r\n\r\n' + JSON.stringify(metadata) + delim + 'Content-Type: ' + ctype + '\r\n' + 'Content-Transfer-Encoding: base64\r\n' + '\r\n' + (data != null ? binary ? data : window.btoa && !mxClient.IS_IE && !mxClient.IS_IE11 ? Graph.base64EncodeUnicode(data) : Base64.encode(data) : '') + close
    };
    if (!revision) {
  ...
  DriveClient.prototype.createLinkPicker = function () {
    ...
    if (picker == null || pickers[name + 'Token'] != _token) {
      ...
      var view = new google.picker.DocsView(google.picker.ViewId.FOLDERS).setParent('root').setIncludeFolders(true).setSelectFolderEnabled(true);
      var view2 = new google.picker.DocsView().setIncludeFolders(true).setSelectFolderEnabled(true);
      var view21 = new google.picker.DocsView().setIncludeFolders(true).setEnableDrives(true).setSelectFolderEnabled(true);
      picker = new google.picker.PickerBuilder().setAppId(this.appId).setLocale(mxLanguage).setOAuthToken(pickers[name + 'Token']).enableFeature(google.picker.Feature.SUPPORT_DRIVES).addView(view).addView(view2).addView(view21).addView(google.picker.ViewId.RECENTLY_PICKED);
    }
    return picker;
  };
      ...
      this.execute(mxUtils.bind(this, function () {
        try {
          ...
          if (pickers[name] == null || pickers[name + 'Token'] != _token) {
            ...
            var view = new google.picker.DocsView(google.picker.ViewId.FOLDERS).setParent('root').setIncludeFolders(true);
            var view2 = new google.picker.DocsView().setIncludeFolders(true);
            var view3 = new google.picker.DocsView().setEnableDrives(true).setIncludeFolders(true);
            var view4 = new google.picker.DocsUploadView().setIncludeFolders(true);
            if (!acceptAllFiles) {
            ...
            pickers[name] = new google.picker.PickerBuilder().setOAuthToken(pickers[name + 'Token']).setLocale(mxLanguage).setAppId(this.appId).enableFeature(google.picker.Feature.SUPPORT_DRIVES).addView(view).addView(view2).addView(view3).addView(google.picker.ViewId.RECENTLY_PICKED).addView(view4);
          ...
          this.execute(mxUtils.bind(this, function () {
            try {
              ...
              if (pickers[name] == null || pickers[name + 'Token'] != _token) {
                ...
                // Pseudo-hierarchical directory view, see
                // https://groups.google.com/forum/#!topic/google-picker-api/FSFcuJe7icQ
                var view = new google.picker.DocsView(google.picker.ViewId.FOLDERS).setParent('root').setIncludeFolders(true).setSelectFolderEnabled(true).setMimeTypes('application/vnd.google-apps.folder');
                var view2 = new google.picker.DocsView().setIncludeFolders(true).setSelectFolderEnabled(true).setMimeTypes('application/vnd.google-apps.folder');
                var view3 = new google.picker.DocsView().setIncludeFolders(true).setEnableDrives(true).setSelectFolderEnabled(true).setMimeTypes('application/vnd.google-apps.folder');
                pickers[name] = new google.picker.PickerBuilder().setSelectableMimeTypes('application/vnd.google-apps.folder').setOAuthToken(pickers[name + 'Token']).setLocale(mxLanguage).setAppId(this.appId).enableFeature(google.picker.Feature.SUPPORT_DRIVES).addView(view).addView(view2).addView(view3).addView(google.picker.ViewId.RECENTLY_PICKED).setTitle(mxResources.get('pickFolder'));
                if (urlParams['gPickerSize']) {
                  var cSize = urlParams['gPickerSize'].split(',');
                  pickers[name] = pickers[name].setSize(cSize[0], cSize[1]);
                }
      ...
      this.execute(mxUtils.bind(this, function () {
        try {
          ...
          if (pickers.libraryPicker == null || pickers.libraryPickerToken != _token) {
            ...
            // Pseudo-hierarchical directory view, see
            // https://groups.google.com/forum/#!topic/google-picker-api/FSFcuJe7icQ
            var view = new google.picker.DocsView(google.picker.ViewId.FOLDERS).setParent('root').setIncludeFolders(true).setMimeTypes(this.libraryMimeType + ',application/xml,text/plain,application/octet-stream');
            var view2 = new google.picker.DocsView().setIncludeFolders(true).setMimeTypes(this.libraryMimeType + ',application/xml,text/plain,application/octet-stream');
            var view3 = new google.picker.DocsView().setEnableDrives(true).setIncludeFolders(true).setMimeTypes(this.libraryMimeType + ',application/xml,text/plain,application/octet-stream');
            var view4 = new google.picker.DocsUploadView().setIncludeFolders(true);
            pickers.libraryPicker = new google.picker.PickerBuilder().setOAuthToken(pickers.libraryPickerToken).setLocale(mxLanguage).setAppId(this.appId).enableFeature(google.picker.Feature.SUPPORT_DRIVES).addView(view).addView(view2).addView(view3).addView(google.picker.ViewId.RECENTLY_PICKED).addView(view4);
            if (urlParams['gPickerSize']) {
              var cSize = urlParams['gPickerSize'].split(',');
            ...
            this.sharingObserver = new MutationObserver(mxUtils.bind(this, function (mutations) {
              ...
              for (var i = 0; i < mutations.length; i++) {
                for (var j = 0; j < mutations[i].addedNodes.length; j++) {
                  ...
                  if (child.nodeName == 'BUTTON' && child.getAttribute('name') == 'ok' && child.parentNode != null && child.parentNode.parentNode != null && child.parentNode.parentNode.getAttribute('role') == 'dialog') {
