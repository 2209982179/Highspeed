...
EditorUi.initMinimalTheme = function () {
  ...
  function toggleFormat(ui, visible) {
    if (EditorUi.windowed) {
      ...
      if (ui.formatWindow == null) {
        ...
        ui.formatWindow = new WrapperWindow(ui, mxResources.get('format'), x, y, 240, h, function (container) {
        ...
        ui.formatWindow.window.setVisible(visible != null ? visible : !ui.formatWindow.window.isVisible());
      }
    } else {
  ...
  function createSidebar(ui, container) {
    var css = 'position:absolute;border-width:1px;cusor:pointer;border-style:none;' + 'height:24px;bottom:0px;text-align:center;padding:8px 6px 0 6px;border-top-style:solid;' + 'width:50%;height:32px;box-sizing:border-box;font-size:11px;';
  ...
  function toggleShapes(ui, visible) {
    if (EditorUi.windowed) {
      ...
      if (ui.sidebarWindow == null) {
        ...
        ui.sidebarWindow = new WrapperWindow(ui, mxResources.get('shapes'), 10, 56, w - 6, h - 6, function (container) {
        ...
        ui.sidebarWindow.window.setVisible(visible != null ? visible : !ui.sidebarWindow.window.isVisible());
      }
    } else {
  ...
  mxConstraintHandler.prototype.pointImage = Graph.createSvgImage(5, 5, '<path d="m 0 0 L 5 5 M 0 5 L 5 0" stroke-width="2" style="stroke-opacity:0.4" stroke="#ffffff"/>' + '<path d="m 0 0 L 5 5 M 0 5 L 5 0" stroke="' + fill + '"/>');
  ...
  EditorUi.prototype.toggleFormatPanel = function (visible) {
    if (this.formatWindow != null) {
      this.formatWindow.window.setVisible(visible != null ? visible : !this.formatWindow.window.isVisible());
    } else {
      toggleFormat(this);
    }
  };
  EditorUi.prototype.isFormatPanelVisible = function () {
    return this.formatWindow != null && this.formatWindow.window.isVisible() || this.formatWindow == null && this.formatWidth > 0;
  };
  ...
  Menus.prototype.init = function () {
    ...
    var methods = ['horizontalFlow', 'verticalFlow', '-', 'horizontalTree', 'verticalTree', 'radialTree', '-', 'organic', 'circle'];
  ...
  EditorUi.prototype.installFormatToolbar = function (container) {
    ...
    var div = document.createElement('div');
    div.style.cssText = 'position:absolute;top:10px;z-index:1;border-radius:4px;' + 'box-shadow:0px 0px 3px 1px #d1d1d1;padding:6px;white-space:nowrap;background-color:#fff;' + 'transform:translate(-50%, 0);left:50%;';
  ...
  EditorUi.prototype.init = function () {
    ...
    var libs = Editor.enableCustomLibraries && (urlParams['embed'] != '1' || urlParams['libraries'] == '1');
    var div = document.createElement('div');
    ...
    function addMenuItem(label, fn, small, tooltip, action, img) {
      ...
      // Prevents focus
      mxEvent.addListener(btn, mxClient.IS_POINTER ? 'pointerdown' : 'mousedown', mxUtils.bind(this, function (evt) {
        evt.preventDefault();
      }));
    ...
    ui.buttonContainer.style.cssText = 'display:flex;justify-content:flex-end;padding-right:10px;gap:6px;' + 'white-space:nowrap;background-color:inherit;align-items:center;min-width:0;margin-left:auto;';
    ...
    ui.tabContainer.style.cssText = 'position:absolute;left:0px;right:0px;bottom:0px;height:30px;' + 'white-space:nowrap;visibility:hidden;';
    ...
    if (viewZoomMenu != null) {
      ...
      menubar.style.cssText = 'position:absolute;left:0px;right:0px;top:0px;height:30px;padding:8px;' + 'text-align:left;white-space:nowrap;';
      ...
      ui.setGraphEnabled = function () {
        ...
        if (this.tabContainer != null) {
          ...
          this.diagramContainer.style.bottom = urlParams['pages'] != '0' && this.tabContainer.style.visibility != 'hidden' ? this.tabContainerHeight + 'px' : '0px';
        }
      };
    }
    ...
    function refreshMenu() {
      ...
      createGroup([elt, addMenuItem(mxResources.get('delete'), ui.actions.get('delete').funct, null, mxResources.get('delete'), ui.actions.get('delete'), small ? Editor.trashImage : null)], small ? 60 : null);
      if (iw >= 411) {
        createGroup([undoElt, redoElt], 60);
        if (iw >= 520) {
          createGroup([fitElt, iw >= 640 ? addMenuItem('', zoomInAction.funct, true, mxResources.get('zoomIn') + ' (' + Editor.ctrlKey + ' +)', zoomInAction, Editor.zoomInImage) : null, iw >= 640 ? addMenuItem('', zoomOutAction.funct, true, mxResources.get('zoomOut') + ' (' + Editor.ctrlKey + ' -)', zoomOutAction, Editor.zoomOutImage) : null], 60);
        }
      }
    }
    ;
