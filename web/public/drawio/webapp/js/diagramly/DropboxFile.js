...
DropboxFile.prototype.getRevisions = function (success, error) {
  var promise = this.ui.dropbox.client.filesListRevisions({
        ...
        mxUtils.bind(this, function (stat) {
          revs.push({
            ...
            getUrl: mxUtils.bind(this, function (page) {
              return this.ui.getUrl(window.location.pathname + '?rev=' + stat.rev + '&chrome=0&nav=1&layers=1&edit=_blank' + (page != null ? '&page=' + page : '')) + window.location.hash;
            })
          });
        })(resp.entries[i]);
      }
      ...
      if (checked) {
        try {
          ...
          if (this.ui.useCanvasForExport && /(\.png)$/i.test(this.getTitle())) {
            ...
            }), error, this.ui.getCurrentFile() != this ? this.getData() : null, p.scale, p.border);
          } else {
            doSave(this.getData());
          }
        } catch (e) {
