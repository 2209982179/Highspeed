...
(function () {
      ...
      if (acceptResponse) {
        if (userReq.getStatus() === 401) {
          if (!failOnAuth) {
            ...
            error({
              code: userReq.getStatus(),
              message: this.getErrorMessage(userReq, mxResources.get('accessDenied'))
            });
          }
      ...
      var auth = mxUtils.bind(this, function () {
        ...
        if (authRemembered != null) {
          ...
          this.ui.showAuthDialog(this, true, mxUtils.bind(this, function (remember, authSuccess) {
            var win = window.open(this.baseHostUrl + '/login/oauth/authorize?client_id=' + this.clientId + '&state=' + encodeURIComponent('cId=' + this.clientId +
  ...
  GitHubClient.prototype.showAuthorizeDialog = function (retryFn, cancelFn) {
    this.ui.showError(mxResources.get('accessDenied'), mxResources.get('authorizationRequired'), mxResources.get('help'), mxUtils.bind(this, function () {
      ...
      this.ui.openLink(window.location.hostname == 'test.draw.io' ? 'https://github.com/apps/diagrams-net-app-test' : 'https://github.com/apps/draw-io-app');
  ...
  GitHubClient.prototype.executeRequest = function (req, success, error, ignoreNotFound, returnNotFound) {
    var doExecute = mxUtils.bind(this, function (failOnAuth) {
      ...
      req.send(mxUtils.bind(this, function () {
        ...
        if (acceptResponse) {
          if (req.getStatus() >= 200 && req.getStatus() <= 299 || ignoreNotFound && req.getStatus() == 404) {
            success(req);
            ...
            try {
              ...
              if (temp != null && temp.message == 'Resource not accessible by integration') {
                ...
                error({
                  message: mxResources.get(tooLarge ? 'drawingTooLarge' : 'forbidden')
                });
              }
            } catch (e) {
              error({
            ...
            if (returnNotFound) {
              error({
                code: req.getStatus(),
                message: this.getErrorMessage(req, mxResources.get('fileNotFound'))
              });
            } else {
              authorizeApp();
            }
  ...
  GitHubClient.prototype.getSha = function (org, repo, path, ref, success, error, returnNotFound) {
    ...
    var req = new mxXmlRequest(this.baseUrl + '/repos/' + org + '/' + repo + '/contents/' + path + '?ref=' + ref + rnd, null, 'HEAD');
  ...
  GitHubClient.prototype.getFile = function (path, success, error, asLibrary, checkExists) {
    ...
    if (!checkExists && (/\.v(dx|sdx?)$/i.test(path) || /\.gliffy$/i.test(path) || /\.pdf$/i.test(path) || !this.ui.useCanvasForExport && binary)) {
      // Should never be null
      if (_token != null) {
        var url = this.baseUrl + '/repos/' + org + '/' + repo + '/contents/' + path + '?ref=' + ref;
        var headers = {
      ...
      // Adds random parameter to bypass cache
      var rnd = '&t=' + new Date().getTime();
      var req = new mxXmlRequest(this.baseUrl + '/repos/' + org + '/' + repo + '/contents/' + path + '?ref=' + ref + rnd, null, 'GET');
      this.executeRequest(req, mxUtils.bind(this, function (req) {
        try {
  ...
  GitHubClient.prototype.writeFile = function (org, repo, ref, path, message, data, sha, success, error) {
    if (data.length >= this.maxFileSize) {
      error({
        message: mxResources.get('drawingTooLarge') + ' (' + this.ui.formatFileSize(data.length) + ' / 1 MB)'
      });
    } else {
      var entity = {
        path: path,
      ...
      var req = new mxXmlRequest(this.baseUrl + '/repos/' + org + '/' + repo + '/contents/' + path, JSON.stringify(entity), 'PUT');
      this.executeRequest(req, mxUtils.bind(this, function (req) {
        success(req);
      }), mxUtils.bind(this, function (err) {
        if (err.code == 404) {
  ...
  GitHubClient.prototype.saveFile = function (file, success, error, overwrite, message) {
    ...
    var fn2 = mxUtils.bind(this, function () {
      if (this.ui.useCanvasForExport && /(\.png)$/i.test(path)) {
        ...
        }), error, this.ui.getCurrentFile() != file ? file.getData() : null, p.scale, p.border);
      } else {
  ...
  GitHubClient.prototype.showGitHubDialog = function (showFiles, fn, hideNoFilesError) {
    ...
    var btn = this.ui.createToolbarButton(Editor.refreshImage, mxResources.get('refresh'), mxUtils.bind(this, function () {
      selectRepo();
    }));
    ...
    }), null, null, 'https://www.drawio.com/blog/single-repository-diagrams', null, null, null, null, [[mxResources.get('authorize'), mxUtils.bind(this, function () {
      this.ui.openLink(window.location.hostname == 'test.draw.io' ? 'https://github.com/apps/diagrams-net-app-test' : 'https://github.com/apps/draw-io-app');
    })]], '16px');
    ...
    var selectFile = mxUtils.bind(this, function (page) {
      ...
      var req = new mxXmlRequest(this.baseUrl + '/repos/' + org + '/' + repo + '/contents/' + path + '?ref=' + encodeURIComponent(ref) + '&per_page=' + pageSize + '&page=' + page, null, 'GET');
              ...
              for (var i = 0; i < files.length; i++) {
                mxUtils.bind(this, function (file, idx) {
                  if (showFolders == (file.type == 'dir')) {
                    var temp = listItem.cloneNode();
                    temp.style.backgroundColor = gray ? Editor.isDarkMode() ? '#000000' : '#eeeeee' : '';
                    gray = !gray;
    ...
    var selectRef = mxUtils.bind(this, function (page, auto) {
      ...
      var req = new mxXmlRequest(this.baseUrl + '/repos/' + org + '/' + repo + '/branches?per_page=' + pageSize + '&page=' + page, null, 'GET');
          ...
          if (branches == null || branches.length == 0) {
            ...
            for (var i = 0; i < branches.length; i++) {
              mxUtils.bind(this, function (branch, idx) {
                var temp = listItem.cloneNode();
                temp.style.backgroundColor = idx % 2 == 0 ? Editor.isDarkMode() ? '#000000' : '#eeeeee' : '';
    ...
    var selectRepo = mxUtils.bind(this, function (page) {
      ...
      var req = new mxXmlRequest(this.baseUrl + '/user/repos?per_page=' + pageSize + '&page=' + page, null, 'GET');
      dlg.okButton.setAttribute('disabled', 'disabled');
      this.ui.spinner.spin(div, mxResources.get('loading'));
      if (scrollFn != null) {
        mxEvent.removeListener(div, 'scroll', scrollFn);
