...
function mxFreehand(graph) {
  ...
  this.setClosedPath = function (isClosed)
  //TODO add closed settings
  {
    closedPath = isClosed;
  };
  this.setAutoClose = function (isAutoClose)
  //TODO add auto closed settings
  {
  ...
  var endPath = mxUtils.bind(this, function (e) {
    if (path) {
      ...
      // Click stops drawing
      var doStop = stopClickEnabled && drawPoints.length > 0 && lastPart != null && lastPart.length < 2;
      if (!doStop) {
  ...
  var getStrokeColor = function () {
    ...
    var strokeColor = mxUtils.getValue(graph.currentVertexStyle, mxConstants.STYLE_STROKECOLOR, mxUtils.getValue(defaultStyle, mxConstants.STYLE_STROKECOLOR, '#000'));
      ...
      if (w > 0 && h > 0) {
        ...
        for (var i = 0; i < drawPoints.length; i++) {
          ...
          if (p == null) {
            ...
            var startP = drawPoints[start],
              endP = drawPoints[i - 1];
            if (!closedPath && autoClose) {
              var xdiff = startP.x - endP.x,
                ydiff = startP.y - endP.y;
  ...
  graph.addMouseListener({
    mouseDown: mxUtils.bind(this, function (sender, me) {
      if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
        ...
        drawPoints.push(pt);
        lastPart = [];
        path.setAttribute('d', perfectFreehandMode ? PerfectFreehand.getSvgPathFromStroke([[pt.x, pt.y]], perfectFreehandOptions) : strPath);
