...
GitHubFile.prototype.getId = function () {
  return encodeURIComponent(this.meta.org) + '/' + (this.meta.repo != null ? encodeURIComponent(this.meta.repo) + '/' + (this.meta.ref != null ? this.meta.ref + (this.meta.path != null ? '/' + this.meta.path : '') : '') : '');
};
...
GitHubFile.prototype.saveFile = function (title, revision, success, error, unloading, overwrite, message) {
  if (!this.isEditable()) {
    ...
    if (message != null) {
      ...
      this.peer.showCommitDialog(this.meta.name, this.getDescriptorEtag(this.meta) == null || this.meta.isNew, mxUtils.bind(this, function (message) {
        doSave(message);
      }), error);
    }
  } else if (error != null) {
