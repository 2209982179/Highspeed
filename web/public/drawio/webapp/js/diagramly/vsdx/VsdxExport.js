...
function VsdxExport(editorUi) {
  ...
  function applyMxCellStyle(state, shape, xmlDoc) {
    ...
    if (!strokeClr || strokeClr == "none") shape.appendChild(createCellElem("LinePattern", 0, xmlDoc));else shape.appendChild(createCellElem("LineColor", strokeClr, xmlDoc));
  ...
  function getArrowType(arrow, isFilled) {
    ...
    if (type != null) return type;else return 1;
  }
  ;
  function getArrowSize(size) {
    if (size == null) return 2;
    if (size <= 2) return 0;else if (size <= 3) return 1;else if (size <= 5) return 2;else if (size <= 7) return 3;else if (size <= 9) return 4;else if (size <= 22) return 5;else return 6;
  }
  ;
