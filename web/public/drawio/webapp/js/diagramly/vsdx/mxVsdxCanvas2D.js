...
mxVsdxCanvas2D.prototype.close = function () {
  ...
  if (this.lastMoveToX != this.lastX || this.lastMoveToY != this.lastY) this.lineTo(this.lastMoveToX, this.lastMoveToY);
};
...
mxVsdxCanvas2D.prototype.addForeignData = function (type, index) {
  ...
  if (type != "BMP") foreignData.setAttribute("CompressionType", type);
    ...
    if (type.indexOf('svg') == 0) {
      ...
      this.convertSvg2Png(base64, true, function (pngData) {
        that.zip.file("visio/media/" + imgName, pngData.substring(22), {
          base64: true
        }); //22 is the length of "data:image/png;base64,"
      });
    } else {
      imgName += type;
...
mxVsdxCanvas2D.prototype.text = function (x, y, w, h, str, align, valign, wrap, format, overflow, clip, rotation, dir) {
  ...
  if (this.textEnabled && str != null) {
    ...
    if (format == 'html') {
      ...
      // Removes HTML tags
      if (this.html2txtDiv == null) this.html2txtDiv = document.createElement('div');
    ...
    if (rotation != 0) this.shape.appendChild(this.createCellElem("TxtAngle", (360 - rotation) * Math.PI / 180));
