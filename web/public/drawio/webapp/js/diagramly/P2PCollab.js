...
function P2PCollab(ui, sync, channelId) {
  ...
  var colors = [
  //White font
  '#e6194b', '#3cb44b', '#4363d8', '#f58231', '#911eb4', '#f032e6', '#469990', '#9A6324', '#800000', '#808000', '#000075', '#a9a9a9',
  //Black font
  ...
  this.sendDiff = function (msg) {
    this.sendMessage('diff', encrypted ? {
      ...
      patch: encodeURIComponent(sync.objectToString(msg))
    });
  };
  this.sendNotification = function (msg) {
    this.sendMessage('notify', encrypted ? {
      ...
      data: encodeURIComponent(sync.objectToString(msg))
    });
  };
  ...
  function sendCursor(me) {
    if (ui.shareCursorPosition && !graph.isMouseDown) {
      ...
      var pageId = ui.currentPage != null ? ui.currentPage.getId() : null;
      sendMessage('cursor', {
        pageId: pageId,
        x: Math.round((me.getX() - offset.x + graph.container.scrollLeft) / s - tr.x),
        y: Math.round((me.getY() - offset.y + graph.container.scrollTop) / s - tr.y)
      });
    }
  }
  ;
  this.mouseListeners = {
    ...
    // 5 frame/sec approx TODO with 100 milli (10 fps), the cursor is smoother
  ...
  this.selectionChangeListener = function (sender, evt) {
    ...
    updateThread = window.setTimeout(function () {
      var selection = graph.getSelectionCount() > selectionLimit ? [] : graph.getSelectionCells().map(mapToIds);
      var pageId = ui.currentPage != null ? ui.currentPage.getId() : null;
  ...
  function updateCursor(entry, transition) {
    var pageId = ui.currentPage != null ? ui.currentPage.getId() : null;
    if (entry != null && entry.cursor != null && entry.lastCursor != null) {
      if (entry.lastCursor.hide != null || !ui.isShowRemoteCursors() || entry.lastCursor.pageId != null && entry.lastCursor.pageId != pageId) {
        ...
        function setPosition() {
          var cx = Math.max(graph.container.scrollLeft, Math.min(graph.container.scrollLeft + graph.container.clientWidth - entry.cursor.clientWidth, x));
          var cy = Math.max(graph.container.scrollTop - 22, Math.min(graph.container.scrollTop + graph.container.clientHeight - entry.cursor.clientHeight, y));
      ...
      switch (msg.type) {
        ...
        case 'selectionChange':
          if (urlParams['remote-selection'] != '0') {
            var pageId = ui.currentPage != null ? ui.currentPage.getId() : null;
            if (pageId == null || msgData.pageId != null && msgData.pageId == pageId) {
              createCursor();
              ...
              for (var i = 0; i < msgData.added.length; i++) {
                ...
                if (id != null) {
                  ...
                  if (cell != null) {
                    selection[id] = graph.highlightCell(cell, connectedSessions[sessionId].color, 60000, SELECTION_OPACITY, 3);
                  }
                }
              }
            }
  ...
  function createPeer(id, initiator) {
    ...
    p.on('error', function (err) {
      ...
      if (!destroyed && initiator && p.destroyed && connectedClient[id])
        //If a client left, don't try to reconnect
        {
  ...
  this.joinFile = function (check) {
    ...
    try {
      ...
      ws.addEventListener('close', mxUtils.bind(this, function (event) {
        ...
        if (!destroyed && event.code != 1000 && joinId == ws.joinId)
          //Sometimes, a delayed even sometimes is received after another socket is established
          {
