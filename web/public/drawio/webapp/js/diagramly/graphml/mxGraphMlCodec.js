  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    switch (_for) {
      case mxGraphMlConstants.NODE:
        this.nodesKeys[attName] = {
          key: id,
          keyObj: keyObj
        };
        break;
      case mxGraphMlConstants.EDGE:
        this.edgesKeys[attName] = {
          key: id,
          keyObj: keyObj
        };
        break;
      case mxGraphMlConstants.PORT:
        this.portsKeys[attName] = {
          key: id,
          keyObj: keyObj
        };
        break;
...
mxGraphMlCodec.prototype.dataElem2Obj = function (elem) {
  var ref = this.getDirectFirstChildNamedElements(elem, mxGraphMlConstants.GRAPHML_REFERENCE) || elem.getAttribute(mxGraphMlConstants.REFID);
  var refKey = null;
  var origElem = elem;
  var obj = {};
  if (ref) {
...
mxGraphMlCodec.prototype.mapObject = function (obj, mapping, map) {
  ...
  for (var key in mapping) {
    ...
    if (val == null && obj)
      //some vals doesn't need to be split
      {
        val = obj[key];
      }
    if (val != null) {
      ...
      if (typeof val === "string") {
        if (typeof mappingObj === "string") {
          ...
          switch (mappingObj.mod) {
            case "color":
              //mxGraph support alfa in colors in the standard format
          ...
          if (modVal != null) map[mappingObj.key] = modVal;
        } else {
          mappingObj(val, map);
        }
      } else if (val instanceof Array) {
        this.mapArray(val, mappingObj, map);
      } else if (val.name != null && val.value != null)
        //this is the case when a single y:Property is used 
        {
          this.mapArray([val], mappingObj, map);
        } else {
...
mxGraphMlCodec.prototype.importNode = function (nodeElement, graph, parent, dx, dy) {
  ...
  if (lblObj) lbls = this.addLabels(node, lblObj, style, graph);
  this.nodesMap[id] = {
    node: node,
    ports: portsMap
  };
...
mxGraphMlCodec.prototype.addNodeStyle = function (node, dataObj, style) {
  ...
  bevelNodeStyle["drawShadow"] = {
    key: "shadow",
    mod: "bool"
  };
  bevelNodeStyle["color"] = {
    key: "fillColor",
    mod: "color",
    addGradient: "north"
  };
  ...
  shinyPlateNodeStyle["drawShadow"] = {
    key: "shadow",
    mod: "bool"
  };
  ...
  demoGroupStyle["isCollapsible"] = {
    key: "collapsible",
    mod: "bool"
  };
  demoGroupStyle["borderColor"] = {
    key: "strokeColor",
    mod: "color"
  };
  demoGroupStyle["folderFrontColor"] = {
    key: "fillColor",
    mod: "color"
  }; //TODO fillColor always match strokeColor!
  ...
  genericNodeStyle["y:Fill"]["color2"] = {
    key: "gradientColor",
    mod: "color"
  };
  ...
  if (shape && shape.indexOf("js:") == 0) {
    switch (shape) {
      ...
      case "js:table2":
        ...
        styleMap["swimlaneFillColor"] = styleMap["fillColor"];
        var isHor = tmpMap["isHorz"] == "pool_type_lane_and_column" || tmpMap["isHorz"] == "pool_type_empty" || tmpMap["isHorz"] == "pool_type_lane";
        var th = 0,
          tw = 0;
        if (isHor) {
        ...
        try {
          ...
          for (var i = 0; i < rows.length; i++) {
            if (rows[i]["y:Insets"]) rowStartSize = Math.max(rowStartSize, parseFloat(rows[i]["y:Insets"]["left"]) + parseFloat(rows[i]["y:Insets"]["right"]));
          }
          var colStartSize = parseFloat(tmpMap["colHHeight"]);
          for (var i = 0; i < cols.length; i++) {
            if (cols[i]["y:Insets"]) colStartSize = Math.max(colStartSize, parseFloat(cols[i]["y:Insets"]["top"]) + parseFloat(cols[i]["y:Insets"]["bottom"]));
          }
          if (atts4Rows) {
...
mxGraphMlCodec.prototype.addRow = function (row, parent, odd, y, xShift, rowMapping, defRowStyle) {
  ...
  if (odd) {
    if (rowStyle["oddFill"]) rowStyle["fillColor"] = rowStyle["oddFill"];
    if (rowStyle["oddLaneFill"]) rowStyle["swimlaneFillColor"] = rowStyle["oddLaneFill"];
  } else {
    if (rowStyle["evenFill"]) rowStyle["fillColor"] = rowStyle["evenFill"];
    if (rowStyle["evenLaneFill"]) rowStyle["swimlaneFillColor"] = rowStyle["evenLaneFill"];
  }
  ...
  if (lblObj) this.addLabels(cell, lblObj, rowStyle);
  cell.style = this.styleMap2Str(rowStyle);
  parent.insert(cell);
  ...
  var subY = 0;
  if (subRow) {
    if (!(subRow instanceof Array)) subRow = [subRow];
...
mxGraphMlCodec.prototype.addColumn = function (column, parent, odd, x, yShift, colMapping, defColStyle) {
  ...
  if (odd) {
    if (colStyle["oddFill"]) colStyle["fillColor"] = colStyle["oddFill"];
    if (colStyle["oddLaneFill"]) colStyle["swimlaneFillColor"] = colStyle["oddLaneFill"];
  } else {
    if (colStyle["evenFill"]) colStyle["fillColor"] = colStyle["evenFill"];
    if (colStyle["evenLaneFill"]) colStyle["swimlaneFillColor"] = colStyle["evenLaneFill"];
  }
  ...
  if (lblObj) this.addLabels(cell, lblObj, colStyle);
  cell.style = this.styleMap2Str(colStyle);
  parent.insert(cell);
  var subCol = column["y:Column"];
  var subX = 0;
  if (subCol) {
    if (!(subCol instanceof Array)) subCol = [subCol];
...
mxGraphMlCodec.prototype.addNodeGeo = function (node, geoObj, dx, dy) {
  ...
  var x = 0,
    y = 0,
    w = 30,
    h = 30; //some node has no geometry, this is the default
  if (geoRect) {
...
mxGraphMlCodec.prototype.importEdge = function (edgeElement, graph, parent, dx, dy) {
  ...
  for (var i = 0; i < data.length; i++) {
    var d = data[i];
    var dataObj = this.dataElem2Obj(d);
    var desktopEdgeObj = dataObj["y:PolyLineEdge"] || dataObj["y:GenericEdge"] || dataObj["y:ArcEdge"] || dataObj["y:BezierEdge"] || dataObj["y:QuadCurveEdge"] || dataObj["y:SplineEdge"];
    if (dataObj.key == this.edgesKeys[mxGraphMlConstants.EDGE_GEOMETRY].key) {
      ...
      if (desktopEdgeObj["y:EdgeLabel"]) this.addLabels(edge, desktopEdgeObj["y:EdgeLabel"], style, graph, absPoints);
...
mxGraphMlCodec.prototype.addEdgePath = function (edge, pathObj, style, dx, dy) {
  var absPoints = [];
  if (pathObj) {
    var srcX = parseFloat(pathObj.sx),
      srcY = parseFloat(pathObj.sy),
      trgX = parseFloat(pathObj.tx),
      trgY = parseFloat(pathObj.ty);
...
mxGraphMlCodec.prototype.addEdgeStyle = function (edge, styleObj, styleMap) {
  ...
  var desktopLineStyleFn = function (val, map) {
    ...
    if (pattern) map["dashPattern"] = pattern;
  };
...
mxGraphMlCodec.prototype.addLabels = function (node, lblObj, nodeStyle, graph, absPoints) {
  ...
  if (lblList) {
    ...
    for (var i = 0; i < lblList.length; i++) {
      ...
      this.mapObject(lbl, {
        ...
        //?? how this is compared to alignment?
        "textColor": {
          key: "fontColor",
          mod: "color"
        },
  ...
  for (var i = 0; i < lblTxts.length; i++) {
    if (lblTxts[i]) {
      if (lblLayouts[i] && lblLayouts[i]["bpmn:ParticipantParameter"]) continue;
      ...
      } else if (lblLayouts[i]["y:InteriorLabelModel"])
        //TODO this is probably can be done by setting the value?
        {
        ...
        //Desktop format or edge
        {
          ...
          if (node.edge) {
            ...
            if (graph && absPoints && lblLayouts[i]["y:ModelParameter"] && lblLayouts[i]["y:ModelParameter"]["y:SmartEdgeLabelModelParameter"]) {
              ...
              var angle = parseFloat(layout.angle),
                distance = parseFloat(layout.distance),
                distanceToCenter = layout.distanceToCenter == "true",
                position = layout.position,
                ratio = parseFloat(layout.ratio),
                segment = parseFloat(layout.segment);
...
//TODO this code is taken from yworks.com. Is that OK?
{
  "orgchartconverters.linebreakconverter": function (e, t) {
    if (typeof e === "string") {
      var i = e;
      while (i.length > 20 && i.indexOf(" ") > -1) i = i.substring(0, i.lastIndexOf(" "));
...
var mxGraphMlShapesMap = {
  ...
  //TODO This is not close enough!
  "star6": "mxgraph.basic.6_point_star",
  //;rotation=30", //TODO requires rotation!
  ...
  //TODO not similar! //TODO fixed aspect ratio
  ...
  //;rotation=45", //TODO create this shape!
  ...
  //;rotation=90", //TODO create this shape without rotation!
