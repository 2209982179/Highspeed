 ...
 */
GraphViewer.prototype.editBlankUrl = urlParams['dev'] == '1' ? 'https://test.draw.io/' : 'https://app.diagrams.net/';
...
GraphViewer.prototype.init = function (container, xmlNode, graphConfig) {
  ...
  this.autoFit = this.graphConfig['auto-fit'] != null ? this.graphConfig['auto-fit'] : this.autoFit;
  this.autoCrop = this.graphConfig['auto-crop'] != null ? this.graphConfig['auto-crop'] : this.autoCrop;
  this.autoOrigin = this.graphConfig['auto-origin'] != null ? this.graphConfig['auto-origin'] : this.autoOrigin;
  this.allowZoomOut = this.graphConfig['allow-zoom-out'] != null ? this.graphConfig['allow-zoom-out'] : this.allowZoomOut;
  this.allowZoomIn = this.graphConfig['allow-zoom-in'] != null ? this.graphConfig['allow-zoom-in'] : this.allowZoomIn;
  this.forceCenter = this.graphConfig['forceCenter'] != null ? this.graphConfig['forceCenter'] : this.forceCenter;
  this.center = this.graphConfig['center'] != null ? this.graphConfig['center'] : this.center || this.forceCenter;
  this.checkVisibleState = this.graphConfig['check-visible-state'] != null ? this.graphConfig['check-visible-state'] : this.checkVisibleState;
  this.darkMode = this.graphConfig['dark-mode'] != null ? this.graphConfig['dark-mode'] : this.darkMode;
  this.toolbarItems = this.graphConfig.toolbar != null ? this.graphConfig.toolbar.split(' ') : [];
  ...
  this.responsive = (this.graphConfig['responsive'] != null ? this.graphConfig['responsive'] : this.responsive) && !this.zoomEnabled && !mxClient.NO_FO && !mxClient.IS_SF;
  ...
  if (xmlNode != null) {
    ...
    if (container != null) {
      var render = mxUtils.bind(this, function () {
        ...
        if (this.responsive && this.graph.dialect == mxConstants.DIALECT_SVG) {
          ...
          this.graph.sizeDidChange = function () {
            ...
            root.setAttribute('viewBox', bounds.x + tr.x - this.panDx + ' ' + (bounds.y + tr.y - this.panDy) + ' ' + (bounds.width + 1) + ' ' + (bounds.height + 1));
            this.container.style.backgroundColor = root.style.backgroundColor;
        ...
        this.graph.setBackgroundImage = function (img) {
          if (img != null && Graph.isPageLink(img.src)) {
            ...
            if (comma > 0) {
              ...
              if (index >= 0) {
                img = self.getImageForGraphModel(Editor.parseDiagramNode(self.diagrams[index]));
                img.originalSrc = src;
              }
            }
          }
        ...
        if (!this.responsive) {
          this.graph.panningHandler.isForcePanningEvent = function (me) {
            return !mxEvent.isPopupTrigger(me.getEvent()) && this.graph.container.style.overflow == 'auto';
          };
      ...
      var MutObs = window.MutationObserver || window.WebKitMutationObserver || window.MozMutationObserver;
      if (this.checkVisibleState && container.offsetWidth == 0 && this.getAncestorDetails(container) == null && typeof MutObs !== 'undefined') {
...
GraphViewer.prototype.getObservableParent = function (container) {
  ...
  while (node != document.body && node.parentNode != null && mxUtils.getCurrentStyle(node).display !== 'none') {
...
GraphViewer.prototype.getImageUrl = function (url) {
  if (url != null && url.substring(0, 7) != 'http://' && url.substring(0, 8) != 'https://' && url.substring(0, 10) != 'data:image') {
    if (url.charAt(0) == '/') {
...
GraphViewer.prototype.addSizeHandler = function () {
  ...
  this.fitGraph = mxUtils.bind(this, function (maxScale) {
    ...
    if (cachedOffsetWidth != lastOffsetWidth && !this.handlingResize) {
      ...
      this.graph.maxFitScale = maxScale != null ? maxScale : this.graphConfig.zoom || (this.allowZoomIn ? null : 1);
      ...
      if (this.graphConfig.resize != false || container.style.height == '') {
        this.updateContainerHeight(container, Math.max(this.minHeight, this.graph.getGraphBounds().height + 2 * this.graph.border + 1));
      }
      this.graph.initialViewState = {
        translate: this.graph.view.translate.clone(),
        scale: this.graph.view.scale
  ...
  var positionGraph = mxUtils.bind(this, function (origin) {
    ...
    var maxHeight = this.graphConfig['max-height'] != null ? this.graphConfig['max-height'] : container.style.height != '' && this.autoFit ? container.offsetHeight : undefined;
    if (container.offsetWidth > 0 && origin == null && this.allowZoomOut && (this.allowZoomIn || bounds.width + 2 * this.graph.border > container.offsetWidth || bounds.height + 2 * this.graph.border > maxHeight)) {
      var maxScale = null;
      ...
      this.graph.view.setTranslate(Math.floor(this.graph.border - bounds.x / this.graph.view.scale) + origin.x, Math.floor(this.graph.border - bounds.y / this.graph.view.scale) + origin.y);
...
GraphViewer.prototype.updateContainerHeight = function (container, height) {
  if (this.forceCenter || this.zoomEnabled || !this.autoFit || document.compatMode == 'BackCompat' || document.documentMode == 8) {
    ...
    try {
      ...
      if (source == null) {
        ...
        for (var i = 0; i < childCount; i++) {
          model.setVisible(model.getChildAt(model.root, i), source.isVisible(source.getChildAt(source.root, i)));
        }
      }
    } finally {
      model.endUpdate();
...
GraphViewer.prototype.addToolbar = function () {
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    if (token == 'pages') {
      ...
      pageInfo.style.cssText = 'display:inline-block;position:relative;top:5px;padding:0 4px 0 4px;' + 'vertical-align:top;font-family:Helvetica,Arial;font-size:12px;;cursor:default;color:#000;';
      ...
      if (this.zoomEnabled) {
        ...
        addButton(mxUtils.bind(this, function () {
          this.graph.view.scaleAndTranslate(this.graph.initialViewState.scale, this.graph.initialViewState.translate.x, this.graph.initialViewState.translate.y);
          ...
          if (layersDialog != null) {
            ...
            layersDialog = this.graph.createLayersDialog(mxUtils.bind(this, function () {
              if (this.autoCrop) {
                ...
                } else if (this.graph.originalViewState != null && bounds.x / v.scale + this.graph.originalViewState.translate.x - v.translate.x > 0 && bounds.y / v.scale + this.graph.originalViewState.translate.y - v.translate.y > 0) {
                  v.setTranslate(this.graph.originalViewState.translate.x, this.graph.originalViewState.translate.y);
      ...
      if (def != null) {
        def.elem = addButton(def.enabled == null || def.enabled ? def.handler : function () {}, def.image, def.title, def.enabled);
      }
    }
  }
  ...
  if (this.graphConfig.title != null) {
    ...
    filename.style.cssText = 'display:inline-block;position:relative;padding:3px 6px 0 6px;' + 'vertical-align:top;font-family:Helvetica,Arial;font-size:12px;top:4px;cursor:default;color:#000;';
  ...
  var enter = mxUtils.bind(this, function () {
    toolbar.style.width = this.graphConfig['toolbar-position'] == 'inline' ? 'auto' : Math.max(this.minToolbarWidth, container.offsetWidth) + 'px';
  ...
  graph.addClickHandler(this.graphConfig.highlight, mxUtils.bind(this, function (evt, href, associatedCell) {
    ...
    if (ui != null) {
      if (href == null || graph.isCustomLink(href)) {
        mxEvent.consume(evt);
      } else if (!graph.isExternalProtocol(href) && !graph.isBlankLink(href)) {
    ...
    } else if (href != null && ui == null && graph.isCustomLink(href) && (mxEvent.isTouchEvent(evt) || !mxEvent.isPopupTrigger(evt)) && graph.customLinkClicked(href, associatedCell)) {
      // Workaround for text selection in Firefox on Windows
      mxUtils.clearSelection();
      mxEvent.consume(evt);
    }
  }), mxUtils.bind(this, function (evt) {
    if (ui == null && this.lightboxClickEnabled && (!mxEvent.isTouchEvent(evt) || this.toolbarItems.length == 0)) {
      try {
        this.showLightbox();
      } catch (e) {
        alert(e.message);
...
GraphViewer.prototype.showLightbox = function (editable, closable, target) {
  if (this.graphConfig.lightbox == 'open' || window.self !== window.top) {
    if (this.lightboxWindow != null && !this.lightboxWindow.closed) {
      ...
      editable = editable != null ? editable : this.graphConfig.editable != null ? this.graphConfig.editable : true;
      ...
      this.lightboxWindow = window.open((urlParams['dev'] != '1' ? EditorUi.lightboxHost : 'https://test.draw.io') + '/#P' + encodeURIComponent(JSON.stringify(param)));
    }
  } else {
...
GraphViewer.prototype.showLocalLightbox = function (container) {
  ...
  urlParams['layer-ids'] = this.graphConfig.layerIds != null && this.graphConfig.layerIds.length > 0 ? this.graphConfig.layerIds.join(' ') : null;
  urlParams['nav'] = this.graphConfig.nav != false ? '1' : '0';
...
GraphViewer.logAncestorFrames = function () {
  ...
  try {
    if (window.location.ancestorOrigins && window.location.hostname && window.location.ancestorOrigins.length && window.location.ancestorOrigins.length > 0) {
      ...
      if (hostname.endsWith('.draw.io') && window.location.ancestorOrigins.length == 1 && window.location.ancestorOrigins[0] && window.location.ancestorOrigins[0].endsWith('.atlassian.net')) {
        // do not log *.draw.io domains embedded directly into atlassian.net
      } else if (window.location.ancestorOrigins.length > 0) {
        var img = new Image();
        img.src = 'https://log.diagrams.net/images/1x1.png?src=ViewerAncestorFrames' + (typeof window.EditorUi !== 'undefined' ? '&v=' + encodeURIComponent(EditorUi.VERSION) : '') + '&data=' + encodeURIComponent(message);
      }
    }
  } catch (e) {
    // ignore
...
GraphViewer.getUrl = function (url, onload, onerror) {
  if (GraphViewer.cachedUrls[url] != null) {
    ...
    var xhr = navigator.userAgent != null && navigator.userAgent.indexOf('MSIE 9') > 0 ? new XDomainRequest() : new XMLHttpRequest();
    xhr.open('GET', url);
    xhr.onload = function () {
