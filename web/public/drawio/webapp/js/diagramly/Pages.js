  ...
  if (!this.editor.graph.standalone) {
    ...
    graph.view.validateBackground = mxUtils.bind(this, function () {
      if (this.tabContainer != null) {
        ...
        if (this.fileNode == null || this.pages == null || this.pages.length == 1 && urlParams['pages'] == '0') {
...
Graph.prototype.addExtFont = function (fontName, fontUrl, dontRemember) {
  ...
  if (fontName && fontUrl) {
    if (urlParams['ext-fonts'] != '1') {
      ...
      Graph.recentCustomFonts[fontName.toLowerCase()] = {
        name: fontName,
        url: fontUrl
      };
    }
...
EditorUi.prototype.createPageInsertTab = function () {
  return this.createControlTab(mxResources.get('insertPage'), Editor.plusImage, mxUtils.bind(this, function (evt) {
    this.insertPage();
    mxEvent.consume(evt);
  }));
};
...
EditorUi.prototype.createPageMenuTab = function () {
  return this.createControlTab(mxResources.get('pages'), Editor.verticalDotsImage, mxUtils.bind(this, function (evt) {
...
EditorUi.prototype.getLinkForPage = function (page, params, lightbox) {
  if (page != null && !mxClient.IS_CHROMEAPP && !EditorUi.isElectronApp) {
    ...
    if (file != null && file.constructor != LocalFile && this.getServiceName() == 'draw.io') {
      ...
      return (lightbox && urlParams['dev'] != '1' ? EditorUi.lightboxHost : mxClient.IS_CHROMEAPP || EditorUi.isElectronApp || !/.*\.draw\.io$/.test(window.location.hostname) ? EditorUi.drawHost : 'https://' + window.location.host) + '/' + search + '#' + file.getHash();
    }
  }
  return null;
};
...
EditorUi.prototype.showPageLinkDialog = function (page) {
  var graph = this.editor.graph;
  this.showPublishLinkDialog(mxResources.get('url'), true, null, null, mxUtils.bind(this, function (linkTarget, linkColor, allPages, lightbox, editLink, layers) {
  ...
  codec.afterEncode = function (enc, obj, node) {
    ...
    if (obj.index == null) {
      ...
      if (obj.relatedPage.viewState != null) {
        node.setAttribute('viewState', JSON.stringify(obj.relatedPage.viewState, function (key, value) {
