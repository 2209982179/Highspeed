...
DrawioFileSync = function (file) {
  ...
  this.pusherErrorListener = mxUtils.bind(this, function (err) {
    if (err.error != null && err.error.data != null && err.error.data.code === 4004) {
  ...
  this.connectionListener = mxUtils.bind(this, function () {
    ...
    this.updateStatus();
    if (this.isConnected()) {
      if (!this.announced && Editor.enableRealtimeCache && !Editor.p2pSyncNotify) {
...
DrawioFileSync.prototype.start = function () {
  ...
  if (DrawioFileSync.PULLING_MODE && this.puller == null && document.visibilityState != 'hidden') {
    if (this.puller == null) {
      this.puller = new DrawioFilePuller(this.file, this);
    }
    this.puller.start(this.file.getPullingInterval());
    EditorUi.debug('DrawioFileSync.start (Pulling)', [this], 'version', DrawioFileSync.PROTOCOL, 'rev', this.file.getCurrentRevisionId());
    updateStatus = true;
  } else if (!DrawioFileSync.PULLING_MODE && this.pusher == null && this.channelId != null && document.visibilityState != 'hidden') {
    ...
    if (this.pusher != null) {
      ...
      try {
        ...
        EditorUi.debug('DrawioFileSync.start', [this], 'version', DrawioFileSync.PROTOCOL, 'rev', this.file.getCurrentRevisionId());
      } catch (e) {
        // ignore
      }
...
DrawioFileSync.prototype.updateRealtime = function () {
  if (this.isValidState()) {
    if (this.file.isRealtimeEnabled() && this.file.isRealtimeSupported() && this.isRealtimeActive()) {
    ...
    if (DrawioFileSync.ENABLE_SOCKETS && this.file.isRealtime() && this.p2pCollab == null && this.channelId != null) {
...
DrawioFileSync.prototype.resetRealtime = function () {
  ...
  if (shadow != null) {
    var patch = this.ui.diffPages(shadow, this.file.ownPages);
...
DrawioFileSync.prototype.updateCollaboratorsElement = function () {
  if (this.collaboratorsElement != null) {
    ...
    if (status != null) {
      this.collaboratorsElement.style.backgroundImage = 'url(' + Editor.syncProblemImage + ')';
...
DrawioFileSync.prototype.createCollaboratorsElement = function () {
  ...
  mxEvent.addListener(elt, 'click', mxUtils.bind(this, function (evt) {
    if (this.file.isRealtimeEnabled() && this.file.isRealtimeSupported()) {
      var status = this.ui.getNetworkStatus();
      this.ui.showError(mxResources.get('realtimeCollaboration'), mxUtils.htmlEntities(status != null ? status : mxResources.get('online')));
    } else {
...
DrawioFileSync.prototype.updateStatus = function () {
  if (this.isConnected() && this.lastActivity != null && (Date.now() - this.lastActivity) / 1000 > this.inactivityTimeoutSeconds) {
    this.stop();
  }
  if (!this.file.isModified() && !this.file.inConflictState && this.file.autosaveThread == null && !this.file.savingFile && !this.file.redirectDialogShowing) {
    if (this.enabled && this.ui.statusContainer != null) {
      ...
      this.ui.editor.setStatus('<div ' + rev + 'title="' + mxUtils.htmlEntities(label) + '">' + mxUtils.htmlEntities(label) + '</div>' + (!this.file.isEditable() ? '<div class="geStatusBox" title="' + mxUtils.htmlEntities(mxResources.get('readOnly')) + '">' + mxUtils.htmlEntities(mxResources.get('readOnly')) + '</div>' : '') + (status != null ? '<div class="geStatusBox" title="' + mxUtils.htmlEntities(status) + '">' + mxUtils.htmlEntities(status) + '</div>' : '') + (msg != null ? ' <div class="geStatusBox" data-effect="fade" title="' + mxUtils.htmlEntities(msg) + '">' + mxUtils.htmlEntities(msg) + '</div>' : ''));
      this.resetUpdateStatusThread();
    } else {
...
DrawioFileSync.prototype.notify = function (msg) {
  ...
  if (Editor.enableRealtimeCache && !Editor.p2pSyncNotify) {
    mxUtils.post(EditorUi.cacheUrl, this.getIdParameters() + '&msg=' + encodeURIComponent(this.objectToString(msg)));
  } else if (this.p2pCollab != null) {
    this.p2pCollab.sendNotification(msg);
  }
  EditorUi.debug('DrawioFileSync.notify', [this], 'enableRealtimeCache', Editor.enableRealtimeCache, 'p2pSyncNotify', Editor.p2pSyncNotify, 'msg', msg);
};
...
DrawioFileSync.prototype.handleMessageData = function (data) {
  if (data.a == 'desc') {
    ...
    if (data.name != null) {
      this.lastMessage = mxResources.get(data.a == 'join' ? 'userJoined' : 'userLeft', [decodeURIComponent(data.name)]);
      this.resetUpdateStatusThread();
      this.updateStatus();
    }
  ...
  if (this.reloadThread == null) {
    ...
    if (count < this.maxOptimisticRetries) {
      this.reloadThread = window.setTimeout(mxUtils.bind(this, function () {
        EditorUi.debug('DrawioFileSync.optimisticSync', [this], 'attempt', count, 'of', this.maxOptimisticRetries, 'remoteFileChanged', this.remoteFileChanged);
...
DrawioFileSync.prototype.fileChangedNotify = function (data, immediate) {
  if (this.isValidState()) {
    EditorUi.debug('DrawioFileSync.fileChangedNotify', [this], 'data', [data], 'immediate', immediate, 'saving', this.file.savingFile);
    if (this.file.savingFile) {
...
DrawioFileSync.prototype.doSendLocalChanges = function (changes) {
  if (!this.file.ignorePatches(changes)) {
    ...
    if (this.p2pCollab != null) {
      ...
      if (this.maxSyncMessageSize == 0 || data.length < this.maxSyncMessageSize) {
    ...
    EditorUi.debug('DrawioFileSync.doSendLocalChanges', [this], 'changes', changes, skipped ? '(skipped)' : '');
  }
};
...
DrawioFileSync.prototype.scheduleCleanup = function (lazy) {
  ...
  EditorUi.debug('DrawioFileSync.scheduleCleanup', [this], 'lazy', lazy, 'delay', delay, 'prev', prev, 'thread', this.cleanupThread);
};
...
DrawioFileSync.prototype.cleanup = function (success, error, checkFile) {
  ...
  if (this.isValidState() && !this.file.inConflictState && this.file.isRealtime() && !this.file.isModified()) {
    var patches = [this.ui.diffPages(this.ui.pages, this.file.ownPages)];
    this.file.theirPages = this.ui.clonePages(this.file.ownPages);
    if (urlParams['test'] == '1') {
      EditorUi.debug('DrawioFileSync.cleanup', [this], 'thread', thread, 'patches', patches, 'checkFile', checkFile, 'checksum', this.ui.getHashValueForPages(this.ui.pages));
    }
    if (!this.file.ignorePatches(patches)) {
      this.file.patch(patches);
    }
    if (!checkFile) {
      if (!document.hidden && urlParams['test'] == '1' && urlParams['checksum'] == '1') {
        this.testChecksum();
      }
      if (success != null) {
        success();
      ...
      this.file.getLatestVersion(mxUtils.bind(this, function (newFile) {
        try {
          if (this.isValidState() && !this.file.inConflictState && this.file.isRealtime()) {
            ...
            patches = [this.ui.diffPages(this.ui.pages, pages), this.ui.diffPages(pages, this.file.ownPages)];
            if (!this.file.ignorePatches(patches)) {
              this.file.patch(patches);
            }
            EditorUi.debug('DrawioFileSync.cleanup', [this], 'newFile', newFile, 'patches', patches);
          }
          if (success != null) {
            success();
          }
    ...
    success();
    EditorUi.debug('DrawioFileSync.cleanup', [this], 'checkFile', checkFile, 'modified', this.file.isModified());
  }
};
...
DrawioFileSync.prototype.testChecksum = function () {
  ...
  this.file.getLatestVersion(mxUtils.bind(this, function (latestFile) {
    if (!document.hidden) {
      var remoteChecksum = this.ui.getHashValueForPages(latestFile.getShadowPages());
      var descChecksum = latestFile.getDescriptorChecksum(latestFile.getDescriptor());
      var remoteRev = latestFile.getCurrentRevisionId();
      EditorUi.debug('DrawioFileSync.testChecksum', 'local', [this.file], 'modified', this.file.isModified(), 'inConflictState', this.file.inConflictState, 'autosaveThread', this.file.autosaveThread, 'savingFile', this.file.savingFile, 'localFileWasChanged', this.localFileWasChanged, 'remoteFileChanged', this.remoteFileChanged, 'cleanup', this.cleanupThread, 'checksum', localChecksum);
      EditorUi.debug('DrawioFileSync.testChecksum', 'remote', [latestFile], 'rev', remoteRev == localRev, 'desc', descChecksum == remoteChecksum, 'checksum', remoteChecksum);
      if (remoteChecksum != localChecksum) {
        EditorUi.debug('DrawioFileSync.testChecksum', [this], 'checksums do not match');
        this.ui.alert('Checksums do not match');
      } else {
        EditorUi.debug('DrawioFileSync.testChecksum', [this], 'checksums match');
      }
    }
  }), mxUtils.bind(this, function (err) {
    EditorUi.debug('DrawioFileSync.testChecksum', [this], 'checksum test error', err);
  }));
};
...
DrawioFileSync.prototype.extractLocal = function (patch) {
  return mxUtils.isEmptyObject(patch) ? {} : this.ui.diffPages(this.file.theirPages, this.ui.patchPages(this.ui.clonePages(this.file.theirPages), patch));
};
  ...
  if (patch[EditorUi.DIFF_UPDATE] != null) {
    for (var id in patch[EditorUi.DIFF_UPDATE]) {
      ...
      if (diff.cells != null && diff.cells[EditorUi.DIFF_REMOVE] != null) {
        ...
        temp.cells = {};
        temp.cells[EditorUi.DIFF_REMOVE] = diff.cells[EditorUi.DIFF_REMOVE];
      }
    }
  }
  return result;
...
DrawioFileSync.prototype.patchRealtime = function (patches, backup, own, immediate) {
  ...
  if (this.file.isRealtime()) {
    ...
    all = this.extractRemove(this.ui.diffPages(this.file.getShadowPages(), this.ui.pages));
    var local = this.extractRemove(this.extractLocal(all));

    // Applies incoming, own and local changes to own pages
    var applied = (own == null ? patches : patches.concat(own)).concat([local]);
    this.file.ownPages = this.ui.applyPatches(this.file.ownPages, applied, true, backup);
    ...
    if (!mxUtils.isEmptyObject(local)) {
      ...
      this.scheduleCleanup(immediate != null ? false : null);
    }
    EditorUi.debug('DrawioFileSync.patchRealtime', [this], 'patches', patches, 'backup', backup, 'own', own, 'all', all, 'local', local, 'applied', applied, 'immediate', immediate);
  }
  return all;
};
...
DrawioFileSync.prototype.sendLocalChanges = function () {
  try {
    if (this.file.isRealtime() && this.localFileWasChanged) {
      ...
      this.file.ownPages = this.ui.patchPages(this.file.ownPages, patch, true);
      this.snapshot = snapshot;

      // Creates patch for cross references
      var resolve = this.ui.resolveCrossReferences(patch, this.ui.diffPages(this.file.ownPages, this.ui.pages));

      // Patches own pages to resolve cross references
      this.file.ownPages = this.ui.patchPages(this.file.ownPages, resolve, true);
    ...
    EditorUi.logError('Error in sendLocalChanges', null, this.file.getMode() + '.' + this.file.getId(), uid, e);
  }
};
...
DrawioFileSync.prototype.doReceiveRemoteChanges = function (changes) {
  if (this.file.isRealtime() && this.isRealtimeActive()) {
    ...
    this.file.theirPages = this.ui.applyPatches(this.file.theirPages, changes);
    this.scheduleCleanup();
    EditorUi.debug('DrawioFileSync.doReceiveRemoteChanges', [this], 'changes', changes);
  }
};
...
DrawioFileSync.prototype.merge = function (patches, checksum, desc, success, error, abort, immediate) {
  try {
    ...
    if (!ignored) {
      ...
      this.file.backupPatch = this.file.isModified() && !this.file.isRealtime() ? this.ui.diffPages(shadow, this.ui.pages) : null;
      var pending = !this.file.isRealtime() ? null : this.ui.diffPages(shadow, this.file.ownPages);
      shadow = this.ui.applyPatches(shadow, patches);
      var current = checksum == null ? null : this.ui.getHashValueForPages(shadow);
      EditorUi.debug('DrawioFileSync.merge', [this], 'patches', patches, 'backup', this.file.backupPatch, 'pending', pending, 'checksum', checksum, 'current', current, 'valid', checksum == current, 'attempt', this.catchupRetryCount, 'of', this.maxCatchupRetries, 'from', this.file.getCurrentRevisionId(), 'to', target, 'etag', this.file.getDescriptorEtag(desc), 'immediate', immediate);
      ...
      if (checksum != null && checksum != current) {
        ...
        var logError = mxUtils.bind(this, function (failed) {
          try {
            var user = this.file.getCurrentUser();
            var uid = user != null ? user.id : 'unknown';
            var id = this.file.getId() != '' ? this.file.getId() : '(' + this.ui.hashValue(this.file.getTitle()) + ')';
            var bytes = JSON.stringify(patches).length;
            EditorUi.logError('Merge checksum fallback ' + (failed ? 'failed' : 'success') + ' ' + id, null, this.file.getMode() + '.' + this.file.getId(), 'user_' + uid + '-client_' + this.clientId + '-bytes_' + bytes + '-patches_' + patches.length + '-size_' + this.file.getSize() + (checksum != null ? '-expected_' + checksum : '') + (current != null ? '-current_' + current : '') + '-from_' + this.ui.hashValue(this.file.getCurrentRevisionId()) + '-to_' + this.ui.hashValue(target));
          } catch (e) {
            // ignore
          }
        });
        ...
        if (this.patchRealtime(patches, null, pending, immediate) == null) {
          this.file.patch(patches, DrawioFile.LAST_WRITE_WINS ? this.file.backupPatch : null);
        }
    ...
    try {
      if (this.file.errorReportsEnabled) {
        ...
        this.file.sendErrorReport('Error in merge', 'From: ' + from + '\nTo: ' + to + '\nChecksum: ' + checksum + '\nPatches:\n' + this.file.compressReportData(JSON.stringify(patches, null, 2)), e);
      } else {
        var user = this.file.getCurrentUser();
        var uid = user != null ? user.id : 'unknown';
        EditorUi.logError('Error in merge', null, this.file.getMode() + '.' + this.file.getId(), uid, e);
      }
    } catch (e2) {
      // ignore
    }
...
DrawioFileSync.prototype.fileChanged = function (success, error, abort, lazy, immediate) {
  var thread = window.setTimeout(mxUtils.bind(this, function () {
    if (abort == null || !abort()) {
      EditorUi.debug('DrawioFileSync.fileChanged', [this], 'lazy', lazy, 'immediate', immediate, 'remoteFileChanged', this.remoteFileChanged, 'valid', this.isValidState());
      if (!this.isValidState()) {
        if (error != null) {
          error();
        }
...
DrawioFileSync.prototype.fastForward = function (desc) {
  ...
  this.cleanupThread = null;
  if (urlParams['test'] == '1') {
    EditorUi.debug('DrawioFileSync.fastForward', [this], 'desc', [desc], 'cleanup', thread, 'checksum', this.ui.getHashValueForPages(this.ui.pages));
  }
  if (!document.hidden && urlParams['test'] == '1' && urlParams['checksum'] == '1' && this.cleanupThread == null) {
    this.testChecksum();
  }
};
...
DrawioFileSync.prototype.catchup = function (desc, success, error, abort, immediate) {
  if (desc != null && (abort == null || !abort())) {
    var source = this.file.getCurrentRevisionId();
    var target = this.file.getDescriptorRevisionId(desc);
    EditorUi.debug('DrawioFileSync.catchup', [this], 'desc', [desc], 'from', source, 'to', target, 'immediate', immediate, 'valid', this.isValidState());
    if (source == target) {
      this.file.patchDescriptor(this.file.getDescriptor(), desc);
      if (urlParams['test'] == '1') {
        EditorUi.debug('DrawioFileSync.catchup', [this], 'up to date', 'cleanup', this.cleanupThread, 'checksum', this.ui.getHashValueForPages(this.ui.pages));
      }
      if (!document.hidden && urlParams['test'] == '1' && urlParams['checksum'] == '1' && this.cleanupThread == null) {
        this.testChecksum();
      }
      ...
      if (checksum != null && checksum == this.ui.getHashValueForPages(this.ui.pages)) {
        // Fast-forward to current state if checksum matches
        this.fastForward(desc);
        if (success != null) {
          success(true);
        }
      } else if (!Editor.enableRealtimeCache || secret == null || urlParams['lockdown'] == '1') {
        ...
        var doCatchup = mxUtils.bind(this, function () {
          if (abort == null || !abort()) {
            ...
            if (source != this.file.getCurrentRevisionId()) {
              ...
              mxUtils.get(EditorUi.cacheUrl + '?id=' + encodeURIComponent(this.channelId) + '&from=' + encodeURIComponent(source) + '&to=' + encodeURIComponent(target) + (secret != null ? '&secret=' + encodeURIComponent(secret) : ''), mxUtils.bind(this, function (req) {
                ...
                if (acceptResponse && (abort == null || !abort())) {
                  ...
                  if (source != this.file.getCurrentRevisionId()) {
                    ...
                    var checksum = null;
                    var temp = [];
                    EditorUi.debug('DrawioFileSync.doCatchup', [this], 'request', [req], 'status', req.getStatus(), 'cacheReadyRetryCount', cacheReadyRetryCount, 'maxCacheReadyRetries', this.maxCacheReadyRetries);
                    if (req.getStatus() >= 200 && req.getStatus() <= 299 && req.getText().length > 0) {
                      try {
                        ...
                        if (result != null && result.length > 0) {
                          for (var i = 0; i < result.length; i++) {
                            ...
                            if (value.v > DrawioFileSync.PROTOCOL) {
                              ...
                              temp = [];
                              break;
                            } else if (value.v === DrawioFileSync.PROTOCOL && value.d != null) {
                        ...
                        EditorUi.debug('DrawioFileSync.doCatchup', [this], 'response', [result], 'status', failed ? 'failed' : 'ok', 'temp', temp, 'checksum', checksum);
                      } catch (e) {
                        temp = [];
                    ...
                    try {
                      if (temp.length > 0) {
                        ...
                        this.merge(temp, checksum, desc, success, error, abort, immediate);
                      }
                      // Retries if cache entry was not yet there
                      else if (cacheReadyRetryCount <= this.maxCacheReadyRetries - 1 && !failed && req.getStatus() != 401 && req.getStatus() != 503 && req.getStatus() != 410) {
...
DrawioFileSync.prototype.descriptorChanged = function (source) {
  ...
  if (this.channelId != null) {
    ...
    mxUtils.post(EditorUi.cacheUrl, this.getIdParameters() + '&from=' + encodeURIComponent(source) + '&to=' + encodeURIComponent(target) + '&msg=' + encodeURIComponent(msg) + '&data=' + encodeURIComponent(data));
    this.file.stats.bytesSent += data.length;
    this.file.stats.msgSent++;
    EditorUi.debug('DrawioFileSync.descriptorChanged', [this], 'from', source, 'to', target);
  }
  this.updateStatus();
};
...
DrawioFileSync.prototype.createToken = function (secret, success, error) {
  ...
  mxUtils.get(EditorUi.cacheUrl + '?id=' + encodeURIComponent(this.channelId) + '&secret=' + encodeURIComponent(secret), mxUtils.bind(this, function (req) {
    window.clearTimeout(timeoutThread);
    if (acceptResponse) {
      if (req.getStatus() >= 200 && req.getStatus() <= 299) {
        ...
        error({
          code: req.getStatus(),
          message: 'Token Error ' + req.getStatus()
        });
      }
    }
  }), error);
...
DrawioFileSync.prototype.fileSaved = function (pages, lastDesc, success, error, token, checksum) {
  ...
  if (!this.ui.isOffline(true) && !this.file.inConflictState && !this.file.redirectDialogShowing) {
    ...
    if (this.channelId != null) {
      ...
      var target = this.file.getCurrentRevisionId();
      if (secret == null || token == null || urlParams['lockdown'] == '1' || !Editor.enableRealtimeCache) {
        this.notify(msg);
        if (success != null) {
          success();
        }
        EditorUi.debug('DrawioFileSync.fileSaved', [this], 'from', source, 'to', target, 'etag', this.file.getCurrentEtag());
      } else {
        ...
        mxUtils.post(EditorUi.cacheUrl, this.getIdParameters() + '&from=' + encodeURIComponent(source) + '&to=' + encodeURIComponent(target) + (!Editor.p2pSyncNotify ? '&msg=' + encodeURIComponent(this.objectToString(msg)) : '') + (secret != null ? '&secret=' + encodeURIComponent(secret) : '') + (lastSecret != null ? '&last-secret=' + encodeURIComponent(lastSecret) : '') + (data.length < this.maxCacheEntrySize ? '&data=' + encodeURIComponent(data) : '') + (token != null ? '&token=' + encodeURIComponent(token) : ''), mxUtils.bind(this, function (req) {
          window.clearTimeout(timeoutThread);
          if (acceptResponse) {
            if (req.getStatus() >= 200 && req.getStatus() <= 299) {
              ...
              error({
                message: mxResources.get('realtimeCollaboration') + (req.getStatus() != 0 ? ': ' + req.getStatus() : '')
              });
            }
          }
        }));
        EditorUi.debug('DrawioFileSync.fileSaved', [this], 'from', source, 'to', target, 'etag', this.file.getCurrentEtag(), 'diff', diff, data.length, 'bytes', 'checksum', checksum);
      }
...
DrawioFileSync.prototype.getIdParameters = function () {
  ...
  if (this.pusher != null && this.pusher.connection != null && this.pusher.connection.socket_id != null) {
...
DrawioFileSync.prototype.fileConflict = function (desc, success, error) {
  ...
  EditorUi.debug('DrawioFileSync.fileConflict', [this], 'desc', [desc], 'catchupRetryCount', this.catchupRetryCount, 'maxCatchupRetries', this.maxCatchupRetries);
